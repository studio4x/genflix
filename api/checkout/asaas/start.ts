...
import { z } from 'zod';
import { fetchPaymentGatewayConfiguration, getAsaasAccessToken, getAsaasBaseUrl, getBearerToken, getHeaderValue, getRequestOrigin, } from '../../_shared/asaas.js';
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
    ...
    if (!accessToken) {
        jsonResponse(res, 401, { error: "Token de acesso obrigat?rio para iniciar o checkout." });
        return;
    }
    ...
    if (buyerUserId && buyerUserId !== authUser.id) {
        jsonResponse(res, 403, { error: "buyerUserId no corresponde ao usu?rio autenticado." });
        return;
    }
    ...
    if (usingLegacyCourseSalesSchema) {
        jsonResponse(res, 503, { error: "Checkout indisponvel ate concluir a migracao comercial do banco." });
        return;
    }
    const priceCents = Number(course.price_cents  0);
    const origin = getRequestOrigin(req);
    const buyerName = limitAsaasName(parsed.data.buyerName 
        profile.full_name 
        (authUser.user_metadata.full_name as string | undefined) 
        authUser.email 
        'Aluno GenFlix', 'Aluno GenFlix');
    const buyerEmail = normalizeOptionalText(parsed.data.buyerEmail) 
        normalizeOptionalText(profile.email) 
        normalizeOptionalText(authUser.email);
    const buyerDocument = normalizeDocument(parsed.data.buyerDocument) 
    ...
    const buyerPhone = normalizePhoneNumber(parsed.data.buyerPhone) 
    ...
    if (priceCents <= 0) {
        ...
        await queueUserNotification(adminClient, {
            ...
            body: `O acesso ao curso ${course.title  'selecionado'} ja est disponvel na sua area do aluno.`,
            category: 'payment',
            priority: 'high',
    ...
    if (sessionInsert.error) {
        ...
        jsonResponse(res, 500, { error: "No foi possvel registrar o pedido antes de enviar ao pagamento." });
        return;
    }
    jsonResponse(res, 200, {
        checkoutUrl,
