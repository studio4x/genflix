...
function isRateLimited(ip: string) {
    ...
    if (formRateLimitStore.size > 10000) {
...
function createAdminClient() {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
        throw new Error("Configura\u00E7ao ausente: Supabase URL e service role sao obrigatrios.");
    }
...
async function loadAdminProfiles(adminClient: SupabaseClient, fallbackEmail: string | null) {
    ...
    const adminUserIds = (roleRowsResult.data ?? [])
        ...
        return roles.some((role) => role.code === 'admin') ? [(row as {
                user_id: string;
            }).user_id] : [];
    });
...
async function notifyAdmins(adminClient: SupabaseClient, submission: PublicFormSubmission, template: NotificationTemplate, adminEmail: string | null) {
    ...
    const notificationPromises = adminProfiles.map((profile) => queueUserNotification(adminClient, {
        userId: profile.id,
