...
export async function fetchAsaasBalance(environment: AsaasEnvironment, accessToken: string) {
    ...
    if (!response.ok) {
        throw new Error(payload?.errors?.[0]?.description ?? "N?o foi possvel consultar o saldo disponvel no Asaas.");
    }
...
export async function createAsaasPixTransfer(input: {
    ...
    if (!pixAddressKeyType) {
        throw new Error("Tipo de chave PIX invlido para transferencia Asaas.");
    }
    const requestBody = {
    ...
    if (!response.ok || !payload?.id) {
        throw new Error(readAsaasError(payload, "N?o foi possvel criar a transferencia PIX no Asaas."));
    }
    return {
        requestBody,
        payload,
...
export async function fetchAsaasTransfer(input: {
    ...
    const payload = await response.json().catch(() => null) as AsaasTransferPayload | null;
    if (!response.ok || !payload?.id) {
        throw new Error(readAsaasError(payload, "N?o foi possvel consultar a transferencia PIX no Asaas."));
    }
    return payload;
}
