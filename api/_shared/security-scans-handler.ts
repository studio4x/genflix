...
async function readFilesRecursively(rootDir: string, extensions: string[]): Promise<Array<{ filePath: string; content: string }>> {
...
async function scanDangerousHtml(): Promise<ScannerFinding[]> {
  ...
  for (const file of files) {
    ...
    if (hasSanitizationHint) continue
...
async function scanVercelSecurityHeaders(): Promise<ScannerFinding[]> {
  try {
    ...
    const parsed = JSON.parse(content) as { headers?: Array<{ source?: string; headers?: Array<{ key: string; value: string }> }> }
...
async function applyHeaderFix() {
  ...
  return 'Headers de seguranca adicionados/normalizados em vercel.json.'
}
...
async function ensureSettings(adminClient: SupabaseClient) {
  ...
  if (settingsResult.data) return settingsResult.data
...
async function executeScan(adminClient: SupabaseClient, userId: string | null, triggerSource: 'manual' | 'scheduled') {
  ...
  try {
    ...
    if (insertPayload.length > 0) {
      ...
      if (insertFindingsResult.error) throw insertFindingsResult.error
    }
