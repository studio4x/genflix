...
async function assertAdmin(req: ApiRequest, res: ApiResponse): Promise<Context | null> {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
        jsonResponse(res, 500, { error: "Configura??o ausente: SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY sao obrigat?rios." });
        return null;
    }
    ...
    if (userResult.error || !userResult.data.user) {
        jsonResponse(res, 401, { error: "Token invlido ou expirado." });
        return null;
    }
    ...
    if (rolesResult.error) {
        jsonResponse(res, 500, { error: "No foi possvel validar as permisses do usurio." });
        return null;
    }
    ...
    if (!roles.includes('admin')) {
        jsonResponse(res, 403, { error: "Apenas administradores podem acessar a central de seguran?a." });
        return null;
    }
    return { adminClient, userId: userResult.data.user.id, isCron: false };
}
async function readFilesRecursively(rootDir: string, extensions: string[]): Promise<Array<{
    filePath: string;
    content: string;
}>> {
    const result: Array<{
        ...
        while ((match = regex.exec(content))) {
            ...
            if (status !== 'OK') {
                findings.push({
                    ...
                    severity: 'high',
                    description: "Existe item no checklist de auditoria que ainda no foi marcado como resolvido.",
                    evidence: `Arquivo: docs/AUDITORIA_VULNERABILIDADES_SEGURANCA.md | item: ${title} | status atual: [${status}]`,
                    recommendation: "Corrigir a vulnerabilidade correspondente e atualizar o checklist para [OK] apos validao.",
                    fixAvailable: false,
...
async function scanDangerousHtml(): Promise<ScannerFinding[]> {
    ...
    for (const file of files) {
        ...
        findings.push({
            ...
            severity: 'high',
            description: "Renderizacao direta de HTML pode permitir XSS persistente/refletido se o contedo no for sanitizado.",
...
async function scanCorsWildcard(): Promise<ScannerFinding[]> {
    ...
    for (const file of files) {
        ...
        findings.push({
            ...
            severity: 'medium',
            description: 'Access-Control-Allow-Origin com * amplia superficie de chamada cross-origin.',
            evidence: `Arquivo: ${toRelative(file.filePath)}`,
...
async function scanVercelSecurityHeaders(): Promise<ScannerFinding[]> {
    try {
        ...
        return [{
                ...
                severity: 'medium',
                description: "No foram encontrados todos os headers de seguran?a recomendados para produ??o.",
...
function runCommand(command: string, args: string[], timeoutMs = 180000): Promise<{
    code: number;
    stdout: string;
    stderr: string;
}> {
...
async function scanNpmAudit(): Promise<ScannerFinding[]> {
    ...
    try {
        ...
        return [{
                ...
                recommendation: 'Executar npm audit fix e revisar changelog de dependencias antes de publicar.',
                fixAvailable: true,
                autoFixSupported: true,
            }];
    }
        ...
        for (const signature of signatures) {
            if (signature.test(file.content)) {
                findings.push({
                    ...
                    recommendation: "Revisar manualmente o arquivo e remover codigo suspeito no autorizado.",
                    fixAvailable: false,
                    autoFixSupported: false,
                });
                break;
...
async function applyHeaderFix() {
    ...
    return 'Headers de seguran?a adicionados/normalizados em vercel.json.';
}
...
async function applyFixForFinding(scannerKey: string) {
    ...
    throw new Error("No existe correcao automtica implementada para este tipo de achado.");
}
...
async function executeScan(adminClient: SupabaseClient, userId: string | null, triggerSource: 'manual' | 'scheduled') {
    ...
    if (runResult.error || !runResult.data) {
        throw runResult.error  new Error("No foi possvel iniciar a execucao da varredura.");
    }
...
export async function handleSecurityScans(req: ApiRequest, res: ApiResponse) {
    ...
    try {
        ...
        if (action === 'update_settings') {
            ...
            if (!cronExpression) {
                jsonResponse(res, 400, { error: "Expressao cron obrigatria." });
                return;
            }
            ...
            await context.adminClient.rpc('sync_security_scan_cron');
            const dashboard = await fetchDashboard(context.adminClient);
            jsonResponse(res, 200, dashboard);
            return;
        }
        if (action === 'fix_finding') {
            ...
            if (!findingId) {
                jsonResponse(res, 400, { error: "findingId obrigatrio." });
                return;
            }
            ...
            if (findingResult.error || !findingResult.data) {
                jsonResponse(res, 404, { error: "Achado no encontrado." });
                return;
            }
            ...
            if (!findingResult.data.fix_available) {
                jsonResponse(res, 400, { error: "Este achado no possui correcao automatizada no painel." });
                return;
            }
            try {
        ...
        jsonResponse(res, 400, { error: "Ao invalida." });
    }
    catch (error) {
        jsonResponse(res, 500, {
            error: error instanceof Error  error.message : "Falha ao processar a central de seguran?a.",
        });
    }
}
