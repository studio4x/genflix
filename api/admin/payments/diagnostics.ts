...
import { fetchPaymentGatewayConfiguration, getAsaasAccessToken, getAsaasBaseUrl, getBearerToken, getConfiguredAsaasWebhookSecrets, getHeaderValue, } from '../../_shared/asaas.js';
type ApiRequest = {
    method: string;
...
async function listSupabaseSecretsDetailed(context: {
    accessToken: string;
    projectRef: string;
}) {
...
async function upsertSupabaseSecrets(context: {
    accessToken: string;
    projectRef: string;
}, input: {
    openAiApiKey: string | null;
...
async function listVercelEnv(context: {
    token: string;
    projectId: string;
    teamId: string;
}) {
    const response = await fetch(`https://api.vercel.com/v10/projects/${context.projectId}/envteamId=${context.teamId}`, {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
        const message = await response.text();
        throw new Error(`Falha ao listar vari?veis na Vercel: ${message}`);
    }
...
async function upsertVercelEnv(context: {
    token: string;
    projectId: string;
    teamId: string;
}, input: {
    ...
    const response = await fetch(`https://api.vercel.com/v10/projects/${context.projectId}/envupsert=true&teamId=${context.teamId}`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${context.token}`,
            'Content-Type': 'application/json',
    ...
    if (!response.ok) {
        const message = await response.text();
        throw new Error(`Falha ao salvar vari?veis na Vercel: ${message}`);
    }
}
async function buildNarrationCredentialsDiagnostics(): Promise<CredentialsDiagnosticsResponse> {
    ...
    if (vercelContext) {
        try {
            ...
            checks.push({
                ...
                status: 'ok',
                detail: "Acesso ao projeto Vercel dispon?vel para leitura de vari?veis.",
            });
        }
        catch (error) {
            checks.push({
                ...
                label: 'Gerenciamento Vercel',
                status: 'warning',
                detail: error instanceof Error  error.message : 'Falha ao acessar vari?veis da Vercel.',
            });
        }
    }
    else {
    ...
    if (!hasOpenAiKey && !hasGeminiKey) {
        ...
        checks.push({
            ...
            label: 'Credenciais de narracao',
            status: 'ok',
            detail: 'OpenAI e Gemini configuradas para opera??o primária + fallback.',
        });
    }
    return {
        location,
...
async function handleNarrationCredentialsScope(req: ApiRequest, res: ApiResponse, context: NonNullable<Awaited<ReturnType<typeof assertAdmin>>>) {
    if (req.method === 'GET') {
    ...
    if (targetLocation === 'supabase') {
        const supabaseContext = getSupabaseManagementContext();
        if (!supabaseContext) {
            jsonResponse(res, 500, { error: "No foi possvel salvar no Supabase: credenciais de gerenciamento ausentes." });
            return;
        }
        ...
        const vercelContext = getVercelManagementContext();
        if (!vercelContext) {
            jsonResponse(res, 500, { error: "No foi possvel salvar na Vercel: credenciais de gerenciamento ausentes." });
            return;
        }
...
async function resolveInvoiceUrlFromAsaas(input: {
    ...
    return (readString(payload.invoiceUrl) 
...
async function assertAdmin(req: ApiRequest, res: ApiResponse) {
    ...
    const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!supabaseUrl || !serviceRoleKey) {
        jsonResponse(res, 500, { error: "Configura??o ausente: Supabase URL e service role sao obrigatrios." });
        return null;
    }
    ...
    if (userResult.error || !userResult.data.user) {
        jsonResponse(res, 401, { error: "Token inv?lido ou expirado." });
        return null;
    }
    ...
    if (rolesResult.error) {
        jsonResponse(res, 500, { error: "No foi possvel validar as permiss?es do usu?rio." });
        return null;
    }
...
async function handleResolveInvoiceUrl(req: ApiRequest, res: ApiResponse, context: NonNullable<Awaited<ReturnType<typeof assertAdmin>>>) {
    ...
    if (!sessionId) {
        jsonResponse(res, 400, { error: "sessionId obrigatrio." });
        return;
    }
    ...
    if (sessionResult.error) {
        jsonResponse(res, 500, { error: "No foi possvel carregar o pedido." });
        return;
    }
    const session = (sessionResult.data  null) as SessionRow | null;
    if (!session) {
        jsonResponse(res, 404, { error: "Pedido no encontrado." });
        return;
    }
    ...
    const fallbackUrl = (readNestedString(rawResponse, ['payment', 'invoiceUrl']) 
    ...
    if (!fallbackUrl) {
        jsonResponse(res, 404, { error: "Fatura no dispon?vel para este pedido." });
        return;
    }
    ...
    const checks: DiagnosticCheck[] = [
        {
            ...
            status: 'ok',
            detail: "Variavel dispon?vel no ambiente do deploy.",
        },
    ];
    try {
        ...
        checks.push({
            ...
            detail: hasSandboxToken
                 "Credencial sandbox dispon?vel para testes controlados." : 'Configure ASAAS_ACCESS_TOKEN_SANDBOX antes de validar compras e repasses em sandbox.',
        });
        checks.push({
            key: 'asaas-production-token',
            label: "Token Asaas produ??o",
            status: hasProductionToken  'ok' : activeEnvironment === 'production'  'error' : 'warning',
            detail: hasProductionToken
                 "Credencial de produ??o dispon?vel para opera??o real." : 'Producao ainda pendente. Configure ASAAS_ACCESS_TOKEN_PRODUCTION quando a conta Asaas final estiver aprovada.',
        });
        checks.push({
            key: 'webhook-secret',
        ...
        checks.push({
            ...
            detail: hasSandboxWebhookSecret
                 "Segredo do webhook sandbox dispon?vel." : 'Configure ASAAS_WEBHOOK_SECRET_SANDBOX antes de validar callbacks em sandbox.',
        });
        checks.push({
            key: 'webhook-secret-production',
            label: "Segredo do webhook produ??o",
            status: hasProductionWebhookSecret  'ok' : activeEnvironment === 'production'  'error' : 'warning',
            detail: hasProductionWebhookSecret
                 "Segredo do webhook produ??o dispon?vel." : "Configure ASAAS_WEBHOOK_SECRET_PRODUCTION ou ASAAS_WEBHOOK_SECRET para callbacks de produ??o.",
        });
        checks.push({
            key: 'public-url',
            label: 'URL p?blica do app',
            ...
            detail: process.env.APP_PUBLIC_URL
                ...
                : "APP_PUBLIC_URL no configurada. O sistema usara a origem da requisicao para montar callbacks e webhook.",
        });
        jsonResponse(res, 200, {
        ...
        checks.push({
            ...
            label: 'Registro do gateway',
            status: 'error',
            detail: error instanceof Error  error.message : "No foi possvel carregar a configurao do gateway.",
        });
        jsonResponse(res, 200, {
            environment: null,
            checkedAt: new Date().toISOString(),
