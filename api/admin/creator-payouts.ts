...
async function resolveEligibleSelection(input: {
  ...
  const invalid = commissions.find((commission) => (
    commission.creator_id !== input.creatorId ||
  ...
  if (!creator?.is_payout_enabled || !creator.pix_key || !creator.pix_key_type) {
    throw new Error('O criador precisa estar habilitado para repasse e ter chave PIX cadastrada.')
  }
