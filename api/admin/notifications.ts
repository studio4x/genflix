...
async function assertAdmin(req: ApiRequest, res: ApiResponse) {
    ...
    const relationRows = (rolesResult.data as Array<{
        roles: { code: string } | { code: string }[] | null;
    }> | null) || [];
    const roles = relationRows
        .flatMap((relation) => {
            if (!relation.roles) {
                return [];
            }
    ...
    if (!roles.includes('admin')) {
        jsonResponse(res, 403, { error: 'Apenas administradores podem acessar a central de notificacoes.' });
        return null;
    }
...
async function processEmailQueue(adminClient: SupabaseClient, limit: number) {
    ...
    for (const row of queueRows) {
        ...
        if (row.channel !== 'email') {
            const errorMessage = 'Canal ainda nao possui provedor configurado na GenFlix.';
            await adminClient
                .from('notification_queue')
                .update({
                    status: 'ignored',
        ...
        if (preference && (!preference.email_enabled || preference.email_digest === 'never')) {
            const errorMessage = 'Usuario desativou notificacoes por e-mail.';
            await adminClient
                .from('notification_queue')
                .update({
                    status: 'ignored',
        ...
        if (!profile?.email) {
            const errorMessage = 'Usuario sem e-mail cadastrado.';
            await adminClient
                .from('notification_queue')
                .update({
                    status: 'failed',
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
    ...
    try {
        ...
        jsonResponse(res, 500, {
            error: error instanceof Error ? error.message : 'Nao foi possivel processar a fila de notificacoes.',
        });
    }
}
