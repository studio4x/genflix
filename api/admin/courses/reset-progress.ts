...
const resetCourseProgressSchema = z.object({
    courseId: z.string().uuid("Curso invlido."),
});
...
async function createAdminClient(req: ApiRequest, res: ApiResponse) {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
        res.status(500).json({
            error: "Configurao ausente: SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY sao obrigatrias.",
        });
        return null;
    }
    ...
    if (requesterResult.error || !requesterResult.data.user) {
        res.status(401).json({ error: "Token invlido ou expirado." });
        return null;
    }
    ...
    if (requesterRolesResult.error) {
        res.status(500).json({ error: "N?o foi possvel validar perfil do solicitante." });
        return null;
    }
...
function collectDistinctUserIds(...collections: Array<Array<{
    user_id?: string | null;
}>>) {
    return new Set(collections.flatMap((collection) => collection.map((item) => item.user_id).filter((userId): userId is string => Boolean(userId))));
}
export default async function handler(req: ApiRequest, res: ApiResponse) {
    if (req.method !== 'POST') {
        res.setHeader('Allow', 'POST');
        res.status(405).json({ error: "Mtodo no permitido." });
        return;
    }
    ...
    if (!parsedBody) {
        res.status(400).json({ error: "Body invlido." });
        return;
    }
    ...
    if (!validationResult.success) {
        res.status(400).json({ error: validationResult.error.issues[0]?.message ?? "Dados invlidos." });
        return;
    }
    ...
    if (courseResult.error) {
        res.status(500).json({ error: "N?o foi possvel localizar o curso informado." });
        return;
    }
    ...
    if (modulesResult.error) {
        res.status(500).json({ error: "N?o foi possvel localizar os mdulos do curso." });
        return;
    }
    const moduleIds = ((modulesResult.data as Array<{
        id: string;
    }> | null) ?? []).map((module) => module.id);
    ...
    if (lessonsResult.error) {
        res.status(500).json({ error: "N?o foi possvel localizar as aulas do curso." });
        return;
    }
    const lessonIds = ((lessonsResult.data as Array<{
        id: string;
    }> | null) ?? []).map((lesson) => lesson.id);
    const assessmentsResult = await adminClient
        .from('assessments')
        .select('id')
        .eq('course_id', courseId);
    if (assessmentsResult.error) {
        res.status(500).json({ error: "N?o foi possvel localizar as avaliaes do curso." });
        return;
    }
    const assessmentIds = ((assessmentsResult.data as Array<{
        id: string;
    }> | null) ?? []).map((assessment) => assessment.id);
    ...
    if (existingCourseProgressResult.error || existingLessonProgressResult.error || existingAttemptsResult.error) {
        res.status(500).json({ error: "N?o foi possvel mapear o progresso atual do curso." });
        return;
    }
    const impactedUsers = collectDistinctUserIds((existingCourseProgressResult.data as Array<{
        user_id: string;
    }> | null) ?? [], (existingLessonProgressResult.data as Array<{
        user_id: string;
    }> | null) ?? [], (existingAttemptsResult.data as Array<{
        user_id: string;
    }> | null) ?? []);
    const deletedCounts = {
        course_progress: 0,
    ...
    if (assessmentIds.length > 0) {
        ...
        if (deleteAttemptsResult.error) {
            res.status(500).json({ error: "Falha ao limpar tentativas de avaliao do curso." });
            return;
        }
