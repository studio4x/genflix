...
import { fetchPaymentGatewayConfiguration, getConfiguredAsaasWebhookSecrets, getHeaderValue, } from '../_shared/asaas.js';
...
async function findSessionByKeys(adminClient: SupabaseClient, checkoutId: string | null, externalReference: string | null) {
...
async function activateCourseRelease(adminClient: SupabaseClient, input: {
    ...
    const isLegacyConflictError = message.includes('no unique or exclusion constraint') ||
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
    ...
    const paymentCheckoutSession = typeof payment?.checkoutSession === 'object' && payment.checkoutSession
        ? payment.checkoutSession as Record<string, unknown>
        : null;
    const paymentCheckout = typeof payment?.checkout === 'object' && payment.checkout
        ? payment.checkout as Record<string, unknown>
        : null;
    const checkoutId = readNestedString(checkout, 'id') ??
    ...
    const externalReference = readNestedString(checkout, 'externalReference') ??
    ...
    const isRefundEvent = Boolean(event &&
