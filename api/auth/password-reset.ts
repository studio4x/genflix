...
const RESET_GENERIC_SUCCESS_MESSAGE = "Se o e-mail existir, enviaremos um link de recuperao.";
...
function incrementAndCheckRateLimit(key: string, maxRequests: number) {
    ...
    if (resetRateLimitStore.size > 10000) {
...
function getSupabaseConfig() {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
        throw new Error("Configurao do Supabase ausente.");
    }
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
    ...
    if (!validationResult.success) {
        jsonResponse(res, 400, { error: validationResult.error.issues[0]?.message ?? "Dados invlidos." });
        return;
    }
    ...
    try {
        ...
        jsonResponse(res, 500, {
            error: error instanceof Error ? error.message : "N?o foi possvel enviar o e-mail de recuperao.",
        });
    }
}
