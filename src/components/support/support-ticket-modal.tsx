...
import type { SupportBusinessHoursConfig, SupportCrisisProtocolConfig, SupportModalStep, SupportSlaConfig, SupportTicketCategory, SupportTicketPriority, } from '@/features/support/types';
const emptySettings: {
...
export function SupportTicketModal({ initialStep = 'choice', supportBasePath = '/aluno/suporte', onClose, onCreated, }: {
    ...
    const selectedCategory = useMemo(() => settings.sla.categories.find((item) => item.key === category) ?? settings.sla.categories[0], [category, settings.sla.categories]);
    useEffect(() => {
    ...
    return (<div className="fixed inset-0 z-[150] flex items-center justify-center bg-[#061b21]/58 p-4 backdrop-blur-sm">
      <div className="relative flex max-h-[90vh] w-full max-w-[560px] flex-col overflow-hidden rounded-[30px] border border-[#D8E6EB] bg-white shadow-[0_30px_90px_rgba(6,27,33,0.24)]">
        ...
        {step === 'choice' ? (<div className="overflow-y-auto px-6 py-6">
            ...
            <div className="mt-6">
              <Button type="button" variant="outline" onClick={onClose} className="h-11 w-full rounded-2xl border-[#D8E6EB] bg-white font-black text-[#15323b]">
                Cancelar
              </Button>
            </div>
          </div>) : (<form onSubmit={(event) => void handleSubmit(event)} className="overflow-y-auto px-6 py-6">
            ...
            {errorMessage ? (<div className="mt-4 rounded-[18px] border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">
                {errorMessage}
              </div>) : null}
            ...
            {isLoadingSettings ? (<p className="mt-4 text-sm font-semibold text-[#5F7077]">Carregando formul?rio...</p>) : (<div className="mt-5 space-y-4">
                <label className="grid gap-2">
                  ...
                  <select value={category} onChange={(event) => setCategory(event.target.value as SupportTicketCategory)} className="h-12 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none">
                    {settings.sla.categories.map((item) => (<option key={item.key} value={item.key}>{item.label}</option>))}
                  </select>
                </label>

                <label className="grid gap-2">
                  <span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#5F7077]">Assunto</span>
                  <input value={subject} onChange={(event) => setSubject(event.target.value)} placeholder="Ex: problema com pagamento ou acesso" className="h-12 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none"/>
                </label>

                <label className="grid gap-2">
                  <span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#5F7077]">Prioridade interna</span>
                  <select value={priority} onChange={(event) => setPriority(event.target.value as SupportTicketPriority)} className="h-12 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none">
                    {priorityOptions.map((item) => (<option key={item} value={item}>{getPriorityLabel(item)}</option>))}
                  </select>
                ...
                <div className="grid gap-2">
                  ...
                  {attachment ? (<button type="button" onClick={() => setAttachment(null)} className="justify-self-start text-xs font-black uppercase tracking-[0.14em] text-[#1398B7]">
                      Remover anexo
                    </button>) : null}
                </div>
            ...
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
              <Button type="button" variant="outline" onClick={() => setStep(initialStep === 'form' ? 'form' : 'choice')} className="h-11 rounded-2xl border-[#D8E6EB] bg-white font-black text-[#15323b]">
                {initialStep === 'form' ? 'Fechar' : 'Voltar'}
              </Button>

              <div className="flex flex-col gap-3 sm:flex-row">
                {user?.id ? (<Button type="submit" disabled={isSubmitting || isLoadingSettings} className="h-11 rounded-2xl bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-5 font-black text-white hover:opacity-95">
                    {isSubmitting ? 'Enviando...' : 'Enviar chamado'}
                  </Button>) : (<Button type="button" onClick={() => navigate('/login')} className="h-11 rounded-2xl bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-5 font-black text-white hover:opacity-95">
