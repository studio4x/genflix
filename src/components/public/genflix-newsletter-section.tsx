...
export function GenflixNewsletterSection({ id = 'newsletter', entryPrefix = 'global.newsletter', pageKey = 'global', variant = 'form', ctaHref = '/cursos', }: {
    id?: string;
    entryPrefix?: string;
    pageKey?: SitePageKey;
    ...
    const legacyButtonLabel = useEditableValue(`${entryPrefix}.button.label`, variant === 'cta' ? 'Ver cursos' : 'Quero me inscrever', { pageKey });
    function toggleInterestArea(area: string) {
        setSelectedInterestAreas((currentAreas) => currentAreas.includes(area)
    ...
    const areAllInterestAreasSelected = newsletterSelectableAreas.length > 0 &&
    ...
    async function handleNewsletterSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(payload?.error ?? "N?o foi possvel cadastrar seu e-mail.");
            }
            setNewsletterEmail('');
            ...
            setMessage(error instanceof Error ? error.message : "N?o foi possvel cadastrar seu e-mail.");
        }
        finally {
            setIsSubmitting(false);
        }
    }
    return (<section id={id} className="relative overflow-hidden bg-[#10242B]">
      <EditableImage entryKey={`${entryPrefix}.image`} fallback={image} label="Imagem da newsletter" pageKey={pageKey}>
        {(editableImage) => (<div className="relative" style={{
            ...
            <div className="public-site-container py-20 sm:py-24">
              <div className="mx-auto flex max-w-[760px] flex-col items-center text-center">
                <h2 className="mx-auto w-full max-w-[760px] text-center text-[2.2rem] font-bold leading-[1.02] tracking-[-0.05em] text-white sm:text-[2.8rem]">
                  <EditableText entryKey={`${entryPrefix}.title`} fallback={variant === 'cta' ? 'Veja em detalhes a mais completa plataforma EAD.' : 'Assine nossa newsletter'} label="Título da newsletter" pageKey={pageKey}/>
                </h2>
                <p className="mt-4 max-w-[620px] text-sm leading-7 text-white/74 sm:text-base">
                  <EditableText entryKey={`${entryPrefix}.description`} fallback={variant === 'cta'
                ? 'Conheça cursos, trilhas e recursos para estudar no seu ritmo.'
                : 'Cadastre-se para receber atualizações sobre nossos cursos e conteúdo.'} label="Descrição da newsletter" pageKey={pageKey}/>
                </p>

                {variant === 'cta' ? (<EditableButton entryKey={`${entryPrefix}.button`} fallback={{ label: legacyButtonLabel, href: ctaHref, isInternal: true, tone: 'solid' }} label="Botão da newsletter" pageKey={pageKey}>
                    {(buttonValue) => buttonValue.isHidden === true ? null : (<GenflixCtaButton asChild className="mt-8 min-h-[50px] justify-between px-5 md:min-w-[220px]" tone={normalizeGenflixCtaTone(buttonValue.tone)}>
                        {buttonValue.isInternal === true ? (<Link to={typeof buttonValue.href === 'string' ? buttonValue.href : ctaHref}>
                            {typeof buttonValue.label === 'string' ? buttonValue.label : 'Ver cursos'}
                          </Link>) : (<a href={typeof buttonValue.href === 'string' ? buttonValue.href : ctaHref} target={buttonValue.openInNewTab === true ? '_blank' : undefined} rel={buttonValue.openInNewTab === true ? 'noreferrer' : undefined}>
                            {typeof buttonValue.label === 'string' ? buttonValue.label : 'Ver cursos'}
                          </a>)}
                      </GenflixCtaButton>)}
                  </EditableButton>) : (<form onSubmit={handleNewsletterSubmit} className="relative mt-8 w-full max-w-[640px] overflow-hidden rounded-[36px] border border-white/14 bg-[#08141d]/72 p-4 text-left shadow-[0_32px_90px_rgba(0,0,0,0.38)] backdrop-blur-md sm:p-5">
                    <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(118,223,240,0.16),transparent_42%),linear-gradient(180deg,rgba(8,20,29,0.18)_0%,rgba(8,20,29,0.68)_100%)]"/>
                    ...
                    return (<label key={area} className={[
                      ...
                      <div className="flex flex-col gap-3 rounded-full border border-white/14 bg-white/10 p-1.5 shadow-[0_24px_60px_rgba(0,0,0,0.18)] backdrop-blur-sm md:flex-row md:items-center">
                        <div className="flex min-h-[50px] flex-1 items-center rounded-full bg-white px-5 shadow-[inset_0_1px_0_rgba(16,36,43,0.06)]">
                          <input type="email" value={newsletterEmail} onChange={(event) => setNewsletterEmail(event.target.value)} required placeholder={placeholder} className="w-full border-0 bg-transparent text-sm font-medium text-[#183139] outline-none placeholder:text-[#8BA0A7]"/>
                        </div>

                        <EditableButton entryKey={`${entryPrefix}.button`} fallback={{ label: legacyButtonLabel, tone: 'solid' }} label="Botão da newsletter" pageKey={pageKey}>
                          {(buttonValue) => (<GenflixCtaButton type="submit" disabled={isSubmitting} className="min-h-[50px] justify-between px-5 md:min-w-[220px]" tone={normalizeGenflixCtaTone(buttonValue.tone)}>
