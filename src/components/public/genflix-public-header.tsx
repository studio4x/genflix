...
import { createDefaultSiteAppearance, normalizeSiteAppearance, resolveSiteAppearancePageKey, resolveSiteAppearanceVariant, type SiteAppearanceFields, type SiteAppearanceScopeKey, type SiteAppearanceTheme, type SiteAppearanceViewport, } from '@/features/site-editor/site-appearance';
...
function HeaderNavLink({ item, isActive, className, variant, appearance, }: {
    item: GenflixNavLink;
    isActive: boolean;
    className?: string;
    variant: 'home' | 'light';
    ...
    const classes = cn('relative text-[15px] leading-none tracking-[-0.02em] transition-colors hover:text-[color:var(--genflix-menu-hover-color)]', isActive ? 'font-extrabold' : 'font-semibold', className);
    const style = {
    ...
    if (item.isInternal) {
        return (<Link to={item.href} className={classes} style={style}>
        {item.label}
      </Link>);
    }
    return (<a href={item.href} className={classes} style={style}>
      {item.label}
    </a>);
}
function HeaderAppearanceControl({ label, onClick, }: {
    label: string;
    onClick: () => void;
}) {
    return (<button type="button" onClick={onClick} className="inline-flex items-center gap-2 rounded-full border border-[#D8E6EB] bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-[#163138] shadow-[0_18px_40px_rgba(6,27,33,0.08)] hover:bg-[#F2F7F9]">
...
export function GenflixPublicHeader({ currentPage, navLinks, }: {
    ...
    const pageAppearanceRecord = normalizeSiteAppearance(pageAppearanceEntry ?? globalAppearanceEntry ?? createDefaultSiteAppearance(headerTheme), createDefaultSiteAppearance(headerTheme), pageAppearanceEntry ? 'page' : 'global');
    const globalAppearanceRecord = normalizeSiteAppearance(globalAppearanceEntry ?? createDefaultSiteAppearance('light'), createDefaultSiteAppearance('light'), 'global');
    const currentScopeAppearance = resolveSiteAppearanceVariant(pageAppearanceRecord.scope === 'global' ? globalAppearanceRecord : pageAppearanceRecord, viewport);
    ...
    const openAppearanceEditor = (scopePageKey: SiteAppearanceScopeKey) => {
        ...
        editor.openEditor({
            ...
            entryType: 'json',
            label: scopePageKey === 'global' ? 'Header global' : "Header da pgina",
      ...
      <section className={cn('border-b', useHomeTheme
        ...
        <div className="public-site-container">
          <header className="relative">
            ...
            tone: 'warm',
                  ...
                  {(buttonValue) => buttonValue.isHidden === true ? null : (<GenflixCtaButton asChild tone="warm" className="hidden h-11 px-5 sm:inline-flex">
                      {buttonValue.isInternal === true ? (<Link to={typeof buttonValue.href === 'string' ? buttonValue.href : ctaPath}>
                        ...
                        </Link>) : (<a href={typeof buttonValue.href === 'string' ? buttonValue.href : ctaPath} target={buttonValue.openInNewTab === true ? '_blank' : undefined} rel={buttonValue.openInNewTab === true ? 'noreferrer' : undefined}>
                          {typeof buttonValue.label === 'string' ? buttonValue.label : ctaLabel}
                        </a>)}
                    </GenflixCtaButton>)}
                </EditableButton>

                {editor?.isEditing && scope ? (<div className="hidden items-center gap-2 xl:flex">
                    <HeaderAppearanceControl label={appearanceScopePageKey === 'global' ? 'Header global' : "Header dest pgina"} onClick={() => openAppearanceEditor(appearanceScopePageKey)}/>
                    {appearanceScopePageKey !== 'global' ? (<button type="button" onClick={() => openAppearanceEditor('global')} className="inline-flex items-center gap-2 rounded-full border border-[#D8E6EB] bg-[#F8FCFD] px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-[#163138] hover:bg-white">
                        Global
                      </button>) : null}
                  </div>) : null}
            ...
            {editor?.isEditing && scope ? (<div className="mt-3 flex flex-wrap gap-2 xl:hidden">
                <HeaderAppearanceControl label={appearanceScopePageKey === 'global' ? 'Header global' : "Header dest pgina"} onClick={() => openAppearanceEditor(appearanceScopePageKey)}/>
                {appearanceScopePageKey !== 'global' ? (<button type="button" onClick={() => openAppearanceEditor('global')} className="inline-flex items-center gap-2 rounded-full border border-[#D8E6EB] bg-[#F8FCFD] px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-[#163138] hover:bg-white">
                    Global
                  </button>) : null}
