...
const footerSocialIconMap = new Map(genflixSocialLinks.map((item) => [item.label.trim().toLowerCase(), item.icon] as const));
function getFooterSocialIcon(label: string) {
    return footerSocialIconMap.get(label.trim().toLowerCase()) ?? MessageCircleMore;
}
function FooterNavLink({ href, children, isInternal = false, openInNewTab = false, isButton = false, }: {
    href: string;
    children: string;
    isInternal?: boolean;
    ...
    if (isInternal) {
        if (isButton) {
            return (<GenflixCtaButton asChild tone="surface" className="min-h-[42px] text-[13px] text-[#0A3640]">
    ...
    return (<a href={href} className={className} target={openInNewTab ? '_blank' : undefined} rel={openInNewTab ? 'noreferrer' : undefined}>
      {children}
    </a>);
}
...
export function GenflixPublicFooter({ id = 'footer', }: {
    id?: string;
}) {
    ...
    return (<footer id={id} className="bg-[#0A3640] text-white shadow-none">
        ...
        <div className="space-y-8 px-6 py-8 sm:px-8 lg:px-10">
          <div className="space-y-8 border-b border-white/18 pb-8">
            ...
            return (<div key={column.id}>
                      ...
                      <ul className="mt-4 space-y-2">
                        {items.map((item, index) => (<li key={`${String(item.buttonLabel ?? item.label ?? index)}`}>
                            <FooterNavLink href={item.href} isInternal={item.isInternal} openInNewTab={item.openInNewTab} isButton={Boolean(item.buttonLabel)}>
              ...
              <EditableList entryKey="global.footer.socialLinks" fallback={footerSocialLinks} label="Mídias sociais" pageKey="global">
                {(items) => (<div>
                    <h3 className="text-[11px] font-black uppercase tracking-[0.18em] text-white/92">
                      Mídias sociais
                    </h3>
                ...
                return (<a key={item.id} href={item.href ?? '#'} target="_blank" rel="noreferrer" aria-label={label} className="inline-flex w-fit items-center gap-2 text-white/80 transition-colors hover:text-white">
              ...
              <div className="flex h-full items-center">
                <GenflixCtaButton asChild tone="solid" className="min-h-[42px] w-full text-[13px]">
          ...
          <div className="text-center text-xs text-white/68">
            <p className="mx-auto max-w-[760px]">
              <EditableText entryKey="global.footer.copyright" fallback="Copyright ©️ 2026 e-Clix | Soluções em Conteúdo Educacional | CNPJ 38.112.632/0001-10" label="Copyright" pageKey="global"/>
            </p>
