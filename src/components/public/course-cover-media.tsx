...
export function CourseCoverMedia({
  ...
  return (
    <div className={cn('relative h-full w-full overflow-hidden bg-[#173039]', className)}>
      {!showPlaceholder ? (
        <img
          src={normalizedSrc}
          alt={alt}
      ...
      {showPlaceholder ? (
        <div
          className={cn(
            'absolute inset-0 flex h-full w-full flex-col justify-between bg-[radial-gradient(circle_at_top_left,#1ba8c5_0%,#1398B7_28%,#0A3640_100%)] p-5 text-white',
            placeholderClassName,
          )}
        >
