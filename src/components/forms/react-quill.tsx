...
function updateExistingColumnsLayout(container: HTMLElement, nextCount: number) {
    ...
    const columns = Array.from(container.children).filter((child): child is HTMLElement => (child instanceof HTMLElement && child.classList.contains('genflix-column')));
    if (safeColumns <= 1) {
...
export default function ReactQuill({ value, onChange, onRequestImage, placeholder, className = '', minHeightClassName = 'min-h-[180px]', modules, enableHtmlMode = false, visualTabLabel = 'Visual', htmlTabLabel = 'HTML', }: ReactQuillProps) {
    ...
    const [movableContextMenu, setMovableContextMenu] = useState<{
        x: number;
        y: number;
    } | null>(null);
    ...
    function applyColumnsWidthsFromPrompt() {
        ...
        const columns = Array.from(existingContainer.children).filter((child): child is HTMLElement => (child instanceof HTMLElement && child.classList.contains('genflix-column')));
    ...
    return (<div className={`react-quill-local overflow-hidden rounded-[18px] border border-slate-200 bg-white ${className}`}>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3">
        ...
        {enableHtmlMode ? (<div className="inline-flex overflow-hidden rounded-full border border-slate-200 bg-white">
            <button type="button" onClick={() => setActiveMode('visual')} className={`px-4 py-2 text-xs font-black uppercase tracking-[0.14em] ${activeMode === 'visual' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
              {visualTabLabel}
            </button>
            <button type="button" onClick={() => setActiveMode('html')} className={`px-4 py-2 text-xs font-black uppercase tracking-[0.14em] ${activeMode === 'html' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
              {htmlTabLabel}
            </button>
          </div>) : null}
      </div>
      ...
      {activeMode === 'visual' ? (<>
          <div className="flex flex-wrap gap-2 border-b border-slate-200 bg-white px-4 py-3 text-xs font-semibold text-slate-700">
            {toolbarButtons.undo ? (<button type="button" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('undo')}>
                Desfazer
              </button>) : null}
            {toolbarButtons.redo ? (<button type="button" className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('redo')}>
                Refazer
              </button>) : null}
            {toolbarButtons.header ? (<select className="rounded-lg border border-slate-200 bg-white px-3 py-1.5" onChange={(event) => {
            ...
            {toolbarButtons.bold ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-black hover:bg-slate-100" onClick={() => runEditorCommand('bold')}>
                B
              </button>) : null}
            {toolbarButtons.italic ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 italic hover:bg-slate-100" onClick={() => runEditorCommand('italic')}>
                I
              </button>) : null}
            {toolbarButtons.underline ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 underline hover:bg-slate-100" onClick={() => runEditorCommand('underline')}>
                U
              </button>) : null}
            {toolbarButtons.strike ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 line-through hover:bg-slate-100" onClick={() => runEditorCommand('strikeThrough')}>
                S
              </button>) : null}
            {toolbarButtons.align ? (<select className="rounded-lg border border-slate-200 bg-white px-3 py-1.5" onChange={(event) => {
            ...
            {toolbarButtons.ordered ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('insertOrderedList')}>
                Lista 1.
              </button>) : null}
            {toolbarButtons.bullet ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('insertUnorderedList')}>
                Lista
              </button>) : null}
            {toolbarButtons.columns ? (<select className="rounded-lg border border-slate-200 bg-white px-3 py-1.5" onChange={(event) => {
            ...
            {toolbarButtons.columns ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={applyColumnsWidthsFromPrompt}>
                Larguras
              </button>) : null}
            {toolbarButtons.blockquote ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('formatBlock', 'blockquote')}>
                Citação
              </button>) : null}
            {toolbarButtons.codeBlock ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('formatBlock', 'pre')}>
                Código
              </button>) : null}
            {toolbarButtons.link ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => {
            ...
            {toolbarButtons.link ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('unlink')}>
                Remover link
              </button>) : null}
            {toolbarButtons.image ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => void handleInsertImage()}>
                Imagem
              </button>) : null}
            {toolbarButtons.video ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => {
            ...
            {toolbarButtons.table ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => {
            ...
            {toolbarButtons.horizontalRule ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => runEditorCommand('insertHorizontalRule')}>
                Linha
              </button>) : null}
            {toolbarButtons.textColor ? (<label className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-50">
                <span>Texto</span>
            ...
            {toolbarButtons.backgroundColor ? (<label className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-50">
                <span>Fundo</span>
            ...
            {toolbarButtons.clean ? (<button type="button" className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 hover:bg-slate-100" onClick={() => {
          ...
          <div ref={editorShellRef} className="relative">
            <div ref={editorRef} className={`${minHeightClassName} w-full bg-white px-4 py-4 text-sm leading-7 text-slate-800 outline-none [&_a]:text-sky-700 [&_a]:underline [&_a]:underline-offset-2 [&_blockquote]:border-l-4 [&_blockquote]:border-slate-300 [&_blockquote]:pl-4 [&_blockquote]:italic [&_h1]:my-4 [&_h1]:text-3xl [&_h1]:font-black [&_h1]:leading-tight [&_h2]:my-4 [&_h2]:text-2xl [&_h2]:font-black [&_h2]:leading-tight [&_h3]:my-3 [&_h3]:text-xl [&_h3]:font-bold [&_h3]:leading-tight [&_h4]:my-3 [&_h4]:text-lg [&_h4]:font-bold [&_h4]:leading-tight [&_h5]:my-2 [&_h5]:text-base [&_h5]:font-bold [&_h6]:my-2 [&_h6]:text-sm [&_h6]:font-bold [&_img]:h-auto [&_img]:max-w-full [&_li]:my-1 [&_ol]:my-4 [&_ol]:list-decimal [&_ol]:pl-6 [&_p]:my-3 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-slate-950 [&_pre]:p-4 [&_pre]:font-mono [&_pre]:text-slate-100 [&_strong]:font-bold [&_table]:my-6 [&_table]:w-full [&_table]:border-collapse [&_td]:border [&_td]:border-slate-300 [&_td]:px-3 [&_td]:py-2 [&_th]:border [&_th]:border-slate-300 [&_th]:bg-slate-100 [&_th]:px-3 [&_th]:py-2 [&_ul]:my-4 [&_ul]:list-disc [&_ul]:pl-6 [&_iframe]:min-h-[320px] [&_iframe]:w-full`} contentEditable suppressContentEditableWarning data-placeholder={placeholder} onMouseUp={rememberSelection} onKeyUp={rememberSelection} onMouseDown={(event) => {
                const movableElement = getMovableElement(event.target);
                if (!movableElement) {
                    return;
                }
                ...
                rememberSelection();
            }} onContextMenu={openMovableContextMenu} onKeyDown={(event) => {
            ...
            {selectedImageBounds ? (<div className="pointer-events-none absolute z-20 rounded-xl border-2 border-sky-400/70" style={{
                ...
                <button type="button" aria-label="Redimensionar imagem" className={`pointer-events-auto absolute -bottom-2 -right-2 h-5 w-5 rounded-full border-2 border-white bg-sky-500 shadow-lg ${isResizingImage ? 'cursor-se-resize' : 'cursor-se-resize'}`} onPointerDown={startImageResize}>
                  <span className="sr-only">Redimensionar imagem</span>
                </button>
              </div>) : null}
          </div>
          {movableContextMenu ? (<div className="fixed z-50 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl" style={{ left: movableContextMenu.x, top: movableContextMenu.y }} onPointerDown={(event) => event.stopPropagation()} onMouseLeave={closeMovableContextMenu}>
        ...
        </>) : (<textarea value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} rows={16} className="w-full resize-y bg-[#0B1220] px-4 py-4 font-mono text-sm leading-6 text-slate-100 outline-none"/>)}
