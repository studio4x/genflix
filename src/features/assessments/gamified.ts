...
import type { AssessmentInteractionContent, AssessmentInteractionResponsePayload, AssessmentQuestionAnswerKeyPayload, AssessmentQuestionType, ColoringInteractionContent, ColoringRenderMode, FillInTheBlanksInteractionContent, ImageHotspotAnswerKeyPayload, ImageHotspotInteractionContent, ImageHotspotResponsePayload, TokenMappingAnswerKeyPayload, } from '@/types/content';
...
export function isImageHotspotInteractionContent(content: AssessmentInteractionContent | null | undefined): content is ImageHotspotInteractionContent {
    return content?.kind === 'image_hotspot';
}
export function isImageHotspotAnswerKeyPayload(payload: AssessmentQuestionAnswerKeyPayload | null | undefined): payload is ImageHotspotAnswerKeyPayload {
    return Boolean(payload && 'kind' in payload && payload.kind === 'image_hotspot');
}
export function isImageHotspotResponsePayload(payload: AssessmentInteractionResponsePayload | null | undefined): payload is ImageHotspotResponsePayload {
    return Boolean(payload && 'kind' in payload && payload.kind === 'image_hotspot');
}
export function isTokenMappingAnswerKeyPayload(payload: AssessmentQuestionAnswerKeyPayload | null | undefined): payload is TokenMappingAnswerKeyPayload {
    return Boolean(payload && 'entries' in payload);
}
export function createDefaultInteractionContent(questionType: AssessmentQuestionType, options?: {
    hotspotMode?: ImageHotspotInteractionContent['mode'];
    ...
    if (questionType === 'image_hotspot') {
        return {
            ...
            outside_click_feedback: "Clique em uma das \u00E1reas dest?cadas na imagem.",
...
export function createAnswerKeyFromInteraction(content: AssessmentInteractionContent | null): AssessmentQuestionAnswerKeyPayload | null {
    if (!content) {
        return null;
    }
    ...
    if (content.kind === 'fill_in_the_blanks' && content.editor_groups?.length) {
        return {
            entries: content.editor_groups.flatMap((group) => group.blanks.map((blank) => ({
    ...
    const blanks = content.segments.filter((segment): segment is Extract<FillInTheBlanksInteractionContent['segments'][number], {
        type: 'blank';
    }> => segment.type === 'blank');
    return {
...
export function validateInteractionBundle(questionType: AssessmentQuestionType, content: AssessmentInteractionContent | null | undefined, answerKey: AssessmentQuestionAnswerKeyPayload | null | undefined) {
    ...
    if (parsedContent.data.kind === 'drag_drop_labeling'
        && parsedContent.data.tokens.length !== parsedContent.data.targets.length) {
        throw new Error("N?o arrastar e soltar, o banco de respostas deve ter exatamente um item para cada \u00E1rea.");
    }
    if (parsedContent.data.kind === 'coloring'
...
export function normalizeResponsePayload(payload: AssessmentInteractionResponsePayload | null | undefined) {
    return assessmentInteractionResponsePayloadSchema.parse(payload ?? { entries: [] });
}
export function createDefaultImageHotspotResponsePayload(mode: ImageHotspotInteractionContent['mode']): ImageHotspotResponsePayload {
    return {
        kind: 'image_hotspot',
        mode,
