...
function getCandidateRegionId(element: Element) {
    return (element.getAttribute('data-region-id')
...
function setRuntimePresentationValue(node: Element, propertyName: 'fill' | 'stroke' | 'stroke-width', value: string | null) {
...
function restorePaintNode(node: Element) {
    ...
    setRuntimePresentationValue(node, 'fill', originalFill && originalFill.trim().toLowerCase() !== 'none' ? originalFill : DEFAULT_EMPTY_FILL);
...
export function applyColoringSvgRuntimeState(container: HTMLElement | null, options: ColoringSvgRuntimeOptions = {}) {
    if (!container) {
        return;
    }
