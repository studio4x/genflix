...
async function requestSecurityApi<T>(method: 'GET' | 'POST', body?: Record<string, unknown>) {
  ...
  if (!response.ok) {
    throw new Error(typeof payload?.error === 'string' ? payload.error : 'Falha na central de seguranca.')
  }
...
export async function updateSecuritySettings(input: { cronExpression: string; autoFixEnabled: boolean; enabled: boolean }) {
