...
async function postCreatorPayoutAction<TPayload>(body: Record<string, unknown>, fallbackError: string) {
  ...
  const payload = await response.json().catch(() => null) as (TPayload & { error?: string }) | null
...
export async function registerPaidCreatorPayout(input: {
  ...
  const payload = await postCreatorPayoutAction<{ payoutId?: string }>({
...
export async function updateCreatorPayoutSettings(input: {
  ...
  const payload = await postCreatorPayoutAction<{ payoutSettings?: AdminCreatorPayoutSettings }>({
...
export async function processDueCreatorPayouts(force = true) {
  return postCreatorPayoutAction<{
    processed?: boolean
    payouts?: Array<{ payoutId?: string; status?: string }>
    failures?: Array<{ creatorId?: string; error?: string }>
    synced?: Array<{ payoutId?: string; status?: string }>
    nextRunAt?: string
    reason?: string
  }>({
