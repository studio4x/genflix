...
export function getLessonFooterActionIconName(
  action: Pick<LessonFooterAction, 'action_type' | 'mime_type' | 'file_name' | 'template'>,
) {
  if (action.template?.icon) return action.template.icon
  if (action.action_type === 'url') return 'external-link'

  const mimeType = action.mime_type?.toLowerCase() ?? ''
  const fileName = action.file_name?.toLowerCase() ?? ''

  if (mimeType.includes('pdf') || fileName.endsWith('.pdf')) return 'download'
  if (
