...
export async function createGlobalButton(input: GlobalButtonDefinitionFormInput, userId: string, file?: File | null) {
    ...
    const payload = {
        ...
        modal_subtitle: input.action_type === 'modal' ? (input.modal_subtitle !== undefined ? input.modal_subtitle : null) : null,
...
function resolveFooterActionInsertPayload(context: FooterActionContext, input: LessonFooterActionFormInput, userId: string, storagePath: string | null, file?: File | null) {
    const payload: Record<string, unknown> = {
        ...
        url: input.action_type === 'url' ? input.url?.trim() || null : null,
        storage_path: input.action_type === 'file' ? storagePath ?? input.storage_path ?? null : null,
        file_name: input.action_type === 'file' ? file?.name ?? input.file_name ?? null : null,
        mime_type: input.action_type === 'file' ? file?.type || input.mime_type || null : null,
        file_size_bytes: input.action_type === 'file' ? file?.size ?? input.file_size_bytes ?? 0 : 0,
