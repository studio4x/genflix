    ...
    return (<div className={className}>
      {blocks.map((block, index) => {
            if (block.type === 'columns') {
                ...
                return (<div key={`columns-${index}`} className={`genflix-columns genflix-columns-${columnsCount}`}>
              {block.content.map((columnHtml, columnIndex) => (<div key={`columns-${index}-${columnIndex}`} className="genflix-column">
                  <div className="lesson-rich-text" dangerouslySetInnerHTML={{ __html: sanitizeRichTextHtml(columnHtml) }}/>
                </div>))}
            </div>);
            }
            if (block.type === "table") {
                return (<div key={`table-${index}`} className="my-6 overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
