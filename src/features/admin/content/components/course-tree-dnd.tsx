...
import { type AdminCourseTree, deleteLesson, deleteModule, reorderModules, reorderLessons, toErrorMessage } from '../api';
...
Todas as questões, estudos de caso e tentativas vinculadas serão removidos permanentemente.`;
        ...
        try {
            ...
            if (location.pathname.includes(`/avaliacoes/${assessmentId}`)) {
                navigate(moduleId
                    ? `/admin/cursos/${courseId}/builder/modulos/${moduleId}` : `${coursePath}/assessments`, { replace: true });
            }
        }
        catch (err) {
            alert(toErrorMessage(err));
    ...
    return (<DragDropContext onDragEnd={onDragEnd}>
      <div className="flex-1 overflow-y-auto w-full no-scrollbar p-3 space-y-1">
        ...
        {/* Course Root */}
        <Link to={coursePath} className={`group flex items-center justify-between p-2.5 rounded-lg text-sm transition-colors ${isCourseHome ? 'bg-blue-50 text-blue-700 font-semibold border border-blue-100 shadow-sm' : 'text-slate-700 hover:bg-slate-100'}`}>
        ...
        <Droppable droppableId="modules" type="module">
          {(provided) => (<div {...provided.droppableProps} ref={provided.innerRef} className="space-y-1">
              {tree.modules.map((m, mIdx) => (<Draggable key={m.id} draggableId={m.id} index={mIdx}>
                  {(provided, snapshot) => (<div ref={provided.innerRef} {...provided.draggableProps} className={snapshot.isDragging ? 'z-50' : ''}>
                      <div className="group pt-2">
                        <div className={`flex items-center rounded-lg transition-colors ${location.pathname.includes(`/modulos/${m.id}`) && !location.pathname.includes('/aulas/') ? 'bg-slate-100 border border-slate-200 shadow-sm' : 'hover:bg-slate-100/50'}`}>
                          ...
                          <Link to={`/admin/cursos/${courseId}/builder/modulos/${m.id}`} className="flex-1 p-2 py-2.5 text-sm font-bold text-slate-800 truncate">
                        ...
                        <div className="ml-9 mt-1 space-y-0.5 border-l-2 border-slate-100 pl-2">
                          <Droppable droppableId={`module-lessons-${m.id}`} type={`lesson-${m.id}`}>
                            {(provided) => (<div {...provided.droppableProps} ref={provided.innerRef} className="space-y-0.5">
                                {m.lessons.map((l, lIdx) => {
                            const isActiveLesson = location.pathname.includes(`/aulas/${l.id}`);
                            return (<Draggable key={l.id} draggableId={l.id} index={lIdx}>
                                      {(provided, snapshot) => (<div ref={provided.innerRef} {...provided.draggableProps} className={`flex items-center group/lesson rounded-md transition-colors ${isActiveLesson ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50'} ${snapshot.isDragging ? 'z-50 bg-white shadow-lg ring-1 ring-blue-100' : ''}`}>
                                          {/* Drag Handle */}
                                          ...
                                          <Link to={`/admin/cursos/${courseId}/builder/modulos/${m.id}/aulas/${l.id}`} className="flex-1 py-1.5 px-1 rounded-md text-[13px] font-medium truncate">
                                            <div className="flex items-center gap-2 truncate">
                                               {l.lesson_type === 'video' ? (<svg className={`h-3.5 w-3.5 shrink-0 ${isActiveLesson ? 'text-blue-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>) : (<svg className={`h-3.5 w-3.5 shrink-0 ${isActiveLesson ? 'text-blue-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>)}
                                    ...
                                    }} disabled={deletingKey === `lesson:${l.id}`} className="mr-1 rounded-md p-1.5 text-slate-300 opacity-0 transition-all hover:bg-rose-50 hover:text-rose-500 disabled:cursor-not-allowed disabled:opacity-100 group-hover/lesson:opacity-100" title="Excluir aula">
                        ...
                        return (<div key={a.id} className={`flex items-center group/quiz rounded-md transition-colors ${isActiveAssessment ? 'bg-amber-50 text-amber-700 border border-amber-100 shadow-sm' : 'hover:bg-amber-50/30'}`}>
                                <div className="p-1 px-1.5 text-transparent">
                                   <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8h16M4 16h16"/></svg>
                                </div>
                                <Link to={`/admin/cursos/${courseId}/builder/modulos/${m.id}/avaliacoes/${a.id}`} className="flex-1 py-1.5 px-1 text-[13px] font-medium truncate">
                                   <div className="flex items-center gap-2 truncate">
                            ...
                            }} disabled={deletingKey === `assessment:${a.id}`} className="mr-1 rounded-md p-1.5 text-slate-300 opacity-0 transition-all hover:bg-rose-50 hover:text-rose-500 disabled:cursor-not-allowed disabled:opacity-100 group-hover/quiz:opacity-100" title="Excluir quiz">
                                  <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                  </svg>
                                </button>
        ...
        {tree.courseAssessments.length > 0 ? (<div className="space-y-1 border-t border-slate-100 pt-3">
            ...
            {tree.courseAssessments.map((assessment) => {
                ...
                return (<div key={assessment.id} className={`group/final flex items-center rounded-lg transition-colors ${isActiveFinalAssessment
                        ? 'bg-emerald-50 text-emerald-700 border border-emerald-100 shadow-sm'
                        : 'hover:bg-emerald-50/40'}`}>
                  <Link to={finalAssessmentPath} className="flex-1 px-3 py-2 text-sm font-semibold">
                    <div className="flex items-center gap-2 truncate">
