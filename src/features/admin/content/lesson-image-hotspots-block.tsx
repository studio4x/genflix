...
import {
  ...
  getSignedLessonContentAssetUrl,
...
export function LessonImageHotspotsBlockEditor({
  content,
  onChange,
  onError,
}: LessonImageHotspotsBlockEditorProps) {
    ...
    function handlePointerMove(event: PointerEvent) {
      ...
      emit({
        ...
        hotspots: content.hotspots.map((hotspot) => (
          hotspot.id !== dragState.hotspotId
            ? hotspot
            : {
              ...hotspot,
  ...
  function handleHotspotPointerDown(
    hotspot: LessonImageHotspotItem,
                ...
                {content.hotspots.map((hotspot, index) => {
                  ...
                  return (
                    <button
                      ...
                      onPointerDown={(event) => handleHotspotPointerDown(hotspot, event)}
                      className={cn(
                        'absolute inline-flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-4 border-white bg-slate-900/88 text-white shadow-xl transition-all',
                        isSelected ? 'scale-105 ring-4 ring-sky-300/70' : 'hover:scale-[1.03]',
                      )}
                      style={{
                        left: `${hotspot.x}%`,
                        top: `${hotspot.y}%`,
        ...
        <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 grid max-h-[320px] gap-3 overflow-auto pr-1">
            {content.hotspots.length === 0 ? (
              <div className="rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm font-medium text-slate-500">
          ...
          <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                type="button"
                onClick={removeSelectedHotspot}
                className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-400 transition hover:text-rose-600"
                aria-label="Remover hotspot"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="mt-5 space-y-4">
              <label className="block">
                ...
                <input
                  ...
                  onChange={(event) => updateHotspot(selectedHotspot.id, { title: event.target.value })}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) => updateHotspot(selectedHotspot.id, {
...
export function LessonImageHotspotsBlockRenderer({
  content,
}: LessonImageHotspotsBlockRendererProps) {
  const [resolvedAssetUrl, setResolvedAssetUrl] = useState<string | null>(content.asset.signed_url ?? null)
  const [activeHotspotId, setActiveHotspotId] = useState<string | null>(null)

  const activeHotspot = useMemo(
    () => content.hotspots.find((hotspot) => hotspot.id === activeHotspotId) ?? null,
  ...
  return (
    <div className="my-8 overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-100 px-6 py-4">
        <p className="text-[10px] font-black uppercase tracking-[0.28em] text-blue-700">Imagem Interativa</p>
        <p className="mt-2 text-sm font-medium text-slate-500">
          Clique nos hotspots para explorar os destaques desta imagem.
        </p>
      </div>
      ...
      <div className="p-4 sm:p-6">
        ...
        >
          {resolvedAssetUrl ? (
            ...
            <div className="flex h-full w-full items-center justify-center px-6 text-center text-sm font-medium text-slate-500">
              A imagem interativa desta aula ainda não foi carregada.
            </div>
          )}
          ...
          {activeHotspot ? (
            <button
              type="button"
              className="absolute inset-0 z-10 bg-slate-950/32 transition-opacity"
              onClick={() => setActiveHotspotId(null)}
              aria-label="Fechar hotspot"
            />
          ) : null}
          ...
          {content.hotspots.map((hotspot) => {
            ...
            return (
              <button
                ...
                type="button"
                className={cn(
                  'absolute z-20 inline-flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-4 border-white text-white shadow-xl transition-all',
                  isActive
                    ? 'bg-blue-600 ring-4 ring-blue-200 scale-105'
                    : 'bg-slate-900/88 hover:scale-[1.04] hover:bg-slate-900',
                )}
                style={{
                  left: `${hotspot.x}%`,
                  top: `${hotspot.y}%`,
          ...
          {activeHotspot ? (
            <>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => setActiveHotspotId(null)}
                    className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-400 transition hover:bg-slate-50 hover:text-slate-700"
                    aria-label="Fechar conteúdo do hotspot"
                  >
                    <X className="h-5 w-5" />
                  </button>
                </div>
                <div
                  className="lesson-hotspots-body mt-4 text-left"
                  dangerouslySetInnerHTML={{ __html: activeHotspot.body_html }}
                />
              </div>
              ...
              >
                ...
                <div
                  className="lesson-hotspots-body mt-5 text-left"
                  dangerouslySetInnerHTML={{ __html: activeHotspot.body_html }}
                />
              </div>
            </>
