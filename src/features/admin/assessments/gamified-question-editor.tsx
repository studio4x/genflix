...
import { deleteAssessmentAsset, uploadAssessmentAsset, type AssessmentQuestionWithOptions, } from './api';
...
function createInteractionRecord(question: AssessmentQuestionWithOptions, content: AssessmentInteractionContent): AssessmentQuestionInteraction {
...
function createAnswerKeyRecord(question: AssessmentQuestionWithOptions, answerKey: AssessmentQuestionAnswerKeyPayload, gradingMode: AssessmentQuestionAnswerKey['grading_mode']): AssessmentQuestionAnswerKey {
    const timestamp = new Date().toISOString();
    return {
        question_id: question.id,
...
function syncAnswerKeyWithContent(content: AssessmentInteractionContent, current: AssessmentQuestionAnswerKeyPayload | null | undefined) {
    ...
    if (normalizedContent.kind === 'fill_in_the_blanks' && normalizedContent.editor_groups?.length) {
        ...
        return {
            entries: normalizedContent.editor_groups.flatMap((group) => group.blanks
...
function normalizeInteractionContent(content: AssessmentInteractionContent, current: AssessmentQuestionAnswerKeyPayload | null | undefined): AssessmentInteractionContent {
...
function getSuggestedColorLabel(hex: string) {
    ...
    for (const suggestion of COLOR_NAME_SUGGESTIONS) {
        ...
        const distance = Math.sqrt(((rgb.r - suggestionRgb.r) ** 2)
...
function buildColoringPointTargetsFromRegions(content: Extract<ColoringInteractionContent, {
...
function createSvgColoringPlaceholderContent(source: ColoringInteractionContent) {
    ...
    } satisfies Extract<ColoringInteractionContent, {
        render_mode: 'svg_regions';
    }>;
}
...
function buildFillBlankEditorGroups(content: FillInTheBlanksInteractionContent, answerKey: AssessmentQuestionAnswerKeyPayload | null) {
    const tokenById = new Map(content.tokens.map((token) => [token.id, token]));
    const answerKeyEntries: Array<{
        slot_id: string;
        token_id: string;
    }> = answerKey && 'entries' in answerKey
        ? (answerKey.entries ?? [])
        : [];
    ...
    if (fallbackGroups.length === 0) {
        fallbackGroups.push({
            ...
            leading_text: content.segments
                .filter((segment): segment is Extract<FillInTheBlanksInteractionContent['segments'][number], {
                type: 'text';
            }> => segment.type === 'text')
...
function buildFillBlankBundle(instruction: string, groups: FillBlankEditorGroup[]) {
...
export function GamifiedQuestionEditor({ question, onDraftChange, onPersist, onError, }: GamifiedQuestionEditorProps) {
    ...
    const interactionContent = useMemo(() => {
        if (question.interaction?.content) {
            return normalizeInteractionContent(question.interaction.content, question.answer_key?.answer_key);
        }
    ...
    const answerKeyPayload = useMemo(() => {
        ...
        return syncAnswerKeyWithContent(interactionContent, question.answer_key?.answer_key ?? createAnswerKeyFromInteraction(interactionContent));
    ...
    const answerKeyEntries: Array<{
        slot_id: string;
        token_id: string;
    }> = useMemo(() => answerKeyPayload && 'entries' in answerKeyPayload
        ? (answerKeyPayload.entries ?? [])
        : [], [answerKeyPayload]);
    const assignedTokenBySlot = useMemo(() => new Map(answerKeyEntries.map((entry) => [entry.slot_id, entry.token_id])), [answerKeyEntries]);
    const assignedTokenBySlotObject = useMemo(() => Object.fromEntries(assignedTokenBySlot), [assignedTokenBySlot]);
    ...
    async function commit(content: AssessmentInteractionContent, answerKey = syncAnswerKeyWithContent(content, answerKeyPayload), nextGradingMode = gradingMode) {
    ...
    function updateDraft(content: AssessmentInteractionContent, answerKey = syncAnswerKeyWithContent(content, answerKeyPayload), nextGradingMode = gradingMode) {
        const normalizedContent = normalizeInteractionContent(content, answerKey);
        const normalizedAnswerKey = syncAnswerKeyWithContent(normalizedContent, answerKey);
        latestInteractionRef.current = normalizedContent;
        latestAnswerKeyRef.current = normalizedAnswerKey;
    ...
    function commitLatestDraft(nextGradingMode = gradingMode) {
        ...
        return commit(latestInteraction, latestAnswerKeyRef.current ?? syncAnswerKeyWithContent(latestInteraction, answerKeyPayload), nextGradingMode);
    }
    function updateTokenLabel(tokenId: string, label: string) {
        if (activeInteraction.kind === 'coloring') {
            const nextContent: ColoringInteractionContent = {
                ...activeInteraction,
                tokens: activeInteraction.tokens.map((token) => (token.id === tokenId
                    ? { ...token, label }
                    : token)),
            };
    ...
    function applyColorHexToContent(content: ColoringInteractionContent, tokenId: string, hex: string) {
        return {
            ...
            tokens: content.tokens.map((token, index) => {
                ...
                const nextLabel = (isGenericColorLabel(token.label, index)
    ...
    function renderAssetError() {
        ...
        return (<div className="mt-4 rounded-[24px] border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold leading-relaxed text-rose-700">
        {assetError}
      </div>);
    }
    function renderColoringModeSelector() {
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="grid w-full gap-3 lg:max-w-2xl lg:grid-cols-2">
            ...
            ].map((mode) => (<button key={mode.value} type="button" className={cn('rounded-[24px] border px-4 py-4 text-left transition-all', currentMode === mode.value
                    ? 'border-cyan-300 bg-cyan-50 text-cyan-900 shadow-sm'
                    : 'border-slate-200 bg-slate-50 text-slate-600 hover:border-cyan-200 hover:bg-cyan-50/60')} onClick={() => void switchColoringMode(mode.value)}>
    ...
    function renderSvgInstructionsModal() {
        ...
        return (<div className="fixed inset-0 z-[135] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setIsSvgInstructionsModalOpen(false)}>
        <div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[40px] border border-white/20 bg-white shadow-2xl animate-in zoom-in-95 duration-300" onClick={(event) => event.stopPropagation()}>
          <div className="flex items-center justify-between border-b border-slate-100 p-8">
            ...
            <button type="button" onClick={() => setIsSvgInstructionsModalOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-50 text-slate-400 transition-colors hover:text-slate-900">
          ...
          <div className="border-t border-slate-100 bg-slate-50/60 p-8">
            <Button type="button" className="h-12 rounded-2xl bg-slate-900 px-6 font-black text-white hover:bg-slate-800" onClick={() => setIsSvgInstructionsModalOpen(false)}>
              Fechar guia
            </Button>
          </div>
        </div>
    ...
    function renderSvgMarkupModal() {
        ...
        return (<div className="fixed inset-0 z-[136] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setIsSvgMarkupModalOpen(false)}>
        <div className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[40px] border border-white/20 bg-white shadow-2xl animate-in zoom-in-95 duration-300" onClick={(event) => event.stopPropagation()}>
          <div className="flex items-center justify-between border-b border-slate-100 p-8">
            <div>
            ...
            <button type="button" onClick={() => setIsSvgMarkupModalOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-50 text-slate-400 transition-colors hover:text-slate-900">
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
              </svg>
            </button>
          ...
          <div className="space-y-5 p-8">
            ...
            <label className="block space-y-2">
              ...
              <textarea value={svgMarkupDraft} onChange={(event) => {
                setSvgMarkupDraft(event.target.value);
                if (svgMarkupError) {
                    setSvgMarkupError(null);
                }
            }} placeholder={`<xml version="1.0" encoding="UTF-8">
${SVG_COLORING_EXAMPLE}`} className="min-h-[360px] w-full rounded-[28px] border border-slate-200 bg-slate-50 px-5 py-4 font-mono text-[13px] leading-6 text-slate-800 shadow-inner focus:border-cyan-300 focus:bg-white focus:ring-4 focus:ring-cyan-100" spellCheck={false}/>
            </label>

            {svgMarkupError ? (<div className="rounded-[24px] border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold leading-relaxed text-rose-700">
                {svgMarkupError}
              </div>) : null}

            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
              <button type="button" className="text-sm font-black text-cyan-700 transition-colors hover:text-cyan-900" onClick={() => {
                setIsSvgMarkupModalOpen(false);
                setIsSvgInstructionsModalOpen(true);
            }}>
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row">
                <Button type="button" variant="ghost" className="rounded-2xl text-slate-500" onClick={() => setIsSvgMarkupModalOpen(false)} disabled={isImportingSvgMarkup}>
                  Cancelar
                </Button>
                <Button type="button" className="rounded-2xl bg-cyan-600 text-white hover:bg-cyan-700" onClick={() => void handleSvgMarkupImport()} disabled={isImportingSvgMarkup}>
    ...
    function renderTokenBank() {
        ...
        const coloringSlotIds = activeInteraction.kind === 'coloring'
            ...
            : [];
        return (<section className="rounded-[28px] border border-cyan-100 bg-cyan-50/70 p-5">
        <div className="flex items-center justify-between gap-4">
          ...
          {activeInteraction.kind !== 'drag_drop_labeling' && activeInteraction.kind !== 'coloring' ? (<Button type="button" variant="outline" className="rounded-2xl border-cyan-200 bg-white text-cyan-700 hover:bg-cyan-100" onClick={addToken}>
              Adicionar item
            </Button>) : null}
        </div>

        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {tokenInteraction.tokens.map((token, index) => (<div key={token.id} className="rounded-2xl border border-white/80 bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                {activeInteraction.kind !== 'drag_drop_labeling' && activeInteraction.kind !== 'coloring' ? (<button type="button" className="rounded-xl p-2 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500" onClick={() => deleteToken(token.id)}>
              ...
              {activeInteraction.kind === 'coloring' ? (<label className="mt-3 flex items-center gap-3 rounded-2xl border border-cyan-100 bg-cyan-50/40 px-4 py-3">
                  <input type="color" className="h-10 w-12 cursor-pointer rounded-xl border border-slate-200 bg-white" value={'hex' in token ? token.hex : '#2563eb'} onChange={(event) => updateColorHex(token.id, event.target.value)} onBlur={(event) => {
              ...
              <input type="text" className="mt-3 w-full rounded-2xl border border-cyan-100 bg-cyan-50/40 px-4 py-3 text-sm font-semibold text-slate-800 focus:border-cyan-300 focus:ring-4 focus:ring-cyan-100" value={token.label} onChange={(event) => updateTokenLabel(token.id, event.target.value)} onBlur={(event) => {
                    const nextLabel = event.currentTarget.value;
                    if (activeInteraction.kind === 'coloring') {
                        void commit({
                            ...activeInteraction,
                            tokens: activeInteraction.tokens.map((item) => (item.id === token.id
                    ...
                    void commit({
                        ...activeInteraction,
                        tokens: tokenInteraction.tokens.map((item) => (item.id === token.id
                            ? { ...item, label: nextLabel }
                            : item)),
                    });
        ...
        {activeInteraction.kind === 'drag_drop_labeling' || (activeInteraction.kind === 'coloring' && !isColoringSvgMode) ? (<div className="mt-5 rounded-2xl border border-cyan-100/80 bg-white/70 p-4">
            <button type="button" className="flex w-full items-center justify-between gap-4 text-left" onClick={() => setIsAdvancedAnswerToolsOpen((current) => !current)}>
                <div>
              ...
              <span className="inline-flex items-center gap-2 rounded-full border border-cyan-100 bg-cyan-50 px-3 py-2 text-[11px] font-black uppercase tracking-widest text-cyan-700">
                ...
                <svg className={cn('h-4 w-4 transition-transform', isAdvancedAnswerToolsOpen ? 'rotate-180' : '')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/>
                </svg>
              </span>
            </button>

            {isAdvancedAnswerToolsOpen ? (<div className="mt-4 flex flex-col gap-3 rounded-2xl border border-slate-100 bg-slate-50/70 p-4 md:flex-row md:items-center md:justify-between">
                <div>
                ...
                <Button type="button" variant="outline" className="rounded-2xl border-cyan-200 bg-white text-cyan-700 hover:bg-cyan-50" onClick={() => setIsTargetsModalOpen(true)}>
    ...
    function renderColoringSvgEditor(content: Extract<ColoringInteractionContent, {
        render_mode: 'svg_regions';
    }>) {
        function updateRegion(regionId: string, updates: Partial<(typeof content.regions)[number]>) {
            updateDraft({
                ...content,
                regions: content.regions.map((region) => (region.region_id === regionId
                    ? { ...region, ...updates }
                    : region)),
            });
        }
        ...
        <section className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-3 text-xs font-semibold text-slate-500">Precisa de ajuda para montar o arquivo Abra o guia em modal e veja um exemplo pronto.
              </p>
            </div>

            <div className="flex flex-wrap items-center justify-end gap-3">
              <input ref={fileInputRef} type="file" accept=".svg,image/svg+xml" className="hidden" onChange={(event) => void handleAssetSelected(event)}/>
              ...
              <Button type="button" variant="outline" className="rounded-2xl border-slate-200 bg-white" onClick={openSvgMarkupModal} disabled={isUploadingAsset}>
                Colar SVG/XML
              </Button>
              <Button type="button" variant="outline" className="rounded-2xl border-cyan-200 bg-cyan-50 text-cyan-800 hover:bg-cyan-100" onClick={() => setIsSvgInstructionsModalOpen(true)}>
                Como preparar SVG
              </Button>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 rounded-[24px] border border-dashed border-cyan-200 bg-cyan-50/70 px-4 py-3">
            <p className="text-sm font-semibold text-cyan-950">Tem o código pronto Cole o SVG/XML direto no editor.
            </p>
            <Button type="button" variant="outline" className="rounded-2xl border-cyan-200 bg-white text-cyan-900 hover:bg-cyan-100" onClick={openSvgMarkupModal} disabled={isUploadingAsset}>
              Colar SVG/XML
            </Button>
          </div>
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-4 self-start xl:sticky xl:top-[92px]">
              <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-[radial-gradient(circle_at_top_left,_rgba(6,182,212,0.18),_transparent_38%),linear-gradient(135deg,_#f8fafc,_#eef2ff)] p-3">
                <div ref={svgStageRef} className="relative overflow-hidden rounded-[24px] border border-slate-200 bg-white [&>svg]:h-full [&>svg]:w-full" style={{ aspectRatio: `${content.asset.width || 1200} / ${content.asset.height || 800}` }} dangerouslySetInnerHTML={{ __html: content.svg_markup }} onClick={(event) => {
              ...
              <div className="rounded-[28px] border border-slate-200 bg-slate-50/70 p-4">
                <button type="button" className="flex w-full items-center justify-between gap-4 rounded-2xl text-left" onClick={() => setIsAssetMetadataOpen((current) => !current)}>
                  <div>
                ...
                : 'Clique para abrir texto alternativo, largura e altura.'}
                  ...
                  <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-[11px] font-black uppercase tracking-widest text-slate-600">
                    ...
                    <svg className={cn('h-4 w-4 transition-transform', isAssetMetadataOpen ? 'rotate-180' : '')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/>
                    </svg>
                  </span>
                </button>

                {isAssetMetadataOpen ? (<div className="mt-4 space-y-4">
                    <label className="block space-y-2">
                      <span className="text-xs font-black uppercase tracking-widest text-slate-500">Texto alternativo</span>
                      <input className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 focus:border-cyan-300 focus:ring-4 focus:ring-cyan-100" value={content.asset.alt} onChange={(event) => updateDraft({
                    ...content,
                    asset: {
                        ...content.asset,
                        alt: event.target.value,
                    },
                })} onBlur={() => void commitLatestDraft()} placeholder="Descreva a ilustração para acessibilidade"/>
                    </label>
            ...
            <aside className="self-start rounded-[28px] border border-slate-200 bg-slate-50/70 p-4 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                ...
                return (<div key={region.region_id} className={cn('rounded-[24px] border bg-white p-4 text-left shadow-sm transition-all', isSelected
                        ? 'border-cyan-300 shadow-cyan-100/80 ring-4 ring-cyan-100'
                        : 'border-slate-200')}>
                      <button type="button" className="flex w-full items-start justify-between gap-3 text-left" onClick={() => setSelectedTargetId(region.region_id)}>
                        <div>
                      ...
                      <div className="mt-4 space-y-3">
                        ...
                        <label className="block space-y-2">
                          ...
                          <select className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 focus:border-cyan-300 focus:ring-4 focus:ring-cyan-100" value={assignedTokenId} onChange={(event) => updateSlotAnswer(region.region_id, event.target.value)}>
                        ...
                        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                          <span className="h-9 w-9 rounded-full border border-white shadow-sm" style={{ backgroundColor: assignedToken?.hex ?? '#E2E8F0' }}/>
                          <div>
    ...
    function renderDragDropEditor(content: CanvasInteractionContent) {
        ...
        function updateTarget(targetId: string, field: keyof CanvasInteractionContent['targets'][number], value: string | number) {
        ...
        function handleStageClick(event: MouseEvent<HTMLDivElement>) {
            ...
            const targetTemplate = getCanvasTargetTemplate(content, content.targets.length);
            const x = Math.max(0, Math.min(100 - targetTemplate.w, ((event.clientX - rect.left) / rect.width) * 100 - (targetTemplate.w / 2)));
            const y = Math.max(0, Math.min(100 - targetTemplate.h, ((event.clientY - rect.top) / rect.height) * 100 - (targetTemplate.h / 2)));
        ...
        function startTargetDrag(event: ReactPointerEvent<HTMLButtonElement>, target: CanvasInteractionContent['targets'][number]) {
            ...
            const handlePointerMove = (moveEvent: PointerEvent) => {
                ...
                const x = Math.max(0, Math.min(100 - target.w, ((moveEvent.clientX - currentRect.left) / currentRect.width) * 100 - offsetXPercent));
                const y = Math.max(0, Math.min(100 - target.h, ((moveEvent.clientY - currentRect.top) / currentRect.height) * 100 - offsetYPercent));
                currentContent = {
                    ...currentContent,
                    targets: currentContent.targets.map((item) => (item.id === target.id
                        ? {
                            ...item,
        ...
        function renderTargetsPanel() {
          ...
          <div className="space-y-4">
            {content.targets.map((target, index) => (<div key={target.id} className={cn('rounded-[28px] border bg-white p-4 shadow-sm transition-all', selectedTargetId === target.id ? 'border-cyan-300 shadow-cyan-100/80' : 'border-slate-200')}>
                <div className="flex items-center justify-between gap-3">
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-slate-500">
                    {targetLabelPrefix} {index + 1}
                  </span>
                  <button type="button" className="rounded-xl p-2 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500" onClick={() => removeTarget(target.id)}>
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                  </button>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {editableTargetFields.map((field) => (<label key={field} className="block space-y-2">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{field}%</span>
                        <input type="number" min={0} max={100} step={0.1} className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 focus:border-cyan-300 focus:ring-4 focus:ring-cyan-100" value={target[field]} onChange={(event) => updateTarget(target.id, field, Number(event.target.value || 0))} onBlur={() => void commitLatestDraft()}/>
                      </label>))}
                  </div>

                  {isColoringPointMode ? (<div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs font-semibold text-slate-500">
        ...
        function renderTargetsSidebar() {
            return (<aside className="self-start rounded-[28px] border border-slate-200 bg-slate-50/70 p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button type="button" variant="outline" className="rounded-2xl border-slate-200 bg-white" onClick={() => setSelectedTargetId(null)} disabled={!selectedTargetId}>
              Limpar seleção
            </Button>
            <Button type="button" variant="outline" className="rounded-2xl border-cyan-200 bg-white text-cyan-700 hover:bg-cyan-50" onClick={() => setIsTargetsModalOpen(true)}>
              Ajuste detalhado
          ...
          <div className="mt-4 space-y-3">
            {content.targets.map((target, index) => {
                    ...
                    return (<button key={target.id} type="button" onClick={() => setSelectedTargetId(target.id)} className={cn('block w-full rounded-[24px] border bg-white p-4 text-left shadow-sm transition-all', isSelected
                            ? 'border-cyan-300 shadow-cyan-100/80 ring-4 ring-cyan-100'
            ...
            <div className="rounded-[28px] border border-slate-200 bg-slate-50/60 p-4 shadow-sm">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="hidden items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 md:flex">
                  ...
                  {[0.75, 1, 1.5].map((preset) => (<button key={preset} type="button" className={cn('rounded-xl px-2 py-1 text-xs font-black transition-colors', Math.abs(canvasScale - preset) < 0.01
                    ? 'bg-cyan-600 text-white'
                    : 'text-slate-500 hover:bg-slate-50 hover:text-cyan-700')} onClick={() => setCanvasScale(preset)}>
              ...
              <div className="mt-4 flex items-center gap-3">
                <button type="button" className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-slate-600 transition-colors hover:border-cyan-200 hover:text-cyan-700" onClick={() => setCanvasScale((value) => Math.max(0.6, Math.round((value - 0.1) * 100) / 100))}>
                  -
                </button>
                <input type="range" min={0.6} max={2.4} step={0.05} value={canvasScale} onChange={(event) => setCanvasScale(Number(event.target.value))} className="h-2 w-full cursor-pointer appearance-none rounded-full bg-slate-200 accent-cyan-600"/>
                <button type="button" className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-slate-600 transition-colors hover:border-cyan-200 hover:text-cyan-700" onClick={() => setCanvasScale((value) => Math.min(2.4, Math.round((value + 0.1) * 100) / 100))}>
                  +
                </button>
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
              <div className="space-y-4 self-start xl:sticky xl:top-[92px]">
                <div className="overflow-auto rounded-[28px] border border-slate-200 bg-slate-50/30 p-3">
                  <div className="mx-auto transition-[width] duration-200" style={{
                width: `${canvasScalePercent}%`,
                minWidth: canvasScale > 1 ? `${canvasScalePercent}%` : '320px',
            }}>
                    <div ref={stageRef} role="button" tabIndex={0} onClick={handleStageClick} onKeyDown={(event) => {
                ...
                return (<button key={target.id} type="button" className={cn('absolute bg-transparent transition-all', draggingTargetId === target.id ? 'cursor-grabbing' : 'cursor-grab', isColoringPointMode
                        ? selectedTargetId === target.id
                            ? 'rounded-full'
                            : ''
                        : selectedTargetId === target.id
                            ? 'rounded-2xl border-2 border-dashed border-cyan-500'
                            : 'border border-transparent')} style={{
                        left: `${target.x}%`,
                        top: `${target.y}%`,
                        width: `${target.w}%`,
                        height: `${target.h}%`,
                        ...
                        setSelectedTargetId(target.id);
                            ...
                            <span className={cn('absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border font-black shadow-lg transition-all', isColoringPointMode ? 'h-3.5 w-3.5 text-[7px] border-slate-700/70' : 'h-7 w-7 text-[11px]', selectedTargetId === target.id
                        ? isColoringPointMode
                        ...
                        : isColoringPointMode
                            ...
                            : 'border-white bg-white/95 hover:border-cyan-300 hover:text-cyan-700')} style={isColoringPointMode ? { backgroundColor: pointColor, color: getPointBadgeTextColor(pointColor) } : undefined}>
                              {index + 1}
                            </span>
                          </button>);
            })}
        ...
        {isTargetsModalOpen ? (<div className="fixed inset-0 z-[130] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[40px] border border-white/20 bg-white shadow-2xl animate-in zoom-in-95 duration-300">
              <div className="flex items-center justify-between border-b border-slate-100 p-8">
                ...
                <button type="button" onClick={() => setIsTargetsModalOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-50 text-slate-400 transition-colors hover:text-slate-900">
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                  </svg>
                </button>
              ...
              <div className="border-t border-slate-100 bg-slate-50/60 p-8">
                <Button type="button" className="h-12 rounded-2xl bg-slate-900 px-6 font-black text-white hover:bg-slate-800" onClick={() => setIsTargetsModalOpen(false)}>
                  Fechar
                </Button>
              </div>
            </div>
          </div>) : null}
        {isSvgInstructionsModalOpen ? (<div className="fixed inset-0 z-[135] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setIsSvgInstructionsModalOpen(false)}>
            <div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-[40px] border border-white/20 bg-white shadow-2xl animate-in zoom-in-95 duration-300" onClick={(event) => event.stopPropagation()}>
              <div className="flex items-center justify-between border-b border-slate-100 p-8">
                <div>
                  <h3 className="text-left text-xl font-black tracking-tight text-slate-900">Como preparar o SVG para colorir</h3>
    ...
    function renderFillInTheBlanksEditor(content: FillInTheBlanksInteractionContent) {
        ...
        function patchBlank(groupId: string, blankId: string, updates: Partial<FillBlankEditorBlank>) {
            return patchGroups(groupId, (group) => ({
                ...group,
                blanks: group.blanks.map((blank) => (blank.blank_id === blankId
                    ? { ...blank, ...updates }
                    : blank)),
            }));
        }
        ...
        function updateExtraToken(groupId: string, tokenId: string, label: string) {
            updateGroupsDraft(patchGroups(groupId, (currentGroup) => ({
                ...currentGroup,
                extra_tokens: (currentGroup.extra_tokens ?? []).map((token) => (token.id === tokenId
                    ? { ...token, label }
                    : token)),
            })));
        }
        ...
        return (<section className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 md:flex-row md:items-center md:justify-between">
          ...
          <Button type="button" className="rounded-2xl bg-teal-600 px-5 hover:bg-teal-700" onClick={addPromptItem}>
        ...
        <div className="mt-5 space-y-4">
          {groups.map((group, index) => {
                ...
                const previewLabel = group.blanks
                    ...
                    .join(' • ');
                return (<div key={group.id} className="overflow-hidden rounded-[28px] border border-slate-200 bg-slate-50/70">
                <div className="flex items-center gap-2 px-5 py-4">
                  <button type="button" className="flex min-w-0 flex-1 items-center justify-between gap-4 text-left transition-colors hover:text-teal-700" onClick={() => setOpenFillBlankId((current) => current === group.id ? null : group.id)}>
                  ...
                  <button type="button" className="rounded-xl p-2 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500" onClick={() => removePromptItem(group.id)}>
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                  </button>
                ...
                {isOpen ? (<div className="space-y-5 border-t border-slate-200 bg-white px-5 py-5">
                    <label className="block space-y-2">
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Texto inicial da pergunta</span>
                      <textarea className="min-h-[120px] w-full rounded-2xl border border-slate-200 bg-slate-50/60 px-4 py-3 text-sm text-slate-700 focus:border-teal-300 focus:ring-4 focus:ring-teal-100" value={group.leading_text} onChange={(event) => updateGroup(group.id, { leading_text: event.target.value })} onBlur={(event) => void commitGroups(patchGroups(group.id, (currentGroup) => ({
                            ...currentGroup,
                    ...
                    <div className="space-y-4">
                      {group.blanks.map((blank, blankIndex) => (<div key={blank.blank_id} className="rounded-[24px] border border-teal-100 bg-teal-50/40 p-4">
                          <div className="flex items-center justify-between gap-3">
                            <span className="rounded-full bg-white px-3 py-1 text-[10px] font-black uppercase tracking-widest text-teal-700 shadow-sm">
                              Lacuna {blankIndex + 1}
                            </span>
                            <button type="button" className="rounded-xl p-2 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500" onClick={() => removeBlank(group.id, blank.blank_id)}>
                              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                              </svg>
                            </button>
                          ...
                          <div className="mt-4 grid gap-4 md:grid-cols-2">
                            <label className="block space-y-2">
                              ...
                              <input className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 focus:border-teal-300 focus:ring-4 focus:ring-teal-100" value={blank.placeholder ?? ''} onChange={(event) => updateBlank(group.id, blank.blank_id, { placeholder: event.target.value })} onBlur={(event) => void commitGroups(patchBlank(group.id, blank.blank_id, { placeholder: event.currentTarget.value }))} placeholder="Ex.: termo principal"/>
                            </label>

                            <label className="block space-y-2">
                              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Resposta correta</span>
                              <input className="w-full rounded-2xl border border-teal-200 bg-white px-4 py-3 text-sm font-semibold text-slate-800 focus:border-teal-300 focus:ring-4 focus:ring-teal-100" value={blank.answer_text} onChange={(event) => updateBlank(group.id, blank.blank_id, { answer_text: event.target.value })} onBlur={(event) => void commitGroups(patchBlank(group.id, blank.blank_id, { answer_text: event.currentTarget.value }))} placeholder="Digite a resposta correta completa"/>
                            </label>
                          </div>
                    ...
                    <div className="rounded-[24px] border border-slate-200 bg-slate-50/70 p-5">
                      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <Button type="button" variant="outline" className="rounded-2xl border-slate-200 bg-white text-slate-700 hover:bg-slate-100" onClick={() => addExtraToken(group.id)}>
                          Adicionar alternativa
                        </Button>
                      </div>

                      {(group.extra_tokens ?? []).length > 0 ? (<div className="mt-4 space-y-3">
                          {(group.extra_tokens ?? []).map((token, tokenIndex) => (<div key={token.id} className="flex items-center gap-3 rounded-2xl border border-white bg-white p-3 shadow-sm">
                              <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-slate-500">
                                Alternativa {tokenIndex + 1}
                              </span>
                              <input className="flex-1 rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-3 text-sm font-semibold text-slate-700 focus:border-teal-300 focus:ring-4 focus:ring-teal-100" value={token.label} onChange={(event) => updateExtraToken(group.id, token.id, event.target.value)} onBlur={(event) => commitExtraToken(group.id, token.id, event.currentTarget.value)} placeholder="Ex.: azul, verde, anatomia..."/>
                              <button type="button" className="rounded-xl p-2 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500" onClick={() => removeExtraToken(group.id, token.id)}>
                                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                              </button>
                    ...
                    <div className="rounded-[24px] border border-teal-100 bg-teal-50/60 p-5">
                      ...
                      <p className="mt-4 text-base leading-8 text-slate-700">
                        ...
                        {group.blanks.map((blank) => (<span key={blank.blank_id}>
    ...
    return (<div className="space-y-6 rounded-[32px] border border-cyan-200 bg-[linear-gradient(180deg,_rgba(236,254,255,0.55),_rgba(255,255,255,1))] p-6">
      <div className="grid gap-6 xl:grid-cols-[1fr_260px]">
        ...
        })} onBlur={() => void commitLatestDraft()} placeholder="Explique como o aluno deve interagir com o exercício."/>
        </div>
        ...
        <div className="rounded-[28px] border border-white bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3">
            ...
            },
            {
                ...
                ? 'border-cyan-300 bg-cyan-50 text-cyan-900 shadow-sm'
                : 'border-slate-200 bg-slate-50 text-slate-600 hover:border-cyan-200 hover:bg-cyan-50/60')} onClick={() => void commit(activeInteraction, answerKeyPayload
      ...
      {activeInteraction.kind === 'drag_drop_labeling'
            ...
            : activeInteraction.kind === 'image_hotspot'
                ? (<ImageHotspotQuestionEditor content={activeInteraction} onDraftChange={updateDraft} onPersist={(nextContent) => commit(nextContent)} onError={onError}/>)
                : activeInteraction.kind === 'coloring'
