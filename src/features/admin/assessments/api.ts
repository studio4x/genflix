...
async function hydrateInteractionAsset(interaction: AssessmentQuestionInteraction) {
    if (interaction.content.kind !== 'drag_drop_labeling'
        && interaction.content.kind !== 'image_hotspot'
        && interaction.content.kind !== 'coloring') {
        return interaction;
    }
...
async function getNextAssessmentItemPosition(assessmentId: string) {
    ...
    return Math.max(questionPositionResult.data?.[0]?.position ?? 0, caseStudyPositionResult.data?.[0]?.position ?? 0) + 1;
}
async function syncQuestionInteractionData(input: {
    ...
    validateInteractionBundle(input.questionType, input.interactionContent ?? null, input.answerKey ?? null);
...
function mapQuestionsWithRelations(questions: AssessmentQuestion[], options: AssessmentOption[], interactions: AssessmentQuestionInteraction[], answerKeys: AssessmentQuestionAnswerKey[]) {
...
function mapCaseStudiesWithQuestions(caseStudies: AssessmentCaseStudy[], questions: AssessmentQuestionWithOptions[]) {
...
export async function createModuleAssessment(courseId: string, moduleId: string, input: AssessmentFormInput, userId: string) {
    const result = await supabase
        .from('assessments')
        .insert({
        course_id: courseId,
...
export async function createFinalAssessment(courseId: string, input: AssessmentFormInput, userId: string) {
    const result = await supabase
        .from('assessments')
        .insert({
        course_id: courseId,
...
export async function fetchAssessmentQuestions(assessmentId: string) {
    ...
    return mapQuestionsWithRelations(questions, (optionsResult.data as AssessmentOption[]) ?? [], await Promise.all((((interactionsResult.data as RawInteractionRow[]) ?? []).map(mapInteractionRow)).map(hydrateInteractionAsset)), ((answerKeysResult.data as RawAnswerKeyRow[]) ?? []).map(mapAnswerKeyRow));
}
export async function fetchAssessmentCaseStudies(assessmentId: string, questionsInput?: AssessmentQuestionWithOptions[]) {
...
export async function updateAssessmentQuestion(questionId: string, input: AssessmentQuestionFormInput) {
    const result = await supabase
        .from('assessment_questions')
...
export async function createAssessmentCaseStudy(assessmentId: string, input: AssessmentCaseStudyFormInput) {
...
export async function updateAssessmentCaseStudy(caseStudyId: string, input: AssessmentCaseStudyFormInput) {
    const result = await supabase
        .from('assessment_case_studies')
        .update({
        title: input.title?.trim() || null,
...
export async function updateAssessmentOption(optionId: string, input: Omit<AssessmentOptionFormInput, 'question_id'>) {
    const result = await supabase
        .from('assessment_options')
        .update({
        option_text: input.option_text,
...
export async function exportFinalAssessmentContent(courseId: string): Promise<ImportAssessmentData> {
    ...
    if (!finalAssessment) {
        throw new Error("Nenhuma avaliação final encontrada para este curso.");
    }
...
function normalizeImportQuestionType(questionType: ImportAssessmentQuestionData['question_type'], insideCaseStudy: boolean): AssessmentQuestion['question_type'] {
    if (insideCaseStudy) {
...
async function insertImportedOptions(questionId: string, questionType: AssessmentQuestion['question_type'], options: ImportAssessmentQuestionData['options']) {
