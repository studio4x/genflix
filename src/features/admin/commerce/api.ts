...
export async function updatePaymentGatewaySettings(environment: PaymentGatewayEnvironment, updatedBy?: string | null) {
    const result = await supabase
        .from('payment_gateway_settings')
        .upsert({
        id: 1,
...
export async function fetchPaymentGatewayDiagnostics(): Promise<PaymentGatewayDiagnostics> {
    ...
    const payload = await response.json().catch(() => null) as PaymentGatewayDiagnostics & {
        error?: string;
    } | null;
    if (!response.ok) {
...
export async function fetchAdminPaymentInvoiceUrl(sessionId: string) {
    ...
    const accessToken = sessionResult.data.session?.access_token;
    if (!accessToken) {
        throw new Error("Sesso expirada. Entre novamente para abrir a fatura.");
    }
    const response = await fetch('/api/admin/payments/diagnostics', {
        method: 'POST',
        headers: {
    ...
    if (!response.ok || !payload?.url) {
        throw new Error(payload?.error ?? "N?o foi possvel abrir a fatura do pedido.");
    }
    return payload.url;
}
