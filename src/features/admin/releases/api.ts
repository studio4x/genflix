...
import type { AccessGroup, AccessGroupMember, CourseRelease, } from '@/types/content';
import type { AccessGroupFormInput, GroupReleaseInput, UserReleaseInput, } from './schemas';
...
export async function createAccessGroup(payload: AccessGroupFormInput, createdBy: string) {
    const result = await supabase
        .from('access_groups')
        .insert({
        name: payload.name,
...
export async function updateAccessGroup(groupId: string, payload: AccessGroupFormInput) {
    const result = await supabase
        .from('access_groups')
        .update({
        name: payload.name,
...
export async function fetchCourseReleases(courseId: string): Promise<CourseReleaseView[]> {
    ...
    const profileMap = new Map(profilesResult.map((profile) => [profile.id, profile]));
...
export async function createUserRelease(courseId: string, payload: UserReleaseInput, createdBy: string) {
    const result = await supabase
        .from('course_releases')
        .insert({
        course_id: courseId,
...
export async function createGroupRelease(courseId: string, payload: GroupReleaseInput, createdBy: string) {
    const result = await supabase
        .from('course_releases')
        .insert({
        course_id: courseId,
