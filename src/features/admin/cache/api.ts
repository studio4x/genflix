...
async function getAuthenticatedEdgeContext() {
    ...
    if (!accessToken) {
        throw new Error("Sesso expirada. Entre novamente para limpar o cache.");
    }
...
export async function clearServerCache() {
    ...
    if (!response.ok || !payload?.ok) {
        throw new Error(payload?.error ?? "N?o foi possvel limpar o cache do servidor.");
    }
}
...
export async function clearBrowserCache({ preserveLogin }: {
