...
async function getAuthenticatedEdgeContext() {
    ...
    if (!accessToken) {
        throw new Error("Sesso expirada. Entre novamente para visualizar o uso do storage.");
    }
    const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
    const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY;
    if (!supabaseUrl || !supabaseAnonKey) {
        throw new Error("Configurao do Supabase ausente no frontend.");
    }
...
export async function fetchR2UsageOverview() {
    ...
    const payload = await response.json().catch(() => null) as (R2UsageOverview & {
        error?: string;
    }) | null;
    if (!response.ok || !payload) {
        throw new Error(payload?.error ?? "N?o foi possvel carregar o uso do storage R2.");
    }
    return payload;
}
export async function fetchR2Objects(input: {
    ...
    const payload = await response.json().catch(() => null) as (R2ObjectsListResponse & {
        error?: string;
    }) | null;
    if (!response.ok || !payload) {
        throw new Error(payload?.error ?? "N?o foi possvel listar arquivos do R2.");
    }
    return payload;
}
export async function deleteR2Object(input: {
    ...
    if (!response.ok || !payload?.ok) {
        throw new Error(payload?.error ?? "N?o foi possvel excluir arquivo do R2.");
    }
}
