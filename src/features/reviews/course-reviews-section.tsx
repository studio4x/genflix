...
import { fetchCourseReviews, fetchCourseReviewStats, submitCourseReview, voteReviewHelpful, type CourseReview, type ReviewStats, } from './api';
function StarRating({ value, onChange, readonly = false, size = 'md', }: {
    ...
    return (<div className="flex items-center gap-1" aria-label={`${value} de 5 estrelas`}>
      {[1, 2, 3, 4, 5].map((star) => {
            const isFilled = star <= Math.round(value);
            return (<button key={star} type="button" disabled={readonly} onClick={() => onChange?.(star)} className={cn('text-[#D8E6EB] transition-colors', isFilled ? 'text-[#1398B7]' : '', readonly ? 'cursor-default' : 'hover:text-[#0A3640]')} aria-label={`${star} estrelas`}>
...
function ReviewCard({ review, onVote, isVoting, }: {
    review: CourseReview;
    onVote: (reviewId: string, isHelpful: boolean) => void;
    isVoting: boolean;
}) {
    return (<article className="border border-[#D8E6EB] bg-white p-5 shadow-[0_14px_32px_rgba(21,50,59,0.04)]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          {review.is_verified_purchase ? (<span className="mt-3 inline-flex items-center gap-1 bg-[#E8F6FA] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-[#0A3640]">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        <button type="button" disabled={isVoting} onClick={() => onVote(review.id, true)} className="inline-flex items-center gap-2 border border-[#D8E6EB] bg-[#F2F7F9] px-3 py-2 text-xs font-black text-[#5F7077] transition-colors hover:border-[#1398B7] hover:text-[#0A3640] disabled:opacity-60">
          <ThumbsUp className="h-3.5 w-3.5"/>
          Útil {review.helpful_count}
        </button>
        <button type="button" disabled={isVoting} onClick={() => onVote(review.id, false)} className="inline-flex items-center gap-2 border border-[#D8E6EB] bg-[#F2F7F9] px-3 py-2 text-xs font-black text-[#5F7077] transition-colors hover:border-[#1398B7] hover:text-[#0A3640] disabled:opacity-60">
...
export function CourseReviewsSection({ courseId, courseTitle, }: {
    courseId?: string;
    courseTitle: string;
}) {
    const { user } = useAuth();
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            setContent('');
            setRating(5);
            setMessage(savedReview.moderation_status === 'pending'
      ...
      <div className="mx-auto max-w-[1320px] px-6 lg:px-10">
        <div className="grid gap-8 lg:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="border border-[#D8E6EB] bg-[#F2F7F9] p-6">
            ...
            return (<div key={star} className="grid grid-cols-[34px_1fr_42px] items-center gap-3 text-xs font-bold text-[#5F7077]">
                    <span>{star}★</span>
                    <div className="h-2 bg-[#D8E6EB]">
                      <div className="h-full bg-[linear-gradient(180deg,#1398B7_0%,#0A3640_100%)]" style={{ width: `${distribution.percent}%` }}/>
                    </div>
          ...
          <div className="space-y-6">
            <form onSubmit={(event) => void handleSubmit(event)} className="border border-[#D8E6EB] bg-[#F2F7F9] p-6">
              ...
              {!user ? (<div className="mt-5 border border-[#D8E6EB] bg-white p-4 text-sm font-semibold text-[#5f7077]">
                  ...
                  <input value={title} onChange={(event) => setTitle(event.target.value)} minLength={3} maxLength={100} required placeholder="Título da avaliação" className="h-12 border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#183139] outline-none transition-colors focus:border-[#1398B7]"/>
                  <textarea value={content} onChange={(event) => setContent(event.target.value)} minLength={3} maxLength={3000} required rows={4} placeholder="Conte como foi sua experiência com este curso..." className="resize-none border border-[#D8E6EB] bg-white px-4 py-3 text-sm font-semibold leading-7 text-[#183139] outline-none transition-colors focus:border-[#1398B7]"/>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    {message ? <p className="text-sm font-semibold text-[#5f7077]">{message}</p> : <span />}
                    <Button type="submit" disabled={isSubmitting} className="rounded-none bg-[#1398B7] px-5 font-readex text-sm font-medium text-white hover:bg-[#0A3640] disabled:opacity-70">
            ...
            {isLoading ? (<div className="border border-[#D8E6EB] bg-white p-6 text-sm font-semibold text-[#6d7f85]">
                Carregando avaliações...
              </div>) : reviews.length === 0 ? (<div className="border border-dashed border-[#BEE3EA] bg-white p-8 text-center">
                ...
                {reviews.map((review) => (<ReviewCard key={review.id} review={review} onVote={handleVote} isVoting={votingReviewId === review.id}/>))}
              </div>)}
          </div>
        </div>
      </div>
