...
export async function upsertLessonNote(input: LessonNoteInput): Promise<LessonNote> {
  ...
  if (!parsed.success) {
    throw new Error(parsed.error.issues[0]?.message ?? 'Dados invalidos para salvar a anotacao.')
  }
...
export async function deleteLessonNote(input: LessonNoteDeleteInput): Promise<void> {
  ...
  if (!parsed.success) {
    throw new Error(parsed.error.issues[0]?.message ?? 'Dados invalidos para excluir a anotacao.')
  }
