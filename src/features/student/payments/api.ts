...
export async function fetchStudentPaymentHistory() {
    ...
    if (!accessToken) {
        throw new Error("Sesso expirada. Faca login novamente.");
    }
    ...
    if (!response.ok) {
        throw new Error(payload?.error ?? "N?o foi possvel consultar o hist?rico de pagamentos.");
    }
    return payload?.payments ?? [];
}
export async function requestStudentRefund(input: {
    ...
    if (!response.ok) {
        throw new Error(payload?.error ?? "N?o foi possvel solicitar o reembolso.");
    }
    return {
