...
type JsPdfDocument = {
    ...
    addImage: (dataUrl: string, format: string, x: number, y: number, width: number, height: number, alias?: string, compression?: string) => void;
...
function buildMaterialsHtml(materials: LessonMaterialLink[]) {
    ...
    const items = materials
        .map((material) => `<li><a href="${escapeHtml(material.signedUrl)}" target="_blank" rel="noreferrer">${escapeHtml(material.fileName)}</a></li>`)
        .join('');
    return `
...
function buildLessonSupportInfo(lesson: LessonForPdf) {
    ...
    if (lesson.youtube_url) {
        ...
        items.push(`<span><strong>V\u00eddeo complementar:</strong> <a href="${safeUrl}" target="_blank" rel="noreferrer">${safeUrl}</a></span>`);
    }
...
function buildPdfImageHotspotsFallbackHtml(imageUrl: string | null, imageAlt: string, hotspots: Array<{
    title: string;
    bodyHtml: string;
}>) {
...
async function hydrateInteractiveLessonContent(textContent: string | null) {
    ...
    await Promise.all(hotspotBlocks.map(async (block) => {
        ...
        wrapper.innerHTML = buildPdfImageHotspotsFallbackHtml(signedUrl, parsed.asset.alt, parsed.hotspots.map((hotspot) => ({
            title: hotspot.title,
...
function loadImage(url: string) {
    return new Promise<HTMLImageElement>((resolve, reject) => {
        ...
        image.onerror = () => reject(new Error("N?o foi possvel carregar o logotipo da marca dagua."));
        image.src = url;
    });
}
async function createTransparentImage(url: string, opacity: number): Promise<PreparedImage> {
    ...
    if (!context) {
        throw new Error("N?o foi possvel preparar a marca dagua do PDF.");
    }
...
function fitImageWithin(imageWidth: number, imageHeight: number, maxWidth: number, maxHeight: number) {
...
async function fetchLicenseContext(): Promise<LicenseContext> {
    ...
    if (!user) {
        return {
            ...
            studentEmail: "email-n?o-informado",
            userCode: 'USUARIO',
    ...
    return {
        ...
        studentEmail: profile?.email ?? user.email ?? "email-n?o-informado",
        userCode,
...
export async function exportLicensedModulePdf(moduleTitle: string, storagePath: string) {
    ...
    if (!response.ok) {
        throw new Error("N?o foi possvel baixar o PDF base do m?dulo.");
    }
...
function applyPdfFinishing(pdf: JsPdfDocument, watermark: PreparedImage, watermarkSizePercent: number, courseTitle: string, licenseContext: LicenseContext) {
    ...
    for (let page = 1; page <= totalPages; page += 1) {
        pdf.setPage(page);
        pdf.addImage(watermark.dataUrl, 'PNG', watermarkX, watermarkY, watermarkBounds.width, watermarkBounds.height, undefined, 'FAST');
...
export async function exportModuleToPdf(courseTitle: string, moduleTitle: string, moduleId: string) {
    const [lessonsResult, moduleResult, licenseContext, materialsMap, watermarkSettings] = await Promise.all([
        ...
        (async () => {
            ...
            const lessonIds = ((lessonIdsResult.data as {
                id: string;
    ...
    const lessons = await Promise.all((((lessonsResult.data as LessonRow[]) ?? []).map(async (lesson) => ({
        ...lesson,
