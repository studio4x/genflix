...
export function GamifiedInteraction({ question, value, onChange, readOnly = false, }: GamifiedInteractionProps) {
    const parsed = assessmentInteractionContentSchema.safeParse(question.interaction?.content ?? null);
    if (!parsed.success) {
        return (<div className="rounded-[32px] border border-rose-200 bg-rose-50 px-6 py-5 text-sm font-semibold text-rose-700">A interação dest? questão está inválida ou incompleta.
      </div>);
    }
    if (parsed.data.kind === 'drag_drop_labeling') {
        return (<DragDropLabelingView content={parsed.data} value={value} onChange={onChange} readOnly={readOnly}/>);
    }
    if (parsed.data.kind === 'coloring') {
        return (<ColoringView content={parsed.data} value={value} onChange={onChange} readOnly={readOnly}/>);
    }
    if (parsed.data.kind === 'image_hotspot') {
        return (<div className="rounded-[32px] border border-amber-200 bg-amber-50 px-6 py-5 text-sm font-semibold text-amber-700">
        Este tipo de questão usa um player específico de hotspot.
      </div>);
    }
    return (<FillInTheBlanksView content={parsed.data} value={value} onChange={onChange} readOnly={readOnly}/>);
}
function ColoringView({ content, value, onChange, readOnly, }: {
    ...
    const [selectedRegionId, setSelectedRegionId] = useState<string | null>('regions' in content
    ...
    return (<div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
      <div className="self-start rounded-[32px] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/30 xl:sticky xl:top-6">
        <div className="rounded-[28px] border border-slate-200 bg-slate-50/70 p-4">
          ...
          <div className="mt-4 grid gap-3">
            <button type="button" disabled={readOnly} onClick={() => !readOnly && setArmedColorId(selectedColor?.id ?? content.tokens[0]?.id ?? null)} className={`flex items-center gap-3 rounded-2xl border px-4 py-3 text-left transition-all ${selectedColor
            ? 'border-cyan-400 bg-cyan-50 shadow-lg shadow-cyan-100'
            : 'border-slate-200 bg-white'} ${readOnly ? 'cursor-default' : 'hover:border-cyan-300'}`}>
              <span className="h-10 w-10 rounded-full border border-slate-300 shadow-sm" style={{ backgroundColor: selectedColor?.hex ?? '#e2e8f0' }}/>
              <span>
            ...
            <button type="button" disabled={readOnly} onClick={() => !readOnly && setArmedColorId(ERASER_TOOL_ID)} className={`flex items-center gap-3 rounded-2xl border px-4 py-3 text-left transition-all ${armedColorId === ERASER_TOOL_ID
            ? 'border-cyan-400 bg-cyan-50 text-cyan-800 shadow-lg shadow-cyan-100'
            : 'border-slate-200 bg-white text-slate-700 hover:border-cyan-300'} ${readOnly ? 'cursor-default' : ''}`}>
        ...
        <div className="mt-4 rounded-[28px] border border-slate-200 bg-white p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            {!readOnly ? (<button type="button" onClick={clearAllAssignments} className="rounded-xl border border-slate-200 px-3 py-2 text-[11px] font-black uppercase tracking-wider text-slate-500 hover:bg-slate-50">
                Limpar
              </button>) : null}
          </div>
          ...
          <div className="mt-5 grid grid-cols-5 gap-3">
            ...
            return (<button key={token.id} type="button" disabled={readOnly} title={token.label} onClick={() => {
                    if (readOnly)
                        return;
                    setArmedColorId(token.id);
                }} className={`group flex h-11 w-11 items-center justify-center rounded-full border-2 transition-all ${isArmed
                    ? 'scale-110 border-cyan-500 shadow-lg shadow-cyan-100'
                    : 'border-white shadow-sm hover:scale-105 hover:border-cyan-200'}`} style={{ backgroundColor: token.hex }}>
                  <span className="sr-only">{token.label}</span>
                  {isArmed ? (<span className="h-3 w-3 rounded-full border border-white/70 bg-white/80"/>) : null}
                </button>);
        })}
          </div>
          ...
          <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
            ...
            <div className="mt-3 h-2 rounded-full bg-slate-200">
              <div className="h-2 rounded-full bg-cyan-500 transition-all" style={{ width: `${slotIds.length ? (coloredTargetsCount / slotIds.length) * 100 : 0}%` }}/>
            </div>
          </div>

          {isSvgMode ? (<div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
              <div className="flex items-center justify-between gap-3">
              ...
              <div className="mt-4 flex max-h-[240px] flex-wrap gap-2 overflow-auto">
                ...
                return (<button key={region.region_id} type="button" onClick={() => setSelectedRegionId(region.region_id)} className={`rounded-full border px-3 py-1.5 text-xs font-black transition-all ${isSelected
      ...
      <div className="min-w-0 overflow-hidden rounded-[36px] border border-slate-200 bg-[#fcfdff] shadow-inner">
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          <div className="relative mx-auto overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-lg" style={{ aspectRatio: `${content.asset.width} / ${content.asset.height}` }}>
            {isSvgMode ? (svgContent?.svg_markup ? (<div ref={svgPreviewRef} className="h-full w-full [&>svg]:h-full [&>svg]:w-full" aria-label={content.asset.alt} role="img" dangerouslySetInnerHTML={{ __html: svgContent.svg_markup }} onClick={(event) => {
            ...
            return (<button key={target.id} type="button" disabled={readOnly} onClick={() => {
                    if (readOnly || !armedColorId)
                        return;
                    if (armedColorId === ERASER_TOOL_ID) {
                ...
                }} className="absolute bg-transparent transition-transform hover:scale-[1.02]" style={{
                    left: `${target.x}%`,
                    top: `${target.y}%`,
                    width: `${target.w}%`,
                    height: `${target.h}%`,
                }} title={target.label ?? 'Ponto'}>
                  <span className="absolute left-1/2 top-1/2 flex h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-slate-700/70 text-[7px] font-black shadow-[0_3px_10px_rgba(15,23,42,0.22)]" style={{
...
function DragDropLabelingView({ content, value, onChange, readOnly, }: {
    ...
    return (<div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px] xl:grid-cols-[minmax(0,1.55fr)_320px] 2xl:grid-cols-[minmax(0,2.1fr)_340px]">
      <div className="min-w-0 overflow-hidden rounded-[36px] border border-slate-200 bg-[#fcfdff] shadow-inner">
        <div className="border-b border-slate-200 bg-white/80 px-6 py-4">
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          <div className="relative mx-auto overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-lg 2xl:mx-0" style={{ aspectRatio: `${content.asset.width} / ${content.asset.height}` }}>
            ...
            return (<div key={target.id} className="absolute" style={{
                    left: `${target.x}%`,
                    top: `${target.y}%`,
                    width: `${target.w}%`,
                    height: `${target.h}%`,
                }}>
                  <button type="button" disabled={readOnly} onClick={() => handleSlotClick(target.id)} onDragOver={(event) => {
                    if (!readOnly)
                ...
                }} className={`flex h-full w-full items-center justify-center rounded-2xl border-2 px-3 py-2 pr-8 text-center text-xs font-black shadow-lg backdrop-blur transition-all ${assignedToken
                    ? 'border-cyan-500 bg-cyan-500/90 text-white'
                    : armedTokenId
                        ? 'border-cyan-400 bg-white/90 text-cyan-700'
                        : 'border-rose-300 bg-white/85 text-rose-500'} ${readOnly ? 'cursor-default' : 'hover:scale-[1.02]'}`}>
                    <span className="line-clamp-2">
                      {assignedToken?.label ?? target.label ?? 'Solte aqui'}
                    </span>
                  </button>

                  {assignedToken && !readOnly ? (<button type="button" onClick={(event) => {
                        event.stopPropagation();
                        clearSlot(target.id);
                    }} className="absolute right-1.5 top-1.5 flex h-6 w-6 items-center justify-center rounded-full border border-white/70 bg-slate-950/20 text-white transition-colors hover:bg-slate-950/35" aria-label={`Remover ${assignedToken.label}`}>
      ...
      <div className="self-start rounded-[32px] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/30 lg:sticky lg:top-6">
        ...
        <div className="mt-5 grid gap-3">
          {content.tokens.map((token) => {
            ...
            return (<button key={token.id} type="button" disabled={readOnly || isAssigned} draggable={!readOnly && !isAssigned} onDragStart={(event) => event.dataTransfer.setData('text/plain', token.id)} onClick={() => {
                    if (readOnly || isAssigned)
                        return;
                    setArmedTokenId((current) => current === token.id ? null : token.id);
                }} className={`flex min-h-[60px] items-center justify-center rounded-2xl border px-4 py-3 text-center text-sm font-black transition-all ${isAssigned
        ...
        {availableTokens.length < content.tokens.length ? (<div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {content.targets
                ...
                .map((target) => (<button key={target.id} type="button" disabled={readOnly} onClick={() => !readOnly && onChange(target.id, null)} className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-xs font-black text-cyan-700 hover:bg-cyan-100">
...
function FillInTheBlanksView({ content, value, onChange, readOnly, }: {
    content: FillInTheBlanksInteractionContent;
    value: Record<string, string | null>;
    onChange: (slotId: string, tokenId: string | null) => void;
    readOnly: boolean;
}) {
    const [armedSelection, setArmedSelection] = useState<{
        tokenId: string;
        groupId: string;
    } | null>(null);
    const tokenById = useMemo(() => new Map(content.tokens.map((token) => [token.id, token])), [content.tokens]);
        ...
        if (content.editor_groups?.length) {
            return content.editor_groups.map((group) => {
                ...
                for (const extraToken of group.extra_tokens ?? []) {
                    if (seenTokenIds.has(extraToken.id))
                        continue;
                    tokens.push({
                        id: extraToken.id,
    ...
    function handleBlankClick(groupId: string, blankId: string) {
        if (readOnly || !armedSelection || armedSelection.groupId !== groupId)
            return;
    ...
    return (<div className="rounded-[36px] border border-slate-200 bg-white shadow-2xl shadow-slate-200/30">
      ...
      <div className="space-y-6 p-6">
        {groups.map((group, groupIndex) => {
            const assignedTokenIds = new Set(group.blankIds
            ...
            return (<section key={group.id} className="rounded-[32px] border border-slate-200 bg-slate-50/50 p-5 shadow-sm">
                ...
                <div className="rounded-[30px] border border-slate-200 bg-[#fcfdff] px-6 py-6 text-lg leading-[2.2] text-slate-800">
                  {group.segments.map((segment, index) => {
                    if (segment.type === 'text') {
                        return (<span key={`${group.id}-text-${index}`} className="whitespace-pre-wrap">
                          {segment.text}
                        </span>);
                    }
                    const assignedToken = value[segment.id] ? tokenById.get(value[segment.id] ?? '') : null;
                    return (<button key={segment.id} type="button" disabled={readOnly} onClick={() => handleBlankClick(group.id, segment.id)} onDragOver={(event) => {
                            if (!readOnly)
                                event.preventDefault();
                        }} onDrop={(event) => {
                            if (readOnly)
                            ...
                            setArmedSelection(null);
                        }} className={`mx-1 inline-flex min-w-[140px] items-center justify-center rounded-2xl border-2 px-3 py-1.5 align-middle text-base font-black transition-all ${assignedToken
                ...
                <div className="rounded-[28px] border border-slate-200 bg-white p-5">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-sm font-semibold text-slate-600">Escolha um item dest? pergunta e depois toque na lacuna correspondente.</p>
                    </div>
                    {!readOnly ? (<button type="button" onClick={() => clearGroupAssignments(group.id, group.blankIds)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-[11px] font-black uppercase tracking-wider text-slate-500 hover:bg-slate-50">
                        Limpar selecao
                      </button>) : null}
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    ...
                    return (<button key={token.id} type="button" disabled={readOnly || isAssigned} draggable={!readOnly && !isAssigned} onDragStart={(event) => event.dataTransfer.setData('text/plain', token.id)} onClick={() => {
                            if (readOnly || isAssigned)
                                return;
                            setArmedSelection((current) => (current?.tokenId === token.id && current.groupId === group.id
                                ? null
                                : { tokenId: token.id, groupId: group.id }));
                        }} className={`rounded-2xl border px-4 py-2.5 text-sm font-black transition-all ${isAssigned
                            ? 'cursor-not-allowed border-slate-200 bg-slate-100 text-slate-300'
                            : isArmed
                                ? 'border-cyan-500 bg-cyan-50 text-cyan-800 shadow-lg shadow-cyan-100'
                  ...
                  {availableTokens.length < group.tokens.length ? (<div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                      <p className="text-[10px] font-black uppercase tracking-[0.28em] text-slate-400">Itens usados nest? pergunta</p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        {group.blankIds
                        .filter((blankId) => Boolean(value[blankId]))
                        .map((blankId) => (<button key={blankId} type="button" disabled={readOnly} onClick={() => !readOnly && onChange(blankId, null)} className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-xs font-black text-cyan-700 hover:bg-cyan-100">
