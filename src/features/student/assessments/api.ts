...
import type { Assessment, AssessmentAnswer, AssessmentAttempt, AssessmentCaseStudy, AssessmentInteractionResponsePayload, ImageHotspotResponsePayload, AssessmentQuestion, } from '@/types/content';
import { submitAssessmentAttemptSchema, type AssessmentAnswerInput, } from './schemas';
...
export async function submitAssessmentAttempt(assessmentId: string, answers: AssessmentAnswerInput[]) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        ...
        const payload = await response.json().catch(() => null) as SubmitAssessmentAttemptResult | {
            error?: string;
        } | null;
...
export async function requestAssessmentAttemptRetry(assessmentId: string, requestedMessage?: string) {
    ...
    return (result.data?.[0] as {
        request_id: string;
        status: AssessmentAttemptRequest['status'];
    } | undefined) ?? null;
}
...
export function createStudentImageHotspotAnswerPayload(mode: ImageHotspotResponsePayload['mode']): ImageHotspotResponsePayload {
    return {
        kind: 'image_hotspot',
        mode,
...
async function resolveAccessToken(forceRefresh = false) {
    ...
    if (!accessToken) {
        throw new Error("Sesso expirada. Faca login novamente para enviar a avalia??o.");
    }
    return accessToken;
}
