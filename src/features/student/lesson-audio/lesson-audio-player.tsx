...
export function LessonAudioPlayer({ lessonId, isAdmin = false }: LessonAudioPlayerProps) {
    ...
    return (<section className="overflow-hidden rounded-[32px] border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-cyan-50 shadow-sm">
      <div className="space-y-5 p-6 sm:p-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          {canGenerateAsStudent ? (<Button size="lg" className="h-12 rounded-2xl bg-slate-900 px-6 font-bold hover:bg-slate-800" disabled={isPreparing} onClick={() => void handlePrepareNarration('generate')}>
        ...
        {narration ? (<div className="space-y-4 rounded-[28px] border border-white/80 bg-white/90 p-5 shadow-sm">
            ...
            {currentPart ? (<audio ref={audioRef} key={currentPart.path} controls preload="metadata" className="w-full" src={currentPart.url} onEnded={handlePartEnded} onLoadedMetadata={() => setError(null)} onError={handleAudioError}/>) : null}

            {narration.parts.length > 1 ? (<p className="text-xs font-medium leading-relaxed text-slate-500">A narração foi dividida em partes para processar todo o conteúdo da aula. O player avança automticamente para o próximo trecho.
              </p>) : null}
          </div>) : null}

        {error ? (<div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700">
            {error}
          </div>) : null}

        {!isAdmin && technicalErrorMessage ? (<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm">
            <p className="font-medium text-amber-900">
              Se o problema continuar, você pode solicitar revisão da equipe.
            </p>
            <Button variant="outline" className="border-amber-300 bg-white text-amber-800 hover:bg-amber-100" disabled={isRequestingModeration || hasPendingModerationRequest} onClick={() => void handleRequestModeration()}>
