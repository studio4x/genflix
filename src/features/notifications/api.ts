...
export async function fetchPlatformNotifications(limit = 25) {
    ...
    const emptyProfileResult = { data: [] as Array<{
            id: string;
            full_name: string | null;
            email: string | null;
        }>, error: null };
    const emptyQueueResult = { data: [] as Array<{
            notification_id: string | null;
            channel: NotificationChannel;
        }>, error: null };
...
export async function processNotificationQueue(limit = 50) {
    ...
    const response = await fetch("/api/admin/notificationstask=process_queue", {
        method: 'POST',
        headers: {
    ...
    const payload = await response.json().catch(() => null) as Partial<NotificationQueueProcessResult> & {
        error?: string;
    } | null;
    if (!response.ok) {
