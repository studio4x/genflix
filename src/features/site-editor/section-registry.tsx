...
export function SectionStructureControl({ buttonLabel, pageKey, entryKey, label, sections, schema, className, }: {
    buttonLabel: string;
    pageKey: SitePageKey;
    entryKey: string;
    label: string;
    ...
    return (<div className={className ?? 'public-site-container py-4'}>
...
export function renderVisibleSectionList(sections: EditableListItem[], renderSection: (item: EditableListItem) => ReactNode | null) {
    return sections.map((item) => (<div key={item.id}>
      {renderSection(item)}
    </div>));
}
