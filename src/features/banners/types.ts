...
export type SiteBannerBackgroundPosition = 'center center' | 'left center' | 'right center' | 'center top' | 'center bottom' | 'left top' | 'right top' | 'left bottom' | 'right bottom';
...
export function createDefaultSiteBanner(sortOrder: number, locationKey: SiteBannerLocationKey = HOME_HERO_BANNER_LOCATION): Omit<SiteBanner, 'id' | 'createdBy' | 'updatedBy' | 'createdAt' | 'updatedAt'> {
    return {
        locationKey,
        name: `Banner ${sortOrder + 1}`,
        title: "O conhecimento que a sua carreira est?va esperando.",
