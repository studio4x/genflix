...
import { cloneBannerLayout, createDefaultSiteBanner, defaultBannerLayoutDesktop, defaultBannerLayoutMobile, defaultBannerBackgroundConfig, defaultBannerElementStyles, defaultBannerHeightDesktop, defaultBannerHeightMobile, defaultPrimaryBannerCta, defaultSecondaryBannerCta, HOME_HERO_BANNER_LOCATION, isBannerCta, type SiteBanner, type SiteBannerCta, type SiteBannerElementStyle, type SiteBannerElementStyles, type SiteBannerLayoutDesktop, type SiteBannerLayoutMobile, type SiteBannerCarouselTarget, type SiteBannerPlacementKey, type SiteBannerLocationKey, type SiteBannerThemePreset, type SiteBannerBackgroundConfig, type SiteBannerVersion, cloneBannerElementStyles, } from '@/features/banners/types';
...
type SiteBannerRow = {
    ...
    global_link_target: string | null;
    is_active: boolean;
    sort_order: number;
...
type UpdateSiteBannerInput = {
    ...
    globalLinkTarget?: SiteBanner['globalLinkTarget'];
    isActive?: boolean;
    sortOrder?: number;
    changeReason?: string;
};
...
function normalizeBanner(row: SiteBannerRow): SiteBanner {
    ...
    return {
        ...
        globalLinkTarget: row.global_link_target === 'new-tab' || row.global_link_target === 'new-window' || row.global_link_target === 'same-tab'
            ? row.global_link_target
            : (row.global_link_open_in_new_tab ? 'new-tab' : 'same-tab'),
...
function buildBannerSnapshot(banner: SiteBanner) {
    return {
        ...
        globalLinkTarget: banner.globalLinkTarget,
...
function normalizeBannerVersion(row: SiteBannerVersionRow): SiteBannerVersion | null {
    ...
    const snapshot = normalizeBanner({
        ...
        global_link_target: snapshotRecord.globalLinkTarget === 'new-tab' || snapshotRecord.globalLinkTarget === 'new-window' || snapshotRecord.globalLinkTarget === 'same-tab'
            ? snapshotRecord.globalLinkTarget
            : null,
...
export async function createSiteBanner(locationKey: SiteBannerLocationKey = HOME_HERO_BANNER_LOCATION) {
    ...
    const { data, error } = await supabase
        ...
        global_link_target: draft.globalLinkTarget,
...
export async function updateSiteBanner(input: UpdateSiteBannerInput) {
    ...
    if (input.backgroundAssetId !== undefined)
        payload.background_asset_id = input.backgroundAssetId;
    if (input.backgroundUrl !== undefined)
        payload.background_url = input.backgroundUrl;
    if (input.backgroundAssetIdMobile !== undefined)
        payload.background_asset_id_mobile = input.backgroundAssetIdMobile;
    if (input.backgroundUrlMobile !== undefined)
        payload.background_url_mobile = input.backgroundUrlMobile;
    ...
    if (input.themePreset !== undefined)
        payload.theme_preset = input.themePreset;
    if (input.layoutDesktop !== undefined)
        payload.layout_desktop = input.layoutDesktop;
    if (input.layoutMobile !== undefined)
        payload.layout_mobile = input.layoutMobile;
    if (input.heightDesktop !== undefined)
        payload.height_desktop = input.heightDesktop;
    if (input.heightMobile !== undefined)
        payload.height_mobile = input.heightMobile;
    if (input.elementStyles !== undefined)
        payload.element_styles = input.elementStyles;
    if (input.primaryCta !== undefined)
        payload.primary_cta = input.primaryCta;
    if (input.secondaryCta !== undefined)
        payload.secondary_cta = input.secondaryCta;
    if (input.globalLinkHref !== undefined)
        payload.global_link_href = input.globalLinkHref;
    if (input.globalLinkIsInternal !== undefined)
        payload.global_link_is_internal = input.globalLinkIsInternal;
    if (input.globalLinkOpenInNewTab !== undefined)
        payload.global_link_open_in_new_tab = input.globalLinkOpenInNewTab;
    if (input.globalLinkTarget !== undefined)
        payload.global_link_target = input.globalLinkTarget;
...
export async function duplicateSiteBanner(banner: SiteBanner) {
    ...
    const { data, error } = await supabase
        ...
        global_link_target: banner.globalLinkTarget,
        is_active: false,
...
export async function restoreSiteBannerVersion(version: SiteBannerVersion) {
    return updateSiteBanner({
        ...
        globalLinkTarget: version.snapshot.globalLinkTarget,
