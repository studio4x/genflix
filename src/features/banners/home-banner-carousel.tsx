...
import { DESKTOP_BANNER_DESIGN_WIDTH, MOBILE_BANNER_DESIGN_WIDTH, type SiteBanner, type SiteBannerCta, type SiteBannerLayoutItem, type SiteBannerLayoutKey, type SiteBannerLocationKey, } from '@/features/banners/types';
...
function BannerLink({ cta, className, customColors, }: {
    cta: SiteBannerCta;
    className?: string;
    customColors?: {
    ...
    if (cta.isInternal) {
        return (<GenflixCtaButton asChild tone={cta.tonePreset} className={className} customColors={customColors}>
...
function DesktopBannerContent({ banner, scaleFactor }: {
    banner: SiteBanner;
    scaleFactor: number;
}) {
    const theme = bannerThemeStyles[banner.themePreset];
    ...
    return (<div className="pointer-events-none absolute inset-0 hidden lg:block">
      <div className="absolute left-0 top-0 h-full" style={{
        ...
        {desktopElements.map((element) => element.shouldRender ? (<div key={element.key} className="pointer-events-auto absolute" style={{
...
function MobileBannerContent({ banner, scaleFactor }: {
    banner: SiteBanner;
    scaleFactor: number;
}) {
    const theme = bannerThemeStyles[banner.themePreset];
    ...
    return (<div className="pointer-events-none absolute inset-0 lg:hidden">
      <div className="absolute left-0 top-0 h-full" style={{
            width: `${MOBILE_BANNER_DESIGN_WIDTH}px`,
            transform: `scale(${scaleFactor})`,
            transformOrigin: 'top left',
        }}>
        {mobileElements.map((element) => element.shouldRender ? (<div key={element.key} className="pointer-events-auto absolute" style={{
                ...toDesignStyle(element.item),
                transform: getVerticalAnchorTransform(banner.elementStyles[element.key].verticalAlign),
            }}>
            {element.content}
...
export function HomeBannerCarousel({ fallback, autoplayIntervalMs = 6000, locationKey = 'home-hero', }: {
    fallback: ReactNode;
    ...
    return (<section className="relative overflow-hidden" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)} onFocusCapture={() => setIsPaused(true)} onBlurCapture={() => setIsPaused(false)} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} aria-label="Banner principal da home">
      <div className="home-banner-carousel-track relative" style={{
        ...
        {slides.map((banner, index) => {
            ...
            return (<article key={banner.id} className={cn('absolute inset-0 transition-opacity duration-700 ease-out', banner.globalLinkHref.trim() ? 'cursor-pointer' : '', isVisible ? 'opacity-100' : 'pointer-events-none opacity-0')} aria-hidden={!isVisible} onClick={(event) => handleBannerClick(banner, event)}>
      ...
      {slides.length > 1 ? (<>
          <div className="pointer-events-none absolute inset-y-0 left-0 z-20 flex items-center pl-3 sm:pl-4">
            <button type="button" onClick={() => goToNext(-1)} aria-label="Banner anterior" className="pointer-events-auto inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-[#0B2630]/45 text-white backdrop-blur-sm transition-colors hover:bg-[#0B2630]/65">
              <ChevronLeft className="h-5 w-5"/>
            </button>
          </div>

          <div className="pointer-events-none absolute inset-y-0 right-0 z-20 flex items-center pr-3 sm:pr-4">
            <button type="button" onClick={() => goToNext(1)} aria-label="Próximo banner" className="pointer-events-auto inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-[#0B2630]/45 text-white backdrop-blur-sm transition-colors hover:bg-[#0B2630]/65">
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-5 z-20 flex justify-center">
            <div className="pointer-events-auto inline-flex items-center gap-2 rounded-full border border-white/18 bg-[#0B2630]/32 px-3 py-2 backdrop-blur-sm">
              {slides.map((banner, index) => (<button key={banner.id} type="button" onClick={() => goToSlide(index)} aria-label={`Ir para o banner ${index + 1}`} aria-current={index === activeIndex} className={cn('h-2.5 rounded-full transition-all', index === activeIndex ? 'w-8 bg-white' : 'w-2.5 bg-white/42 hover:bg-white/68')}/>))}
            </div>
          </div>
        </>) : null}
    </section>);
