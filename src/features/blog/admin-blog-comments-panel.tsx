...
import { deleteBlogComment, listAdminBlogComments, moderateBlogComment, replyBlogComment, type BlogComment, type BlogCommentStatus, } from '@/features/blog/comments-api';
...
export function AdminBlogCommentsPanel() {
    ...
    return (<section className="space-y-4">
      ...
      <section className="border border-[#D8E6EB] bg-[#F8FBFC] p-4">
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_auto]">
          <label className="relative block">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} className="h-12 w-full border border-[#D8E6EB] bg-white pl-10 pr-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Buscar por artigo, nome, e-mail ou comentário"/>
          </label>

          <select value={status} onChange={(event) => setStatus(event.target.value as BlogCommentStatus | 'all')} className="h-12 border border-[#D8E6EB] bg-white px-3 text-sm font-bold text-[#15323b] outline-none focus:border-[#1398B7]">
      ...
      <section className="space-y-4">
        {isLoading && items.length === 0 ? (<div className="border border-[#D8E6EB] bg-white p-6 text-sm font-bold text-[#6d7f84]">Carregando comentários...</div>) : items.length === 0 ? (<div className="border border-dashed border-[#D8E6EB] bg-white p-8 text-center">
            <p className="font-readex text-lg font-semibold text-[#15323b]">Nenhum comentário neste recorte</p>
          </div>) : items.map((item) => (<article key={item.id} className="border border-[#D8E6EB] bg-white p-5 shadow-[0_14px_32px_rgba(21,50,59,0.04)]">
            <div className="flex flex-col gap-4">
              ...
              <div className="prose prose-sm max-w-none text-[#5f7077]" dangerouslySetInnerHTML={{ __html: sanitizeCommentHtml(item.content) }}/>

              {item.admin_response ? (<div className="rounded border border-blue-100 bg-blue-50 p-3 text-xs text-blue-700">
                  <p className="mb-2 font-bold">Resposta do admin:</p>
                  <div className="prose prose-sm max-w-none text-blue-700" dangerouslySetInnerHTML={{ __html: sanitizeCommentHtml(item.admin_response) }}/>
                </div>) : null}
              ...
              <textarea value={adminResponse} onChange={(event) => setAdminResponse(event.target.value)} rows={2} maxLength={500} className="w-full resize-none border border-[#D8E6EB] bg-white px-3 py-2 text-sm font-semibold leading-6 text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Resposta publica do administrador (opcional na aprova??o)"/>
              <textarea value={moderationReason} onChange={(event) => setModerationReason(event.target.value)} rows={2} maxLength={300} className="w-full resize-none border border-[#D8E6EB] bg-white px-3 py-2 text-sm font-semibold leading-6 text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Motivo para rejeição"/>
