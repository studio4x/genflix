...
export async function resolveBrazilCepAddress(postalCode: string) {
    ...
    if (!response.ok) {
        throw new Error("N?o foi possvel consultar o CEP informado.");
    }
    ...
    if (payload.erro || !payload.cep || !payload.uf || !payload.ibge) {
        throw new Error("CEP no encontrado.");
    }
    ...
    useEffect(() => {
        ...
        const timeoutId = window.setTimeout(() => {
            ...
            void resolveBrazilCepAddress(normalizedPostalCode)
                ...
                if (isActive) {
                    setAddress(null);
                    setAddressError(error instanceof Error ? error.message : "N?o foi possvel consultar o CEP informado.");
                }
            })
                .finally(() => {
                if (isActive) {
