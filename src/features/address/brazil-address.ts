...
export const brazilStateOptions: BrazilStateOption[] = [
    ...
    { value: 'RN', label: "Rio Grande do N?orte" },
...
async function loadBrazilCitiesByState(stateCode: string) {
    ...
    const response = await fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${encodeURIComponent(stateCode)}/municipios`);
    if (!response.ok) {
        throw new Error("N?o foi possvel carregar as cidades do est?do selecionado.");
    }
