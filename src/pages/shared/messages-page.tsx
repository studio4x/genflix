...
import { createCourseCreatorConversation, createDirectConversation, fetchConversationMessages, fetchConversations, fetchMessageReports, markConversationRead, reportConversationMessage, resolveMessageReport, searchMessageRecipients, sendConversationMessage, type ConversationMessage, type ConversationSummary, type MessageReportReason, type MessageReportSummary, type MessageRecipient, } from '@/features/messages/api';
    ...
    const loadConversations = useCallback(async () => {
        ...
        try {
            ...
            if (!selectedConversationId && rows.length > 0) {
                const nextConversationId = requestedConversationIdRef.current && rows.some((conversation) => conversation.conversation_id === requestedConversationIdRef.current)
    ...
    return (<div className="space-y-6">
      <header className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          {contextLabel === 'Admin' ? (<>
              <Button type="button" variant="outline" onClick={() => setSearchParams((current) => {
                const next = new URLSearchParams(current);
                next.set('section', 'messages');
                return next;
            }, { replace: true })} className={`h-11 rounded-none border-[#D8E6EB] bg-white font-black text-[#0A3640] ${adminSection === 'messages' ? 'border-[#1398B7] bg-[#E8F6FA]' : 'hover:border-[#1398B7]'}`}>
                Conversas
              </Button>
              <Button type="button" variant="outline" onClick={() => setSearchParams((current) => {
                const next = new URLSearchParams(current);
                ...
                return next;
            }, { replace: true })} className={`h-11 rounded-none border-[#D8E6EB] bg-white font-black text-[#0A3640] ${adminSection === 'reports' ? 'border-[#1398B7] bg-[#E8F6FA]' : 'hover:border-[#1398B7]'}`}>
                Denúncias
              </Button>
            </>) : null}
          <Button type="button" variant="outline" onClick={() => void loadConversations()} disabled={isLoadingConversations} className="h-11 rounded-none border-[#D8E6EB] bg-white font-black text-[#0A3640] hover:border-[#1398B7]">
      ...
      {contextLabel === 'Admin' && adminSection === 'reports' ? (<section className="border border-[#D8E6EB] bg-[#F8FBFC] p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button type="button" variant="outline" onClick={() => void loadMessageReports()} disabled={isLoadingReports} className="h-10 rounded-none border-[#D8E6EB] bg-white font-black text-[#0A3640] hover:border-[#1398B7]">
          ...
          <div className="mt-4 space-y-3">
            {isLoadingReports && messageReports.length === 0 ? (<p className="text-sm font-semibold text-[#6d7f84]">Carregando denúncias...</p>) : messageReports.length === 0 ? (<div className="border border-dashed border-[#D8E6EB] bg-white p-4 text-sm font-bold text-[#6d7f84]">
                Nenhuma denúncia pendente no momento.
              </div>) : (messageReports.map((report) => (<article key={report.report_id} className="border border-[#D8E6EB] bg-white p-4">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      {report.description ? (<p className="mt-2 text-xs font-semibold leading-5 text-[#6d7f84]">
                    ...
                    <Button type="button" onClick={() => void handleResolveReport(report.report_id)} className="rounded-none bg-[linear-gradient(180deg,#1398B7_0%,#0A3640_100%)] font-black text-white">
      ...
      {adminSection !== 'reports' ? (<section className="grid min-h-[680px] overflow-hidden border border-[#D8E6EB] bg-white shadow-[0_18px_42px_rgba(10,54,64,0.05)] xl:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="border-b border-[#D8E6EB] bg-[#F2F7F9] xl:border-b-0 xl:border-r">
          <div className="border-b border-[#D8E6EB] p-4">
            <label className="relative block">
              ...
              <input value={recipientQuery} onChange={(event) => setRecipientQuery(event.target.value)} className="h-12 w-full border border-[#D8E6EB] bg-white pl-10 pr-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Buscar usuário para conversar"/>
            </label>

            {recipientQuery.trim() ? (<div className="mt-3 max-h-56 overflow-y-auto border border-[#D8E6EB] bg-white">
                {isSearchingRecipients ? (<p className="p-3 text-sm font-semibold text-[#6d7f84]">Buscando...</p>) : recipients.length === 0 ? (<p className="p-3 text-sm font-semibold text-[#6d7f84]">Nenhum usuário encontrado.</p>) : (recipients.map((recipient) => (<button key={recipient.id} type="button" onClick={() => void handleSelectRecipient(recipient)} className="flex w-full items-center gap-3 border-b border-[#D8E6EB] px-3 py-3 text-left last:border-b-0 hover:bg-[#E8F6FA]">
          ...
          <div className="max-h-[560px] overflow-y-auto">
            {isLoadingConversations && visibleConversations.length === 0 ? (<p className="p-5 text-sm font-semibold text-[#6d7f84]">Carregando conversas...</p>) : visibleConversations.length === 0 ? (<div className="p-8 text-center">
              ...
              </div>) : (<div className="divide-y divide-[#D8E6EB]">
                {visibleConversations.map((conversation) => {
                    const isActive = conversation.conversation_id === selectedConversationId;
                    return (<button key={conversation.conversation_id} type="button" onClick={() => setSelectedConversationId(conversation.conversation_id)} className={cn('block w-full px-4 py-4 text-left transition', isActive ? 'bg-white' : 'hover:bg-white/70')}>
                      <span className="flex items-start justify-between gap-3">
                        ...
                        <span className="flex shrink-0 flex-col items-end gap-2">
                          ...
                          {conversation.unread_count > 0 ? (<span className="flex h-5 min-w-5 items-center justify-center bg-[#1398B7] px-1.5 text-[10px] font-black text-white">
        ...
        <div className="flex min-h-[680px] flex-col">
          {selectedConversation ? (<>
              ...
              <div className="flex-1 overflow-y-auto bg-white p-5">
                {isLoadingMessages && messages.length === 0 ? (<p className="text-sm font-semibold text-[#6d7f84]">Carregando mensagens...</p>) : messages.length === 0 ? (<div className="flex h-full min-h-[420px] items-center justify-center text-center">
                    <div>
                      ...
                      <p className="mt-1 text-sm font-medium text-[#6d7f84]">Envie a primeira mensagem dest conversa.</p>
                    </div>
                  </div>) : (<div className="space-y-4">
                    {messages.map((message) => {
                        const isMine = message.sender_id === user?.id;
                        return (<div key={message.id} className={cn('flex', isMine ? 'justify-end' : 'justify-start')}>
                          <div className={cn('max-w-[min(78%,680px)] border px-4 py-3 shadow-sm', isMine
              ...
              {isCreatorMonitoringCourseRoom ? (<div className="border-t border-[#D8E6EB] bg-[#F2F7F9] px-4 py-4">
                  ...
                  <div className="flex gap-3">
                    <textarea value={draft} onChange={(event) => setDraft(event.target.value)} maxLength={5000} rows={2} className="min-h-12 flex-1 resize-none border border-[#D8E6EB] bg-white px-4 py-3 text-sm font-semibold leading-6 text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Digite sua mensagem..." onKeyDown={(event) => {
                        if (event.key === 'Enter' && !event.shiftKey) {
                            event.preventDefault();
                            event.currentTarget.form?.requestSubmit();
                        }
                    }}/>
                    <Button type="submit" disabled={isSending || !draft.trim()} className="h-auto rounded-none bg-[linear-gradient(180deg,#1398B7_0%,#0A3640_100%)] px-5 font-black text-white hover:opacity-95">
      ...
      {reportTarget ? (<div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6" style={{ backgroundColor: 'rgba(10, 54, 64, 0.5)' }}>
          <form onSubmit={(event) => void handleReportMessage(event)} className="w-full max-w-lg border border-[#D8E6EB] bg-white p-5 shadow-[0_28px_80px_rgba(10,54,64,0.18)]">
            <div className="flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => setReportTarget(null)} className="flex h-9 w-9 items-center justify-center border border-[#D8E6EB] text-[#6d7f84] hover:text-[#15323b]" aria-label="Fechar">
                <X className="h-4 w-4"/>
              </button>
            </div>
            ...
            <label className="mt-4 block">
              ...
              <select value={reportReason} onChange={(event) => setReportReason(event.target.value as MessageReportReason)} className="mt-2 h-11 w-full border border-[#D8E6EB] bg-white px-3 text-sm font-bold text-[#15323b] outline-none focus:border-[#1398B7]">
                {Object.entries(reportReasonLabels).map(([value, label]) => (<option key={value} value={value}>{label}</option>))}
              </select>
            </label>
            ...
            <label className="mt-4 block">
              ...
              <textarea value={reportDescription} onChange={(event) => setReportDescription(event.target.value)} rows={4} maxLength={500} className="mt-2 w-full resize-none border border-[#D8E6EB] bg-white px-3 py-3 text-sm font-semibold leading-6 text-[#15323b] outline-none placeholder:text-[#8BA0A7] focus:border-[#1398B7]" placeholder="Conte rapidamente o que aconteceu."/>
            </label>

            <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <Button type="button" variant="outline" onClick={() => setReportTarget(null)} className="rounded-none border-[#D8E6EB] bg-white font-black !text-[#0A3640] hover:!text-[#0A3640]">
                Cancelar
              </Button>
              <Button type="submit" disabled={isReporting} className="rounded-none bg-[linear-gradient(180deg,#1398B7_0%,#0A3640_100%)] font-black text-white">
