...
import { fetchSupportMessageById, fetchSupportSettings, fetchSupportTicketDetail, sendSupportMessage, updateSupportTicketStatus, } from '@/features/support/api';
...
export function SupportTicketDetailPage() {
    ...
    const categoryConfig = useMemo(() => ticket ? getSupportCategoryConfig(ticket.category, settings?.sla) : null, [settings?.sla, ticket]);
    async function loadData() {
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel carregar o ticket.");
        }
        finally {
            setIsLoading(false);
        }
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel enviar a mensagem.");
        }
        finally {
            setIsSending(false);
        }
    ...
    if (!ticket) {
        ...
        <p className="text-sm font-semibold text-[#5F7077]">Ticket no encontrado.</p>
      </div>);
    }
        ...
        <section className="space-y-6 md:col-span-3">
          <article className="overflow-hidden rounded-[30px] border border-[#D8E6EB] bg-white shadow-sm">
            <div className="flex items-center justify-between gap-4 border-b border-[#D8E6EB] px-5 py-4">
              <div>
                <h2 className="font-readex text-xl font-semibold text-[#15323b]">Hist?rico de mensagens</h2>
              </div>

              {isAdminView ? (<select value={ticket.status} onChange={(event) => void handleStatusChange(event.target.value as SupportTicketStatus)} disabled={isUpdatingStatus} className="h-10 rounded-[14px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none">
                  {Object.entries(supportStatusLabelMap).map(([value, label]) => (<option key={value} value={value}>{label}</option>))}
                </select>) : null}
            </div>
            ...
            return (<div key={message.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[min(82%,720px)] rounded-[22px] border px-4 py-4 shadow-sm ${isMine
                    ? 'rounded-tr-[8px] border-[#1398B7] bg-[#1398B7] text-white'
                    : 'rounded-tl-[8px] border-[#D8E6EB] bg-white text-[#15323b]'}`}>
                        {!isMine ? (<p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#8BA0A7]">Equipe de suporte</p>) : null}
                        <p className="mt-2 whitespace-pre-wrap text-sm font-semibold leading-7">{message.message}</p>
                        {message.attachment_url ? (<div className="mt-4">
              ...
              {!canReply ? (<div className="border-t border-[#D8E6EB] bg-amber-50 px-5 py-4 text-sm font-semibold text-amber-800">
                  Este chamado foi encerrado.
                </div>) : null}

              {canReply ? (<form onSubmit={(event) => void handleSubmit(event)} className="border-t border-[#D8E6EB] bg-white p-4">
                  {attachment ? (<div className="mb-3 inline-flex items-center gap-2 rounded-full border border-[#D8E6EB] bg-[#F8FBFC] px-3 py-2 text-xs font-semibold text-[#15323b]">
                  ...
                  <div className="flex gap-3">
                    ...
                    <textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows={2} className="min-h-12 flex-1 resize-none rounded-[18px] border border-[#D8E6EB] bg-[#F8FBFC] px-4 py-3 text-sm font-semibold leading-6 text-[#15323b] outline-none" placeholder="Digite sua mensagem..." onKeyDown={(event) => {
                if (event.key === 'Enter' && !event.shiftKey) {
                    ...
                    <Button type="submit" disabled={isSending || (!draft.trim() && !attachment)} className="h-12 rounded-2xl bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-5 font-black text-white hover:opacity-95">
        ...
        <aside className="space-y-4">
          <article className="rounded-[30px] border border-[#D8E6EB] bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 space-y-4 text-sm">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#8BA0A7]">Usurio</p>
          ...
          <article className="rounded-[24px] border border-rose-200 bg-rose-50 px-4 py-4 text-sm leading-6 text-rose-900">
            <p className="font-black">Se houver fraude, segurana ou risco imediato, mantenha o chamado ativo e registre o maximo de contexto possvel.</p>
          </article>
        </aside>
      </div>
    </div>);
