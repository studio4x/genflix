...
import {
  ...
  type SecurityScanDashboard,
} from '@/features/admin/security-scans/api'
...
export function AdminSecurityScansPage() {
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Falha ao carregar central de seguranca.')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="space-y-7">
      <header className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            ...
            className="h-11 rounded-none bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-5 font-black text-white hover:opacity-95"
          >
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { label: 'Auto-fix disponivel', value: counters.autoFixCapable, helper: 'Itens com correcao automatizada' },
        ].map((card) => (
      ...
      <section className="border border-[#D8E6EB] bg-white p-6 shadow-[0_18px_42px_rgba(10,54,64,0.05)]">
        <div className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-2 font-readex text-2xl font-semibold text-[#15323b]">Cron da varredura de seguranca</h2>
            <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-[#6d7f84]">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
          <label className="space-y-2">
            ...
            <input
              ...
              className="h-12 w-full border border-[#D8E6EB] px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"
              placeholder="0 */6 * * *"
            />
        ...
        <div className="mt-4 flex flex-wrap gap-6">
          ...
          <label className="inline-flex items-center gap-2 text-sm font-semibold text-[#15323b]">
            <input type="checkbox" checked={autoFixEnabled} onChange={(event) => setAutoFixEnabled(event.target.checked)} />
            Correcao automatica ao detectar problema
          </label>
        </div>
      </section>
        ...
        <div className="mt-5 space-y-3">
          {(dashboard?.findings ?? []).length === 0 ? (
            ...
            dashboard?.findings.map((finding) => (
              <article key={finding.id} className="border border-[#D8E6EB] bg-[#FDFEFE] p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={cn(
                        ...
                        finding.status === 'fixed' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-amber-200 bg-amber-50 text-amber-700',
                      )}>
                  ...
                  <div className="flex shrink-0 flex-col gap-2">
                    {finding.fix_available && finding.status === 'open' ? (
                      <Button
                        type="button"
                        onClick={() => void handleFixFinding(finding.id)}
                        disabled={fixingFindingId === finding.id}
                        className="h-10 rounded-none bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-4 text-xs font-black uppercase tracking-[0.16em] text-white"
                      >
                    ...
                    {finding.status === 'fixed' ? (
                      <span className="inline-flex items-center gap-1 text-xs font-black uppercase tracking-[0.16em] text-emerald-700">
                        <CheckCircle2 className="h-4 w-4" /> [OK]
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-xs font-black uppercase tracking-[0.16em] text-amber-700">
      ...
      <section className="border border-[#D8E6EB] bg-white p-6 shadow-[0_18px_42px_rgba(10,54,64,0.05)]">
        <h2 className="border-b border-[#D8E6EB] pb-4 font-readex text-2xl font-semibold text-[#15323b]">Correcoes automaticas e manuais aplicadas</h2>
                ...
                (dashboard?.fixes ?? []).map((fix) => (
                  <tr key={fix.id} className="align-top">
                    ...
                    <td className="px-4 py-3">
                      <span className={cn(
                        'inline-flex border px-2 py-1 text-[10px] font-black uppercase tracking-[0.16em]',
                        fix.status === 'applied' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-red-200 bg-red-50 text-red-700',
                      )}>
                        {fix.status}
                      </span>
                    </td>
