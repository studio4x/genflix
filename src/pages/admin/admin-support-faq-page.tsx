...
import { createSupportFaq, deleteSupportFaq, fetchAdminSupportFaqs, fetchSupportFaqEvents, fetchSupportFaqSuggestions, fetchSupportSettings, updateSupportFaq, } from '@/features/support/api';
...
export function AdminSupportFaqPage() {
    ...
    async function loadData() {
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel carregar os dados. Tente novamente.");
        }
        finally {
    ...
    const categoryLabelMap = useMemo(() => new Map(faqCategoryOptions.map((item) => [item.key, item.label])), [faqCategoryOptions]);
    ...
    const selectedArticle = useMemo(() => filteredFaqs.find((item) => item.id === selectedArticleId) ?? filteredFaqs[0] ?? null, [filteredFaqs, selectedArticleId]);
    const kpis = useMemo(() => {
    ...
    async function handleSaveExisting(item: SupportFaqItem) {
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel salvar a FAQ.");
        }
        finally {
            setIsSaving(null);
        }
    ...
    async function handleCreate() {
        if (!newDraft.question.trim() || !newDraft.answer.trim()) {
            setErrorMessage("Revise os campos obrigatrios antes de publicar.");
            return;
        }
        setIsSaving('new');
        setErrorMessage(null);
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel criar a FAQ.");
        }
        finally {
            setIsSaving(null);
        }
    ...
    async function handleDelete(id: string) {
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel excluir a FAQ.");
        }
        finally {
            setIsSaving(null);
        }
    ...
    return (<div className="space-y-6">
      <header className="flex flex-col gap-4 rounded-[26px] border border-[#D8E6EB] bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 font-readex text-3xl font-semibold text-[#15323b]">Mdulo de FAQ</h1>
          <p className="mt-2 text-sm font-medium text-[#5F7077]">Gest?o editorial, categorizacao e visao operacional da base de ajuda.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <select value={filters.period} onChange={(event) => updateSearchParam('period', event.target.value)} className="h-11 rounded-2xl border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none">
          ...
          <Button type="button" variant="outline" onClick={() => void loadData()} disabled={isRefreshing} className="h-11 rounded-2xl border-[#D8E6EB] bg-white font-black text-[#15323b]">
      ...
      <section className="rounded-[30px] border border-[#D8E6EB] bg-white p-5 shadow-sm">
        <div className="flex flex-wrap gap-2">
          {(['artigos', 'categorias', 'feedback', 'sugestoes', 'analytics'] as AdminFaqTab[]).map((tab) => (<button key={tab} type="button" onClick={() => updateSearchParam('tab', tab)} className={`rounded-full px-4 py-2 text-xs font-black uppercase tracking-[0.14em] transition-colors ${filters.tab === tab
        ...
        {errorMessage ? (<div className="mt-4 rounded-[18px] border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{errorMessage}</div>) : null}
        {successMessage ? (<div className="mt-4 rounded-[18px] border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">{successMessage}</div>) : null}

        {filters.tab === 'artigos' ? (<div className="mt-5 space-y-4">
            <div className="grid gap-3 xl:grid-cols-[180px_180px_1fr]">
              <select value={filters.status} onChange={(event) => updateSearchParam('status', event.target.value)} className="h-11 rounded-[14px] border border-[#D8E6EB] bg-[#F8FBFC] px-3 text-sm font-semibold text-[#15323b] outline-none">
                <option value="all">Status: Todos</option>
                <option value="published">Status: Published</option>
                <option value="draft">Status: Draft</option>
              </select>

              <select value={filters.category} onChange={(event) => updateSearchParam('category', event.target.value)} className="h-11 rounded-[14px] border border-[#D8E6EB] bg-[#F8FBFC] px-3 text-sm font-semibold text-[#15323b] outline-none">
              ...
              <label className="flex h-11 items-center gap-2 rounded-[14px] border border-[#D8E6EB] bg-[#F8FBFC] px-3">
                ...
                <input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Buscar pergunta, resposta ou tag..." className="w-full bg-transparent text-sm font-semibold text-[#15323b] outline-none"/>
              </label>
            </div>

            <div className="grid gap-4 xl:grid-cols-[1.5fr_0.9fr]">
              <div className="overflow-hidden rounded-[22px] border border-[#D8E6EB]">
                {isLoading ? (<p className="p-5 text-sm font-semibold text-[#5F7077]">Carregando dados do FAQ...</p>) : filteredFaqs.length === 0 ? (<p className="p-5 text-sm font-semibold text-[#5F7077]">Nenhum item encontrado com os filtros atuais.</p>) : (<table className="w-full border-collapse text-sm">
                    ...
                    <tbody>
                      {filteredFaqs.map((item) => (<tr key={item.id} className={`cursor-pointer border-t border-[#D8E6EB] transition-colors hover:bg-[#F8FBFC] ${selectedArticle?.id === item.id ? 'bg-[#F2FBFF]' : ''}`} onClick={() => setSelectedArticleId(item.id)}>
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-[0.12em] ${item.is_published ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                              {item.is_published ? 'published' : 'draft'}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex justify-end gap-2">
                              <Button type="button" variant="destructive" onClick={(event) => {
                        event.stopPropagation();
                        void handleDelete(item.id);
                    }} disabled={isSaving === item.id} className="h-8 rounded-full px-3 text-xs font-black">
              ...
              <aside className="rounded-[22px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
                ...
                {!selectedArticle ? (<p className="mt-3 text-sm font-semibold text-[#5F7077]">Selecione um artigo para editar o contedo.</p>) : (<div className="mt-4 space-y-3">
                    <select value={selectedArticle.category_key} onChange={(event) => updateDraft(selectedArticle.id, { category_key: event.target.value })} className="h-10 w-full rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none">
                      {faqCategoryOptions.map((category) => (<option key={category.key} value={category.key}>{category.label}</option>))}
                    </select>
                    <input value={selectedArticle.question} onChange={(event) => updateDraft(selectedArticle.id, { question: event.target.value })} className="h-10 w-full rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none" placeholder="Pergunta"/>
                    <textarea value={selectedArticle.answer} onChange={(event) => updateDraft(selectedArticle.id, { answer: event.target.value })} className="min-h-28 w-full rounded-[12px] border border-[#D8E6EB] bg-white px-3 py-2 text-sm font-medium text-[#15323b] outline-none" placeholder="Resposta"/>
                    <div className="grid grid-cols-2 gap-2">
                      <input type="number" value={selectedArticle.sort_order} onChange={(event) => updateDraft(selectedArticle.id, { sort_order: Number(event.target.value) })} className="h-10 w-full rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none"/>
                      <label className="flex h-10 items-center gap-2 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b]">
                        <input type="checkbox" checked={Boolean(selectedArticle.is_published)} onChange={(event) => updateDraft(selectedArticle.id, { is_published: event.target.checked })}/>
                        Publicado
                      </label>
                    </div>
                    ...
                    <Button type="button" variant="outline" onClick={() => void handleSaveExisting(selectedArticle)} disabled={isSaving === selectedArticle.id} className="h-10 rounded-[12px] border-[#D8E6EB] bg-white px-4 text-sm font-black text-[#15323b]">
            ...
            <article className="rounded-[22px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
              ...
              <div className="mt-3 grid gap-2 lg:grid-cols-[180px_120px_1fr]">
                ...
                <input type="number" value={newDraft.sort_order} onChange={(event) => setNewDraft((current) => ({ ...current, sort_order: Number(event.target.value) }))} className="h-10 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none" placeholder="Ordem"/>
                <label className="flex h-10 items-center gap-2 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b]">
                  <input type="checkbox" checked={newDraft.is_published} onChange={(event) => setNewDraft((current) => ({ ...current, is_published: event.target.checked }))}/>
                  Publicado
                </label>
              </div>
        ...
        {filters.tab === 'categorias' ? (<div className="mt-5 rounded-[22px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
            ...
            <div className="mt-4 rounded-[14px] border border-[#D8E6EB] bg-white p-3">
              ...
              <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                ...
                if (!categoryKey) {
                    setErrorMessage("Informe um nome v?lido para criar a categoria.");
                    return;
                }
            ...
            <div className="mt-4 space-y-2">
              {faqCategoryOptions.map((category) => (<div key={category.key} className="flex items-center justify-between rounded-[12px] border border-[#D8E6EB] bg-white px-3 py-2">
                  <div>
        ...
        {filters.tab === 'feedback' ? (<div className="mt-5 rounded-[22px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
            <div className="flex items-center gap-2">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <article className="rounded-[14px] border border-[#D8E6EB] bg-white p-3">
                <p className="text-xs font-semibold text-[#5F7077]">Votos n?o uteis</p>
        ...
        {filters.tab === 'sugestoes' ? (<div className="mt-5 rounded-[22px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
            <div className="flex items-center gap-2">
              <FileText className="h-4 w-4 text-[#1398B7]"/>
            ...
            <div className="mt-4 overflow-hidden rounded-[18px] border border-[#D8E6EB] bg-white">
              {faqSuggestions.length === 0 ? (<p className="p-4 text-sm font-semibold text-[#5F7077]">Nenhuma sugest?o recebida no periodo.</p>) : (<table className="w-full border-collapse text-sm">
                  <thead className="bg-[#F2F7F9] text-left">
                    <tr>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.08em] text-[#8BA0A7]">Busca</th>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.08em] text-[#8BA0A7]">Sugest?o</th>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.08em] text-[#8BA0A7]">Categoria</th>
                      <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.08em] text-[#8BA0A7]">Data</th>
                    </tr>
                  </thead>
                  <tbody>
                    {faqSuggestions.map((item) => (<tr key={item.id} className="border-t border-[#D8E6EB]">
        ...
        {filters.tab === 'analytics' ? (<div className="mt-5 rounded-[22px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
            <div className="flex items-center gap-2">
            ...
            <div className="mt-3 rounded-[14px] border border-[#D8E6EB] bg-white p-3">
              ...
              {analyticsSummary.topQueries.length === 0 ? (<p className="mt-2 text-sm text-[#5F7077]">Nenhuma busca sem resultado registrada no periodo.</p>) : (<ul className="mt-2 space-y-1 text-sm text-[#15323b]">
                  {analyticsSummary.topQueries.map(([query, count]) => (<li key={query} className="flex items-center justify-between">
                      <span>{query}</span>
