...
export function AdminGroupsPage() {
  ...
  async function handleDeleteGroup(group: AccessGroup) {
    const confirmed = window.confirm(
      `Excluir o grupo "${group.name}"? Liberacoes por grupo relacionadas serao removidas.`,
    )
    if (!confirmed) {
      return
    }
  ...
  async function handleRemoveMember(member: GroupMemberView) {
    const confirmed = window.confirm(
      `Remover ${member.profile?.email ?? 'usuario'} do grupo?`,
    )
    if (!confirmed) {
      return
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600">
          Crie grupos e associe usuarios para liberar cursos por grupo.
        </p>
      </div>
            ...
            {groups.map((group) => (
              <article
                ...
                className={`rounded-lg border p-4 shadow-sm ${
                  selectedGroupId === group.id ? 'bg-slate-50' : 'bg-white'
                }`}
              >
            ...
            <>
              ...
              >
                <label className="flex-1 space-y-1">
                  <span className="text-sm text-slate-700">Adicionar usuario</span>
                  <select
                    className="w-full rounded-md border px-3 py-2 text-sm"
                    value={memberUserId}
              ...
              <div className="grid gap-2">
                ...
                {members.length === 0 ? (
                  <p className="text-sm text-slate-600">Grupo sem membros.</p>
                ) : null}
              </div>
            </>
          )}
