...
export function AdminMaterialsPage() {
  ...
  async function handleDelete(material: LessonMaterial) {
    const confirmed = window.confirm(
      `Excluir o material "${material.file_name}"?`,
    )
    if (!confirmed) {
      return
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3">
        {materials.map((material) => (
          <article key={material.id} className="rounded-lg border bg-white p-4 shadow-sm">
        ...
        {!isLoading && materials.length === 0 ? (
          <p className="text-sm text-slate-600">Nenhum material enviado.</p>
        ) : null}
      </div>
    </div>
  )
