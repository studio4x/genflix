...
export function AdminSupportTicketsPage() {
    ...
    async function loadData() {
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel carregar a fila de tickets.");
        }
        finally {
    ...
    async function handleInlineStatusChange(ticketId: string, nextStatus: SupportTicketStatus) {
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel atualizar o status do ticket.");
        }
    }
    async function handleDeleteTicket() {
        ...
        try {
            ...
            setErrorMessage(error instanceof Error ? error.message : "N?o foi possvel excluir o ticket.");
        }
    }
    return (<div className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <article className="rounded-[26px] border border-[#D8E6EB] bg-white p-5 shadow-sm">
          <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#5F7077]">SLA pblico</p>
      ...
      <section className="rounded-[30px] border border-[#D8E6EB] bg-white p-5 shadow-sm">
        <div className="grid gap-4 xl:grid-cols-[1.4fr_0.8fr_0.8fr_0.8fr]">
          <label className="grid gap-2">
            ...
            <div className="flex h-12 items-center gap-2 rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] px-4">
              ...
              <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Buscar por assunto, nome ou e-mail..." className="w-full bg-transparent text-sm font-semibold text-[#15323b] outline-none"/>
            </div>
          </label>

          <label className="grid gap-2">
            <span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#5F7077]">Status</span>
            <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'all' | SupportTicketStatus)} className="h-12 rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] px-4 text-sm font-semibold text-[#15323b] outline-none">
          ...
          <label className="grid gap-2">
            ...
            <select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value as 'all' | SupportTicketSummary['category'])} className="h-12 rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] px-4 text-sm font-semibold text-[#15323b] outline-none">
              <option value="all">Todas as categorias</option>
              {orderedCategories.map((item) => (<option key={item.key} value={item.key}>{item.label}</option>))}
            </select>
          </label>

          <label className="grid gap-2">
            <span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#5F7077]">Ordenacao</span>
            <select value={sortMode} onChange={(event) => setSortMode(event.target.value as TicketSortMode)} className="h-12 rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] px-4 text-sm font-semibold text-[#15323b] outline-none">
        ...
        <div className="mt-6 overflow-x-auto">
          {isLoading ? (<p className="py-6 text-sm font-semibold text-[#5F7077]">Carregando tickets...</p>) : filteredTickets.length === 0 ? (<div className="py-12 text-center">
            ...
            </div>) : (<table className="min-w-[1180px] w-full border-collapse text-sm">
              <thead className="bg-[#F2F7F9] text-left">
                <tr>
                  <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.08em] text-[#8BA0A7]">Usurio</th>
              ...
              <tbody>
                ...
                return (<tr key={ticket.id} className={`border-t border-[#D8E6EB] align-top transition-colors ${rowTone}`}>
                      ...
                      <td className="px-4 py-3">
                        <select value={ticket.status} onChange={(event) => void handleInlineStatusChange(ticket.id, event.target.value as SupportTicketStatus)} className="h-10 rounded-[14px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          <Button type="button" variant="outline" asChild className="h-9 rounded-full border-[#D8E6EB] bg-white px-4 text-xs font-black text-[#15323b]">
                            <Link to={`/admin/suporte/${ticket.id}`}>Responder</Link>
                          </Button>
                          <Button type="button" variant="destructive" onClick={() => setTicketToDelete(ticket)} className="h-9 rounded-full px-4 text-xs font-black">
      ...
      {ticketToDelete ? (<div className="fixed inset-0 z-[160] flex items-center justify-center bg-[#061b21]/58 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[30px] border border-[#D8E6EB] bg-white p-6 shadow-[0_30px_90px_rgba(6,27,33,0.24)]">
            ...
            <p className="mt-3 text-sm leading-7 text-[#5F7077]">Esta acao remove o ticket e todo o hist?rico de mensagens associado ao assunto <span className="font-black text-[#15323b]">{ticketToDelete.subject}</span>.
            </p>
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <Button type="button" variant="outline" onClick={() => setTicketToDelete(null)} className="h-11 rounded-2xl border-[#D8E6EB] bg-white font-black text-[#15323b]">
                Cancelar
              </Button>
              <Button type="button" variant="destructive" onClick={() => void handleDeleteTicket()} className="h-11 rounded-2xl px-5 font-black">
                Confirmar exclusao
              </Button>
            </div>
          </div>
