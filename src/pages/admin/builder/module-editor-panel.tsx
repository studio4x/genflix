...
import { createModule, deleteModule, deleteModulePdf, importCourseContent, toErrorMessage, updateModule, uploadModulePdf, } from '@/features/admin/content/api';
...
export function ModuleEditorPanel() {
    ...
    async function handleApplyAiFixes() {
        if (!courseId || !moduleId || !currentModule || !analysisTarget || !analysisResult?.corrected_module || didApplyCurrentAnalysis)
            return;
        ...
        try {
            ...
            if (currentReviewId && user?.id) {
                try {
                    ...
                    if (updatedReview) {
                        setReviewHistory((previous) => previous.map((review) => (review.id === updatedReview.id ? updatedReview : review)));
                    }
                }
                catch (historyError) {
    ...
    return (<div className="w-full space-y-6 animate-in fade-in duration-500">
       ...
       {!isNew && currentModule && (<div className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
           <Button type="button" variant="outline" onClick={() => void handleAnalyzeWithAi()} disabled={isAnalyzing || isSubmitting || isDeleting} className="h-10 rounded-xl border-blue-200 text-blue-600 hover:bg-blue-50 hover:text-blue-700">
       ...
       <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
         <div className="p-6 md:p-8 space-y-6">
            <label className="block space-y-2">
              ...
              <input className="w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm shadow-inner transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:bg-white placeholder:text-slate-400" placeholder="Ex: Fundamentos Básicos" value={form.title} onChange={(e) => setForm((prev) => ({ ...prev, title: e.target.value }))} required/>
            </label>

            <label className="block space-y-2">
              <span className="text-sm font-bold text-slate-800">Descrição Organizacional</span>
              <textarea className="min-h-[120px] w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm shadow-inner transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:bg-white resize-y placeholder:text-slate-400" placeholder="Detalhes para os alunos ou notas internas sobre este bloco." value={form.description} onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))}/>
            </label>
            ...
            <label className="flex items-start gap-4 p-5 rounded-xl border border-blue-100 bg-blue-50/30 cursor-pointer hover:bg-blue-50/50 transition-colors">
              <div className="flex items-center h-6">
                <input checked={form.is_required} type="checkbox" className="h-5 w-5 rounded border-blue-300 text-blue-600 focus:ring-blue-600 shadow-sm" onChange={(e) => setForm((prev) => ({ ...prev, is_required: e.target.checked }))}/>
              </div>
              <div>
            ...
            <div className="rounded-[24px] border border-slate-200 bg-slate-50/50 p-5">
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <label className="block space-y-2">
                  ...
                  <input type="datetime-local" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" value={form.starts_at ?? ''} onChange={(event) => setForm((prev) => ({ ...prev, starts_at: event.target.value }))}/>
                </label>
                <label className="block space-y-2">
                  <span className="text-sm font-bold text-slate-700">Expirar em</span>
                  <input type="datetime-local" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" value={form.ends_at ?? ''} onChange={(event) => setForm((prev) => ({ ...prev, ends_at: event.target.value }))}/>
                </label>
              </div>
              <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50/50 p-4">
                <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                  ...
                  <label className="block space-y-2 md:min-w-[220px]">
                    ...
                    <input type="number" min={0} step={1} className="w-full rounded-xl border border-blue-200 bg-white px-4 py-3 text-sm shadow-sm focus:border-blue-400 focus:ring-2 focus:ring-blue-100" value={form.release_days_after_enrollment ?? ''} onChange={(event) => setForm((prev) => ({ ...prev, release_days_after_enrollment: event.target.value }))} placeholder="Ex.: 7"/>
                  </label>
                </div>
              </div>
            ...
            {!isNew ? (<div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
                ...
                {currentModule?.module_pdf_storage_path ? (<div className="mt-4 flex gap-3">
                    <Button type="button" variant="outline" className="rounded-2xl border-rose-200 text-rose-600 hover:bg-rose-50" onClick={() => void handleDeleteModulePdf()}>
                      Remover PDF
                    </Button>
                  </div>) : null}
              </div>) : null}
         ...
         {error && (<div className="mx-6 md:mx-8 mb-6 p-4 rounded-xl border border-red-200 bg-red-50 text-sm text-red-600 flex items-start gap-2">
         ...
         <div className="bg-slate-50 border-t border-slate-100 p-6 flex items-center justify-between">
           {!isNew && (<Button type="button" variant="outline" onClick={handleDelete} disabled={isDeleting || isSubmitting} className="border-rose-200 text-rose-600 hover:bg-rose-50 hover:text-rose-700">
               {isDeleting ? 'Excluindo...' : 'Excluir Módulo'}
             </Button>)}
           <div className={`flex gap-3 ${isNew ? 'w-full justify-end' : ''}`}>
             <Button type="submit" disabled={isSubmitting} className="bg-blue-600 hover:bg-blue-700 text-white min-w-[140px] shadow-sm font-bold">
        ...
        {isAppliedAdjustmentsModalOpen && (<div className="fixed inset-0 z-[112] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="max-h-[95vh] w-full max-w-4xl overflow-y-auto rounded-[32px] border border-white/20 bg-white shadow-2xl animate-in zoom-in-95 duration-300">
              <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-8">
                ...
                <button type="button" onClick={() => setIsAppliedAdjustmentsModalOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-50 text-slate-400 transition-colors hover:text-slate-900">
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                  </svg>
                </button>
              ...
              <div className="flex justify-end border-t border-slate-100 bg-slate-50/50 p-8">
                <Button type="button" variant="ghost" className="h-12 rounded-2xl px-8 font-black text-slate-600" onClick={() => setIsAppliedAdjustmentsModalOpen(false)}>
                  Fechar
                </Button>
              </div>
            </div>
