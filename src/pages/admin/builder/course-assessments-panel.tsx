        ...
        <div className="grid gap-6">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 p-5">
              ...
              <div className="flex items-center gap-2">
                <Button size="sm" variant="ghost" onClick={() => void handleExportFinalAssessment()} className="font-bold text-slate-600 hover:bg-slate-100" disabled={isExportingFinal}>
            ...
            <div className="p-6">
              {courseTree.courseAssessments.length > 0 ? (courseTree.courseAssessments.map((assessment) => (<div key={assessment.id} className="flex items-center justify-between rounded-xl border border-emerald-100 bg-emerald-50/20 p-4">
                    <div>
                      ...
                      <p className="mt-1 text-[11px] font-medium uppercase text-emerald-600">Quiz de certificação • Nota m\u00ednima {assessment.passing_score}%
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => void handleDeleteAssessment(assessment.id)} className="h-8 w-8 p-0 font-bold text-slate-400 hover:text-red-500" title="Excluir Avaliação Final">
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                        </svg>
                      </Button>
                    </div>
                  </div>))) : (<div className="rounded-2xl border border-slate-100 bg-slate-50/50 py-8 text-center">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              {courseTree.modules.map((module, index) => (<div key={module.id} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    ...
                    {module.assessments.map((assessment) => (<div key={assessment.id} className="flex items-center gap-1">
                        <Button variant="outline" size="sm" className="bg-white" asChild>
                          <Link to={`/admin/cursos/${courseId}/builder/modulos/${module.id}/avaliacoes/${assessment.id}`}>Editar Quiz</Link>
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => void handleDeleteAssessment(assessment.id)} className="h-8 w-8 p-0 font-bold text-slate-400 hover:text-red-500" title="Excluir Quiz do Módulo">
                          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                          </svg>
                        </Button>
                ...
                setIsImportModalOpen(true);
            }} className="h-8 w-8 p-0 font-bold text-slate-400 hover:text-blue-600" title="Importar de IA para este módulo">
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z"/>
                      </svg>
                    </Button>
