...
import { createAssessmentCaseStudy, createAssessmentOption, createAssessmentQuestion, createFinalAssessment, createModuleAssessment, deleteAssessment, deleteAssessmentCaseStudy, deleteAssessmentOption, deleteAssessmentOptionsByQuestion, deleteAssessmentQuestion, fetchAssessmentById, fetchAssessmentCaseStudies, fetchAssessmentQuestions, fetchFinalAssessment, importAssessmentContentStructured, toErrorMessage, updateAssessmentCaseStudy, updateAssessment, updateAssessmentOption, updateAssessmentQuestion, type AssessmentCaseStudyWithQuestions, type AssessmentQuestionWithOptions, type ImportAssessmentData, } from '@/features/admin/assessments/api';
import { createAnswerKeyFromInteraction, createDefaultInteractionContent, assessmentInteractionContentSchema, getInteractionSlotIds, isGamifiedQuestionType, } from '@/features/assessments/gamified';
import { canCourseUseCaseStudies, isCourseQuestionTypeEnabled, normalizeCourseQuizTypeSettings, } from '@/features/assessments/course-quiz-type-settings';
...
function sortCaseStudyQuestions(items: AssessmentQuestionWithOptions[]) {
    return [...items].sort((questionA, questionB) => (questionA.case_question_position ?? 0) - (questionB.case_question_position ?? 0));
}
function buildQuestionState(question: Omit<AssessmentQuestionWithOptions, 'options' | 'interaction' | 'answer_key'>, overrides?: Partial<AssessmentQuestionWithOptions>): AssessmentQuestionWithOptions {
    return {
        ...question,
...
function getQuestionScoringExplanation(question: AssessmentQuestionWithOptions) {
    if (question.question_type === 'essay_ai') {
        return "Pergunta com feedback da IA. Ela no soma pontos na nota final do quiz.";
    }
...
function getDefaultGamifiedState(questionId: string, questionType: AssessmentQuestionType, options?: {
    hotspotMode?: ImageHotspotMode;
}) {
...
function collectImportedQuestionTypes(data: ImportAssessmentData) {
    ...
    for (const caseStudy of data.case_studies ?? []) {
        for (const question of caseStudy.questions ?? []) {
            importedTypes.add(question.question_type === 'case_study_ai' || question.question_type === 'case_study_single_choice'
...
export function AssessmentBuilderPanel() {
    ...
    function updateQuestionState(questionId: string, updater: (question: AssessmentQuestionWithOptions) => AssessmentQuestionWithOptions) {
        const standaloneQuestion = questionsRef.current.find((question) => question.id === questionId);
        if (standaloneQuestion) {
            const nextQuestions = questionsRef.current.map((question) => (question.id === questionId ? updater(question) : question));
        ...
        const nextCaseStudies = caseStudiesRef.current.map((caseStudy) => ({
            ...caseStudy,
            questions: caseStudy.questions.map((question) => (question.id === questionId ? updater(question) : question)),
        }));
    ...
    function updateOptionState(optionId: string, updater: (option: AssessmentQuestionWithOptions['options'][number]) => AssessmentQuestionWithOptions['options'][number]) {
    ...
    async function handleAddQuestion(questionType: AssessmentQuestionType = 'single_choice', options?: {
        hotspotMode?: ImageHotspotMode;
    }) {
        if (!assessment)
            return;
    ...
    function handleCaseStudyChange(caseStudyId: string, updates: {
        title?: string;
        case_text?: string;
    }) {
        setCaseStudies((prev) => prev.map((caseStudy) => (caseStudy.id === caseStudyId
            ? {
                ...caseStudy,
                title: updates.title ?? caseStudy.title,
                case_text: updates.case_text ?? caseStudy.case_text,
    ...
    async function handleDeleteCaseStudy(caseStudyId: string) {
        if (!window.confirm("Excluir este estudo de caso e todas as perguntas vinculadas"))
            return;
        try {
    ...
    async function handleAddCaseQuestion(caseStudyId: string, questionType: 'case_study_ai' | 'case_study_single_choice' = 'case_study_single_choice') {
        ...
        try {
            ...
            setCaseStudies((prev) => prev.map((caseStudyItem) => (caseStudyItem.id !== caseStudyId
                ? caseStudyItem
                : {
                    ...caseStudyItem,
    ...
    async function handleUpdateQuestionType(questionId: string, questionType: AssessmentQuestionType) {
        ...
        try {
            ...
            const nextQuestionState = (question: AssessmentQuestionWithOptions) => (question.id !== questionId
                ? question
                : {
                    ...question,
                    question_type: questionType,
    ...
    async function handleAddOption(questionId: string) {
        const question = findQuestion(questionId);
        if (!question || (question.question_type !== 'single_choice' && question.question_type !== 'case_study_single_choice'))
            return;
        try {
    ...
    async function handleUpdateOption(optionId: string, optionText: string, isCorrect: boolean) {
        try {
            ...
            const standaloneQuestion = questions.find((question) => question.options.some((option) => option.id === optionId));
            if (standaloneQuestion) {
                setQuestions((prev) => prev.map((question) => (question.options.some((option) => option.id === optionId)
            ...
            setCaseStudies((prev) => prev.map((caseStudy) => ({
                ...caseStudy,
                questions: caseStudy.questions.map((question) => (question.options.some((option) => option.id === optionId)
                    ? updateQuestionOptions(question)
                    : question)),
            })));
        }
...
Todas as quest\u00F5es, estudos de caso e tentativas vinculadas ser\u00E3o removidos permanentemente.` : `CUIDADO: Excluir o quiz "${assessment.title}"
...
Todas as quest\u00F5es, estudos de caso e tentativas vinculadas ser\u00E3o removidos permanentemente.`;
        ...
        try {
            ...
            navigate(isFinal
                ...
                : `/admin/cursos/${courseId}/builder/modulos/${moduleId}`, { replace: true });
        }
        catch (deleteError) {
            setError(toErrorMessage(deleteError));
        }
    ...
    function renderQuestionCard(question: AssessmentQuestionWithOptions, indexLabel: string, context: 'standalone' | 'case-study') {
        ...
        const isCurrentTypeDisabled = !isCourseQuestionTypeEnabled(quizTypeSettings, question.question_type, globalQuizTypeSettings);
        ...
        const typeButtons: Array<{
            ...
            ...(isStandalone
                ...
                : []),
        ].filter((button) => (isCourseQuestionTypeEnabled(quizTypeSettings, button.type, globalQuizTypeSettings) ||
            question.question_type === button.type));
        return (<div key={question.id} className={cn('group animate-in rounded-3xl border border-slate-200 bg-white shadow-sm transition-all slide-in-from-bottom-4 duration-500 hover:border-blue-300', isGamified ? 'overflow-visible' : 'overflow-hidden')}>
        <div className="flex items-start gap-4 border-b border-slate-100 bg-slate-50/50 p-6">
          ...
          <div className="flex-1 space-y-3">
            ...
            {showLegacyControls ? (<div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={() => setType(isStandalone ? 'single_choice' : 'case_study_single_choice')} className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest transition-colors ${canUseSingleChoice
                    ? 'bg-blue-600 text-white'
                    : 'border border-slate-200 bg-white text-slate-500 hover:border-blue-200 hover:text-blue-600'}`}>
                Múltipla Escolha
              </button>
              <button type="button" onClick={() => setType(isStandalone ? 'essay_ai' : 'case_study_ai')} className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest transition-colors ${canUseEssay
                    ? 'bg-amber-500 text-white'
                    : 'border border-slate-200 bg-white text-slate-500 hover:border-amber-200 hover:text-amber-700'}`}>
                Discursiva com IA
              </button>
              {isStandalone ? (<>
                  <button type="button" onClick={() => setType('drag_drop_labeling')} className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest transition-colors ${canUseDragDrop
                        ? 'bg-cyan-600 text-white'
                        : 'border border-slate-200 bg-white text-slate-500 hover:border-cyan-200 hover:text-cyan-700'}`}>
                    Arrastar e Soltar
                  </button>
                  <button type="button" onClick={() => setType('fill_in_the_blanks')} className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest transition-colors ${canUseFillInTheBlanks
                        ? 'bg-teal-600 text-white'
                        : 'border border-slate-200 bg-white text-slate-500 hover:border-teal-200 hover:text-teal-700'}`}>
                    Preencher Lacunas
                  </button>
                  <button type="button" onClick={() => setType('image_hotspot')} className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest transition-colors ${canUseImageHotspot
                        ? 'bg-sky-600 text-white'
                        : 'border border-slate-200 bg-white text-slate-500 hover:border-sky-200 hover:text-sky-700'}`}>
                    Quiz de Hotspot
                  </button>
                  <button type="button" onClick={() => setType('coloring')} className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest transition-colors ${canUseColoring
            ...
            <textarea className="w-full resize-none border-none bg-transparent p-0 font-bold text-slate-800 placeholder:text-slate-300 focus:ring-0" value={question.question_text} onChange={(event) => handleQuestionTextChange(question.id, event.target.value)} onBlur={() => void handlePersistQuestion(question.id, { question_text: question.question_text })} placeholder="Escreva sua pergunta aqui..." rows={2}/>
          </div>

          <button onClick={() => void handleDeleteQuestion(question.id)} className="rounded-lg p-2 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500">
        ...
        <div className="space-y-4 p-6">
          <div className="flex items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
            ...
            <input type="number" min={question.question_type === 'essay_ai' ? 0 : 1} className="w-20 rounded-xl border border-slate-200 bg-white px-3 py-2 text-right text-sm font-black text-slate-700 focus:border-blue-300 focus:ring-4 focus:ring-blue-100" value={question.question_type === 'essay_ai' ? 0 : Number(question.points || 1)} disabled={question.question_type === 'essay_ai'} onChange={(event) => {
                const points = Number(event.target.value || 0);
                void handlePersistQuestion(question.id, { points });
                if (question.case_study_id) {
                    setCaseStudies((prev) => prev.map((caseStudy) => ({
                        ...caseStudy,
                        questions: caseStudy.questions.map((item) => (item.id === question.id
                            ? { ...item, points }
                            : item)),
                    })));
                }
          ...
          {isEssay ? (<>
              ...
              <textarea className="min-h-[140px] w-full rounded-2xl border border-amber-100 bg-white px-4 py-4 text-sm text-slate-700 focus:border-amber-300 focus:ring-4 focus:ring-amber-100" value={question.essay_expected_answer ?? ''} onChange={(event) => void handleUpdateEssayExpectedAnswer(question.id, event.target.value)} onBlur={() => void handlePersistQuestion(question.id, { essay_expected_answer: question.essay_expected_answer ?? '' })} placeholder="Descreva a resposta esperada para orientar a avaliação da IA."/>
            </>) : isGamified ? (<GamifiedQuestionEditor question={question} onDraftChange={(updates) => updateQuestionState(question.id, (currentQuestion) => ({
                    ...currentQuestion,
                    ...updates,
                }))} onPersist={(updates) => handlePersistQuestion(question.id, updates)} onError={setError}/>) : (<>
              <p className="mb-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Alternativas de Resposta</p>
              <div className="grid gap-3">
                {question.options.map((option) => (<div key={option.id} className="group/opt flex items-center gap-3">
                    <button onClick={() => void handleUpdateOption(option.id, option.option_text, !option.is_correct)} className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition-all ${option.is_correct
                        ? 'border-emerald-500 bg-emerald-500 text-white'
                        : 'border-slate-200 hover:border-emerald-300'}`}>
                      {option.is_correct ? (<svg className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/>
                        </svg>) : null}
                    </button>

                    <input className={`flex-1 rounded-xl border-none px-4 py-3 text-sm transition-all focus:ring-2 focus:ring-blue-100 ${option.is_correct
                        ? 'bg-emerald-50 font-bold text-emerald-700'
                        : 'bg-slate-50 text-slate-600'}`} value={option.option_text} onChange={(event) => handleOptionTextChange(option.id, event.target.value)} onBlur={() => void handleUpdateOption(option.id, option.option_text, option.is_correct)} placeholder="Descreva a alternativa..."/>

                    <button onClick={() => void handleDeleteOption(option.id)} className="p-2 text-slate-300 opacity-0 transition-all group-hover/opt:opacity-100 hover:text-rose-500">
              ...
              <button onClick={() => void handleAddOption(question.id)} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-100 py-3 text-xs font-bold text-slate-400 transition-all hover:border-blue-200 hover:bg-blue-50 hover:text-blue-600">
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    if (!assessment || !assessmentDraft) {
        return (<div className="mx-auto max-w-2xl animate-in space-y-6 py-20 text-center fade-in zoom-in duration-500">
    ...
    return (<div className="w-full animate-in space-y-8 pb-24 fade-in duration-500">
      <div className="flex flex-col justify-between gap-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:flex-row md:items-end">
        <div className="flex-1 space-y-1">
          ...
          <input className="w-full border-none bg-transparent p-0 text-3xl font-black text-slate-900 placeholder:text-slate-300 focus:ring-0" value={assessmentDraft.title} onChange={(event) => setAssessmentDraft((prev) => (prev ? { ...prev, title: event.target.value } : prev))} onBlur={() => void persistAssessmentDraft({ title: assessmentDraft.title })} placeholder="Título do quiz..."/>
          <input className="w-full border-none bg-transparent p-0 text-sm font-medium text-slate-500 placeholder:text-slate-400 focus:ring-0" value={assessmentDraft.description} onChange={(event) => setAssessmentDraft((prev) => (prev ? { ...prev, description: event.target.value } : prev))} onBlur={() => void persistAssessmentDraft({ description: assessmentDraft.description })} placeholder="Breve descrição ou instruções para o aluno..."/>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-2">
          <div className="border-r border-slate-200 px-3 text-center">
            ...
            <input type="number" className="w-12 border-none bg-transparent p-0 text-center font-black text-blue-600 focus:ring-0" value={assessmentDraft.passing_score} onChange={(event) => setAssessmentDraft((prev) => (prev ? { ...prev, passing_score: Number(event.target.value || 0) } : prev))} onBlur={() => void persistAssessmentDraft({ passing_score: assessmentDraft.passing_score })}/>
            <span className="text-xs font-bold text-blue-600">%</span>
          </div>

          <div className="px-3 text-center">
            <p className="text-[10px] font-black uppercase tracking-tighter text-slate-400">Tentativas</p>
            <input type="number" className="w-12 border-none bg-transparent p-0 text-center font-black text-blue-600 focus:ring-0" value={assessmentDraft.max_attempts} onChange={(event) => setAssessmentDraft((prev) => (prev ? { ...prev, max_attempts: Number(event.target.value || 0) } : prev))} onBlur={() => void persistAssessmentDraft({ max_attempts: assessmentDraft.max_attempts })}/>
          </div>
          ...
          <div className="border-l border-slate-200 px-3 text-center">
            ...
            <input type="number" className="w-12 border-none bg-transparent p-0 text-center font-black text-blue-600 focus:ring-0" value={assessmentDraft.estimated_minutes} onChange={(event) => setAssessmentDraft((prev) => (prev ? { ...prev, estimated_minutes: Number(event.target.value || 0) } : prev))} onBlur={() => void persistAssessmentDraft({ estimated_minutes: assessmentDraft.estimated_minutes })}/>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <Button variant="default" onClick={() => void handleSaveAssessmentSettings()} disabled={isLoading} className="flex h-10 items-center gap-2 rounded-xl bg-blue-600 px-4 font-bold text-white transition-all hover:bg-blue-700">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <Button variant="outline" onClick={() => setIsImportModalOpen(true)} className="flex h-10 items-center gap-2 rounded-xl border-slate-200 bg-white px-4 font-bold text-slate-500 transition-all hover:bg-slate-50">
      ...
      {error ? (<div className="flex items-center gap-2 rounded-xl border border-rose-100 bg-rose-50 p-4 text-sm font-medium text-rose-600">
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="space-y-6">
        ...
        {caseStudies.map((caseStudy, caseStudyIndex) => (<section key={caseStudy.id} className="overflow-hidden rounded-[32px] border border-amber-200 bg-white shadow-sm">
            <div className="border-b border-amber-100 bg-amber-50/70 p-6">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 space-y-4">
                  ...
                  <input className="w-full border-none bg-transparent p-0 text-2xl font-black text-slate-900 placeholder:text-slate-400 focus:ring-0" value={caseStudy.title ?? ''} onChange={(event) => handleCaseStudyChange(caseStudy.id, { title: event.target.value })} onBlur={() => void handleUpdateCaseStudy(caseStudy.id, { title: caseStudy.title ?? '' })} placeholder="Título do estudo de caso"/>

                  <textarea className="min-h-[180px] w-full rounded-3xl border border-amber-100 bg-white px-5 py-4 text-sm leading-relaxed text-slate-700 focus:border-amber-300 focus:ring-4 focus:ring-amber-100" value={caseStudy.case_text} onChange={(event) => handleCaseStudyChange(caseStudy.id, { case_text: event.target.value })} onBlur={() => void handleUpdateCaseStudy(caseStudy.id, { case_text: caseStudy.case_text })} placeholder="Descreva o contexto completo do caso para o aluno analisar."/>
                </div>

                <button onClick={() => void handleDeleteCaseStudy(caseStudy.id)} className="rounded-xl p-3 text-slate-300 transition-colors hover:bg-rose-50 hover:text-rose-500">
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v2m3 4h5"/>
                  </svg>
                </button>
      ...
      {isHotspotModeModalOpen ? (<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
          <div className="w-full max-w-xl rounded-[32px] border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-900/20">
            <div className="flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => setIsHotspotModeModalOpen(false)} className="rounded-full border border-slate-200 px-3 py-1 text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50">
                Fechar
              </button>
            </div>
      ...
      {isImportModalOpen ? (<div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-300">
