...
const BACKGROUND_POSITION_OPTIONS: Array<{
    value: SiteBannerBackgroundPosition;
    label: string;
}> = [
...
const BACKGROUND_SIZE_OPTIONS: Array<{
    value: SiteBannerBackgroundSize;
    label: string;
}> = [
...
const BACKGROUND_REPEAT_OPTIONS: Array<{
    value: SiteBannerBackgroundRepeat;
    label: string;
}> = [
...
function keepLayoutInsideCanvas<T extends Record<SiteBannerLayoutKey, SiteBannerLayoutItem>>(layout: T, canvasWidth: number, canvasHeight: number) {
    const nextLayout = { ...layout } as T;
    let movedCount = 0;
    for (const key of LAYOUT_KEYS) {
...
function ColorField({ label, value, fallback, onChange, }: {
    ...
    return (<label className="grid gap-2">
      ...
      <div className="flex items-center gap-2 rounded-[16px] border border-[#D8E6EB] bg-white px-3 py-2">
        <input type="color" value={displayValue} onChange={(event) => onChange(event.target.value)} className="h-8 w-10 cursor-pointer rounded-md border border-[#D8E6EB] bg-transparent p-0"/>
        <input value={displayValue} onChange={(event) => onChange(event.target.value)} className="h-8 flex-1 bg-transparent text-sm font-semibold text-[#15323b] outline-none"/>
      </div>
    </label>);
}
function CollapsibleCard({ title, summary, open, onToggle, children, className, bodyClassName, }: {
    title: string;
    summary?: string;
    open: boolean;
    onToggle: () => void;
    ...
    return (<section className={cn('rounded-[24px] border border-[#D8E6EB] bg-[#F8FBFC] p-4', className)}>
...
function LayoutControls({ item, labelPrefix, minWidth = DEFAULT_MIN_LAYOUT_WIDTH, onVisibleChange, onWidthChange, onZIndexChange, }: {
    ...
    return (<div className="grid min-w-0 gap-2.5">
      <label className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-[#5F7077]">
        <input type="checkbox" checked={item.visible} onChange={(event) => onVisibleChange(event.target.checked)}/>
        Visivel
      </label>

      <label className="grid gap-2">
        <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">{labelPrefix} largura</span>
        <input type="range" min={minWidth} max={100} step={0.5} value={item.width} onChange={(event) => onWidthChange(Number(event.target.value))}/>
...
function PreviewCta({ cta, colors, className, }: {
    cta: SiteBannerCta;
    colors?: SiteBannerElementStyle;
    className?: string;
}) {
    return (<GenflixCtaButton type="button" tone={cta.tonePreset} customColors={{
...
function BannerCanvasElement({ elementKey, item, children, verticalAlign, onPointerDown, onResizePointerDown, draggable = true, style, }: {
    ...
    return (<div className={cn('group absolute', draggable ? 'cursor-grab touch-none select-none active:cursor-grabbing' : 'cursor-default')} style={style ?? {
            left: `${item.x}px`,
            top: `${item.y}px`,
            width: `${item.width}%`,
            zIndex: item.zIndex,
      ...
      <div className={cn('pointer-events-none absolute -left-2 -top-2 inline-flex items-center gap-1 rounded-full border border-white/10 bg-[#0A3640]/92 px-2 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-white shadow-sm transition-opacity backdrop-blur-sm', draggable ? 'opacity-0 group-hover:opacity-100' : 'opacity-100')}>
        <Grip className="h-3 w-3"/>
        {bannerElementLabels[elementKey]}
      </div>
      {children}
      {draggable ? (<button type="button" aria-label={`Redimensionar ${bannerElementLabels[elementKey]}`} className="absolute -right-2 top-1/2 inline-flex h-5 w-5 -translate-y-1/2 items-center justify-center rounded-full border border-[#D8E6EB] bg-white text-[#15323b] opacity-0 shadow-sm transition-opacity group-hover:opacity-100" onPointerDown={(event) => onResizePointerDown(elementKey, event)}>
...
export function AdminBannersPage() {
    ...
    const selectedBanner = useMemo(() => banners.find((banner) => banner.id === selectedBannerId) ?? null, [banners, selectedBannerId]);
    const isDirty = useMemo(() => getDraftSignature(draft) !== getDraftSignature(selectedBanner), [draft, selectedBanner]);
    async function loadBanners(preferredBannerId?: string | null, locationKey = selectedLocationKey) {
        ...
        try {
            ...
            setError(loadError instanceof Error ? loadError.message : "N?o foi possvel carregar os banners.");
        }
        finally {
            setLoading(false);
        }
    ...
    useEffect(() => {
        ...
        void fetchSiteAssets(120)
            ...
            setError(loadError instanceof Error ? loadError.message : "N?o foi possvel carregar a biblioteca de midia.");
        })
            .finally(() => {
            if (!isMounted) {
                return;
    ...
    useEffect(() => {
        ...
        void fetchSiteBannerVersions(selectedBannerId)
            ...
            setError(loadError instanceof Error ? loadError.message : "N?o foi possvel carregar o hist?rico do banner.");
            setBannerVersions([]);
        })
            .finally(() => {
            if (!isMounted) {
    ...
    function maybeChangeSelection(nextId: string) {
        ...
        if (isDirty && !window.confirm("Existem alteracoes no salvas neste banner. Deseja descartar e trocar de item")) {
            return;
        }
    ...
    async function handleDuplicateBanner() {
        ...
        try {
            ...
            setError(duplicateError instanceof Error ? duplicateError.message : "N?o foi possvel duplicar o banner.");
        }
        finally {
            setSaving(false);
        }
    ...
    async function handleDeleteBanner(targetBanner?: SiteBanner) {
        ...
        try {
            ...
            setError(deleteError instanceof Error ? deleteError.message : "N?o foi possvel excluir o banner.");
        }
        finally {
            setSaving(false);
        }
    ...
    async function handleSaveBanner() {
        ...
        if (!draft.title.trim()) {
            setError("O t?tulo do banner e obrigatrio.");
            return;
        }
        setSaving(true);
        setMessage(null);
        ...
        try {
            ...
            setError(saveError instanceof Error ? saveError.message : "N?o foi possvel salvar o banner.");
        }
        finally {
            setSaving(false);
        }
    ...
    async function handleToggleActive(banner: SiteBanner) {
        ...
        try {
            ...
            setError(toggleError instanceof Error ? toggleError.message : "N?o foi possvel alterar o status do banner.");
        }
        finally {
            setSaving(false);
        }
    ...
    async function handleMoveBanner(bannerId: string, direction: -1 | 1) {
        ...
        try {
            ...
            setError(reorderError instanceof Error ? reorderError.message : "N?o foi possvel atualizar a ordem.");
    ...
    async function handleRestoreVersion(version: SiteBannerVersion) {
        ...
        try {
            ...
            setError(restoreError instanceof Error ? restoreError.message : "N?o foi possvel rest?urar est? revis?o.");
        }
        finally {
    ...
    async function handleBackgroundUpload(variant: BannerBackgroundVariant, file: File | null) {
        ...
        try {
            ...
            setError(uploadError instanceof Error ? uploadError.message : "N?o foi possvel enviar a imagem.");
        }
        finally {
    ...
    function handleBackgroundLibraryApply(variant: BannerBackgroundVariant) {
        ...
        if (!selectedAsset || !selectedAsset.public_url) {
            setError("N?o foi possvel usar o item selecionado da biblioteca.");
            return;
        }
    ...
    async function handleCreateCarousel() {
        ...
        try {
            ...
            setError(createError instanceof Error ? createError.message : "N?o foi possvel criar o carrossel.");
        }
        finally {
            setSaving(false);
        }
    ...
    async function handleAddCarouselTarget() {
        ...
        try {
            ...
            setCarouselTargets(targets);
            setMessage("Destino de exibi??o adicionado ao carrossel.");
        }
        catch (targetError) {
            setError(targetError instanceof Error ? targetError.message : "N?o foi possvel adicionar destino ao carrossel.");
        }
        finally {
            setSaving(false);
        }
    ...
    async function handleRemoveCarouselTarget(targetId: string) {
        ...
        try {
            ...
            setError(targetError instanceof Error ? targetError.message : "N?o foi possvel remover destino do carrossel.");
        }
        finally {
            setSaving(false);
        }
    ...
    const renderVersionsList = () => (<div className="mt-4 grid gap-3">
      {loadingVersions ? (<div className="rounded-[18px] border border-dashed border-[#D8E6EB] bg-white px-4 py-4 text-sm font-semibold text-[#5F7077]">Carregando hist?rico...
        </div>) : bannerVersions.length === 0 ? (<div className="rounded-[18px] border border-dashed border-[#D8E6EB] bg-white px-4 py-4 text-sm font-semibold text-[#5F7077]">Nenhuma revis?o registrada para este banner.
        </div>) : bannerVersions.slice(0, 12).map((version, index) => (<div key={version.id} className="rounded-[18px] border border-[#D8E6EB] bg-white p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#1398B7]">Revis?o {bannerVersions.length - index} · {new Date(version.createdAt).toLocaleString('pt-BR')}
              </p>
              <p className="mt-1 text-xs font-semibold text-[#5F7077]">
                {version.changeReason} · {version.snapshot.name || 'Banner sem nome'}
              </p>
            </div>
            <Button type="button" variant="outline" disabled={restoringVersionId === version.id || saving} onClick={() => void handleRestoreVersion(version)} className="rounded-xl border-[#D8E6EB] px-3 text-[10px] font-black uppercase tracking-[0.12em]">
          ...
          <div className="mt-3 rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] px-3 py-3">
            <p className="text-sm font-black text-[#15323b]">{version.snapshot.title || "Sem t?tulo"}</p>
            {version.snapshot.subtitle ? (<p className="mt-1 text-xs font-semibold leading-5 text-[#5F7077]">{version.snapshot.subtitle}</p>) : null}
          </div>
        </div>))}
    </div>);
    const renderPreviewCanvas = (canvasRef: React.RefObject<HTMLDivElement | null>, contentRef: React.RefObject<HTMLDivElement | null>) => (<div className="w-full">
      <div ref={canvasRef} className={cn('relative overflow-hidden rounded-[32px] border border-[#D8E6EB] bg-[#0A3640] shadow-[0_24px_60px_rgba(10,54,64,0.16)] transition-all duration-300', draft?.globalLinkHref.trim() ? 'cursor-pointer' : '')} style={{
        ...
        <div ref={contentRef} className="absolute left-0 top-0 h-full" style={{
          ...
          {draft!.primaryCta?.visible ? (<BannerCanvasElement elementKey="primaryCta" item={primaryCtaLayout ?? draft!.layoutDesktop.primaryCta} verticalAlign={draft!.elementStyles.primaryCta.verticalAlign} onPointerDown={handleCanvasPointerDown} onResizePointerDown={handleCanvasResizePointerDown} draggable style={getPreviewElementStyle(primaryCtaLayout ?? draft!.layoutDesktop.primaryCta, draft!.elementStyles.primaryCta.verticalAlign)}>
    ...
    return (<div ref={pageRootRef} className={cn('space-y-7', isImmersiveMode ? 'fixed inset-0 z-[70] overflow-y-auto bg-[#F2F7F9] p-5 sm:p-6 lg:p-8' : '')}>
      {saveConfirmationOpen ? (<div className="fixed inset-0 z-[80] flex items-center justify-center bg-[#0A3640]/45 px-4">
          <div className="w-full max-w-md rounded-[24px] border border-[#D8E6EB] bg-white p-6 shadow-2xl selection:bg-transparent selection:text-[#15323b]">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-[#5F7077]">As alterações do banner foram registradas. Você pode continuar editando ou fechar est? confirmação.
            </p>
            <div className="mt-5 flex justify-end">
              <Button type="button" onClick={() => setSaveConfirmationOpen(false)} className="rounded-2xl bg-[#1398B7] px-5 font-black text-white hover:bg-[#1089A5]">
                Fechar
              </Button>
            </div>
          </div>
        </div>) : null}
      {isPreviewModalOpen && draft ? (<div className="fixed inset-0 z-[85] bg-[#0A3640]/80">
      ...
      {isVersionsModalOpen && draft ? (<div className="fixed inset-0 z-[88] flex items-center justify-center bg-[#0A3640]/60 px-4 py-6">
          <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-[24px] border border-[#D8E6EB] bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#D8E6EB] px-5 py-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#1398B7]">Hist?rico de revis?es</p>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto bg-[#F8FBFC] p-5">
              <p className="text-xs font-semibold leading-5 text-[#5F7077]">Cada salvamento relevante do banner gera uma revis?o. Clique na desejada para reverter e criar uma nova entrada no hist?rico.
              </p>
              {renderVersionsList()}
            </div>
          </div>
        </div>) : null}
      {isLibraryModalOpen ? (<div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#0A3640]/50 px-4 py-6">
      ...
      <header className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          {isImmersiveMode ? (<Button type="button" variant="outline" onClick={() => void handleExitImmersiveMode()} className="rounded-2xl border-[#D8E6EB]">
      ...
      {error ? (<div className="rounded-[22px] border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-semibold text-rose-700">
          {error}
        </div>) : null}

      {message ? (<div className="rounded-[22px] border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm font-semibold text-emerald-700">
          {message}
        </div>) : null}
      ...
      <section className="space-y-6">
        <aside className="space-y-4">
          <article className="rounded-[28px] border border-[#D8E6EB] bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {loading ? (<p className="text-sm font-semibold text-[#5F7077]">Carregando banners...</p>) : banners.length === 0 ? (<div className="rounded-[22px] border border-dashed border-[#D8E6EB] bg-[#F8FBFC] px-4 py-6 text-center md:col-span-2 xl:col-span-4">
                ...
                </div>) : banners.map((banner, index) => {
            const isSelected = banner.id === selectedBannerId;
            return (<article key={banner.id} className={cn('relative h-full rounded-[24px] border p-4 transition-all', isSelected ? 'border-[#1398B7] bg-[#E8F6FA]' : 'border-[#D8E6EB] bg-[#F8FBFC] hover:border-[#B8D8E1]')}>
                    <button type="button" aria-label={`Excluir ${banner.name}`} onClick={(event) => {
                    event.stopPropagation();
                    void handleDeleteBanner(banner);
                }} disabled={saving} className="absolute right-3 top-3 inline-flex h-8 w-8 items-center justify-center rounded-full border border-rose-200 bg-white text-rose-600 transition-colors hover:bg-rose-50 hover:text-rose-700 disabled:opacity-50">
                      <X className="h-4 w-4"/>
                    </button>
                    <button type="button" onClick={() => maybeChangeSelection(banner.id)} className="w-full text-left">
                      <div className="flex items-start justify-between gap-3 pr-10">
                        ...
                        <span className={cn('rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em]', banner.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600')}>
                    ...
                    <div className="mt-4 flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button type="button" onClick={() => void handleToggleActive(banner)} disabled={saving} className={cn('inline-flex h-8 items-center justify-center gap-1 rounded-xl border px-3 text-[11px] font-black uppercase tracking-[0.12em]', banner.isActive ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-[#D8E6EB] bg-white text-[#5F7077]')}>
            ...
            <div className="mt-6 rounded-[20px] border border-[#D8E6EB] bg-white p-4">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {carouselTargets.length === 0 ? (<span className="text-xs font-semibold text-[#5F7077]">Nenhum destino configurado para este carrossel.</span>) : carouselTargets.map((target) => (<span key={target.id} className="inline-flex items-center gap-2 rounded-full border border-[#D8E6EB] bg-[#F8FBFC] px-3 py-1 text-xs font-black uppercase tracking-[0.1em] text-[#15323b]">
        ...
        <section className="space-y-6">
          {!draft ? (<div className="rounded-[28px] border border-dashed border-[#D8E6EB] bg-white px-6 py-12 text-center text-sm font-semibold text-[#5F7077] shadow-sm">
              Selecione um banner na lista ou crie um novo para editar.
            </div>) : (<article className="min-w-0 rounded-[28px] border border-[#D8E6EB] bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 grid gap-6 xl:grid-cols-12 xl:items-start">
                  ...
                  <div className="rounded-[28px] border border-[#D8E6EB] bg-[#F8FBFC] p-4 xl:sticky xl:top-24">
                    <div className="mb-4 flex items-center justify-between gap-3">
                      ...
                      <div className="inline-flex items-center rounded-2xl border border-[#D8E6EB] bg-white p-1 shadow-sm">
                        <button type="button" onClick={() => setPreviewMode('desktop')} className={cn('inline-flex h-10 items-center gap-2 rounded-[14px] px-3 text-xs font-black uppercase tracking-[0.14em] transition-colors', previewMode === 'desktop' ? 'bg-[#1398B7] text-white' : 'text-[#5F7077] hover:text-[#15323b]')}>
                          <Monitor className="h-4 w-4"/>
                          Desktop
                        </button>
                        <button type="button" onClick={() => setPreviewMode('mobile')} className={cn('inline-flex h-10 items-center gap-2 rounded-[14px] px-3 text-xs font-black uppercase tracking-[0.14em] transition-colors', previewMode === 'mobile' ? 'bg-[#1398B7] text-white' : 'text-[#5F7077] hover:text-[#15323b]')}>
                  ...
                  <div className="grid gap-6 xl:max-h-[calc(100vh-9rem)] xl:overflow-y-auto xl:pr-2">
                    ...
                    <div className="grid gap-4">
                      <div className="rounded-[18px] border border-[#D8E6EB] bg-white p-4">
                        ...
                        <p className="mt-2 text-xs font-semibold text-[#7C8E94]">Use uma arte para desktop e outra para mobile. Se o mobile n?o tiver imagem propria, ele herda a do desktop.</p>
                ...
                },
                {
                    ...
                    value: draft.backgroundUrlMobile || draft.backgroundUrl,
            ...
            ]).map((field) => (<div key={field.variant} className="min-w-0 rounded-[18px] border border-[#D8E6EB] bg-[#F8FBFC] p-3">
                              ...
                              <label className="mt-3 inline-flex h-11 w-full cursor-pointer items-center justify-center rounded-2xl bg-[#1398B7] px-3 text-center text-[11px] font-black uppercase tracking-[0.08em] text-white hover:bg-[#1089A5]">
                                ...
                                <input type="file" accept=".jpg,.jpeg,.png,.webp,image/*" disabled={uploadingBackground !== null} onChange={(event) => {
                    const file = event.target.files?.[0] ?? null;
                    void handleBackgroundUpload(field.variant, file);
                    event.currentTarget.value = '';
                }} className="sr-only"/>
                              </label>
                              <Button type="button" variant="outline" onClick={() => handleOpenLibraryModal(field.variant)} disabled={loadingLibraryAssets || uploadingBackground !== null} className="mt-2 h-10 w-full rounded-2xl border-[#D8E6EB] px-3 text-[11px] font-black uppercase tracking-[0.08em] whitespace-normal leading-tight">
                                Abrir biblioteca de midia
                              </Button>
                              <Button type="button" variant="outline" onClick={() => handleRemoveBackgroundImage(field.variant)} disabled={uploadingBackground !== null || !field.value} className="mt-2 h-10 w-full rounded-2xl border-[#F0C8CE] bg-white px-3 text-[11px] font-black uppercase tracking-[0.08em] text-[#A63A4B] hover:bg-[#FFF4F6] disabled:cursor-not-allowed disabled:opacity-60">
                                Remover imagem
                              </Button>
                              <div className="mt-3 grid min-w-0 gap-2">
                                <label className="grid min-w-0 gap-1">
                                  <span className="text-[10px] font-black uppercase tracking-[0.12em] text-[#5F7077]">Posicao</span>
                                  <select value={field.variant === 'desktop' ? draft.backgroundDesktop.position : draft.backgroundMobile.position} onChange={(event) => setDraft((current) => current ? ({
                    ...current,
                    ...
                    backgroundMobile: field.variant === 'mobile'
                        ...
                        : current.backgroundMobile,
                }) : current)} className="h-10 w-full min-w-0 rounded-[14px] border border-[#D8E6EB] bg-white px-3 text-xs font-semibold text-[#15323b] outline-none">
                                ...
                                <label className="grid min-w-0 gap-1">
                                  ...
                                  <select value={field.variant === 'desktop' ? draft.backgroundDesktop.size : draft.backgroundMobile.size} onChange={(event) => setDraft((current) => current ? ({
                    ...current,
                    backgroundDesktop: field.variant === 'desktop'
                    ...
                    backgroundMobile: field.variant === 'mobile'
                        ...
                        : current.backgroundMobile,
                                ...
                                <label className="grid min-w-0 gap-1">
                                  ...
                                  <select value={field.variant === 'desktop' ? draft.backgroundDesktop.repeat : draft.backgroundMobile.repeat} onChange={(event) => setDraft((current) => current ? ({
                    ...current,
                    backgroundDesktop: field.variant === 'desktop'
                    ...
                    backgroundMobile: field.variant === 'mobile'
                      ...
                      <label className="grid gap-2">
                        ...
                        <textarea value={draft.body} onChange={(event) => setDraftField('body', event.target.value)} rows={4} className="rounded-[16px] border border-[#D8E6EB] bg-white px-4 py-3 text-sm font-semibold text-[#15323b] outline-none"/>
                      </label>

                      <label className="grid gap-2">
                        <span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#5F7077]">Preset visual</span>
                        <select value={draft.themePreset} onChange={(event) => setDraftField('themePreset', event.target.value as SiteBanner['themePreset'])} className="h-11 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none">
                          {bannerThemePresetOptions.map((option) => (<option key={option.value} value={option.value}>{option.label}</option>))}
                        </select>
                      </label>
                      ...
                      <div className="rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] p-3">
                        ...
                        <div className="mt-3 grid gap-3">
                          <label className="grid gap-2">
                            ...
                            <input value={draft.globalLinkHref} onChange={(event) => setDraftField('globalLinkHref', event.target.value)} placeholder="/cursos ou https://exemplo.com" className="h-11 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none"/>
                          </label>
                          <label className="grid gap-2">
                            <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Tipo do link global</span>
                            <select value={draft.globalLinkIsInternal ? 'internal' : 'external'} onChange={(event) => setDraft((current) => current ? ({
                ...current,
                globalLinkIsInternal: event.target.value === 'internal',
                          ...
                          <label className="grid gap-2">
                            ...
                            <select value={draft.globalLinkTarget} onChange={(event) => setDraft((current) => current ? ({
                ...current,
                      ...
                      <div className="rounded-[18px] border border-[#D8E6EB] bg-white p-3">
                        ...
                        <div className="mt-3 grid gap-3 sm:grid-cols-2">
                          <label className="grid gap-2">
                            ...
                            <input type="text" inputMode="numeric" min={320} max={1200} step={1} value={heightDesktopInput} onChange={(event) => setHeightDesktopInput(event.target.value)} onBlur={() => commitBannerHeight('heightDesktop', heightDesktopInput)} onKeyDown={(event) => {
                if (event.key === 'Enter') {
                    event.currentTarget.blur();
                          ...
                          <label className="grid gap-2">
                            ...
                            <input type="text" inputMode="numeric" min={320} max={1200} step={1} value={heightMobileInput} onChange={(event) => setHeightMobileInput(event.target.value)} onBlur={() => commitBannerHeight('heightMobile', heightMobileInput)} onKeyDown={(event) => {
                if (event.key === 'Enter') {
                    event.currentTarget.blur();
                }
            }} className="h-11 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none"/>
                ...
                return (<CollapsibleCard key={layoutKey} title={bannerElementLabels[layoutKey]} summary={`${item.width}% de largura • camada ${item.zIndex} • ${item.visible ? 'visivel' : 'oculto'}`} open={expandedCards[layoutCardKey]} onToggle={() => toggleCard(layoutCardKey)} className="min-w-0 overflow-hidden rounded-[18px] bg-white p-3" bodyClassName="grid min-w-0 gap-2.5">
                            <LayoutControls item={item} labelPrefix={activeLayoutLabel} onVisibleChange={(visible) => setLayoutItem(layoutKey, (current) => ({ ...current, visible }))} minWidth={getMinWidthForLayoutKey(layoutKey)} onWidthChange={(width) => setLayoutItem(layoutKey, (current) => applyWidthWithinCanvas(current, width, getMinWidthForLayoutKey(layoutKey)))} onZIndexChange={(nextValue) => setLayoutItem(layoutKey, (current) => ({ ...current, zIndex: nextValue }))}/>

                            <ColorField label="Cor do texto" value={draft.elementStyles[layoutKey].textColor} fallback={defaultTextColor} onChange={(value) => setElementStyle(layoutKey, (current) => ({ ...current, textColor: value }))}/>
                            <label className="grid gap-2">
                              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Fonte (font-family)</span>
                              <input value={draft.elementStyles[layoutKey].fontFamily ?? ''} onChange={(event) => setElementStyle(layoutKey, (current) => ({ ...current, fontFamily: event.target.value.trim() || undefined }))} placeholder="Ex.: var(--font-readex), Readex Pro" className="h-11 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none"/>
                            </label>
                            <div className="grid gap-2 sm:grid-cols-2">
                              <label className="grid gap-2">
                                <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Tamanho (px)</span>
                                <input type="number" min={10} max={120} step={1} value={draft.elementStyles[layoutKey].fontSize ?? ''} onChange={(event) => {
                        ...
                        setElementStyle(layoutKey, (current) => ({ ...current, fontSize: Number.isFinite(raw) && raw > 0 ? raw : undefined }));
                              ...
                              <label className="grid gap-2">
                                ...
                                <input type="number" min={100} max={900} step={100} value={draft.elementStyles[layoutKey].fontWeight ?? ''} onChange={(event) => {
                        const raw = Number(event.target.value);
                        setElementStyle(layoutKey, (current) => ({ ...current, fontWeight: Number.isFinite(raw) && raw >= 100 ? raw : undefined }));
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                              <label className="grid gap-2">
                                ...
                                <input type="number" min={-10} max={30} step={0.1} value={draft.elementStyles[layoutKey].letterSpacing ?? ''} onChange={(event) => {
                        const raw = Number(event.target.value);
                        setElementStyle(layoutKey, (current) => ({ ...current, letterSpacing: Number.isFinite(raw) ? raw : undefined }));
                              ...
                              <label className="grid gap-2">
                                ...
                                <select value={draft.elementStyles[layoutKey].textAlign ?? ''} onChange={(event) => {
                        ...
                        setElementStyle(layoutKey, (current) => ({ ...current, textAlign: value || undefined }));
                            ...
                            <div className="grid gap-2 sm:grid-cols-2">
                              <label className="grid gap-2">
                                ...
                                <select value={draft.elementStyles[layoutKey].verticalAlign ?? ''} onChange={(event) => {
                        ...
                        setElementStyle(layoutKey, (current) => ({ ...current, letterSpacing: Number.isFinite(raw) ? raw : undefined }));
                            ...
                            <button type="button" onClick={() => setElementStyle(layoutKey, (current) => ({ ...current, textColor: undefined }))} className="inline-flex h-9 w-full min-w-0 items-center justify-center rounded-xl border border-[#D8E6EB] bg-white px-3 text-center text-xs font-black uppercase tracking-[0.12em] text-[#5F7077] hover:bg-[#F2F7F9]">
                              Usar cor do preset
                            </button>
                            <button type="button" onClick={() => setElementStyle(layoutKey, (current) => ({
                        ...current,
                ...
                return (<CollapsibleCard key={key} title={title} summary={`Defina texto, destino, cores e posicao no layout ${activeLayoutLabel}.`} open={expandedCards[ctaCardKey]} onToggle={() => toggleCard(ctaCardKey)}>
                          <div className="flex flex-col items-start gap-3">
                            <label className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] text-[#5F7077]">
                              <input type="checkbox" checked={cta?.visible ?? false} onChange={(event) => {
                        ...
                        setLayoutItem(key, (current) => ({ ...current, visible: checked }));
                          ...
                          <div className="mt-4 grid gap-3">
                            <LayoutControls item={layoutItem} labelPrefix={activeLayoutLabel} onVisibleChange={(visible) => {
                    ...
                    } as SiteBannerCta))} className="h-11 rounded-[16px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none"/>
                            </label>

                            <label className="grid gap-2">
                              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Link</span>
                              <input value={cta?.href ?? ''} onChange={(event) => setCta(key, (current) => ({
                        ...(current ?? {
                            label: title,
                            isInternal: true,
                            openInNewTab: false,
                        ...
                        href: event.target.value,
                            ...
                            <label className="grid gap-2">
                              ...
                              <select value={cta?.tonePreset ?? (key === 'primaryCta' ? 'solid' : 'surface')} onChange={(event) => setCta(key, (current) => ({
                        ...(current ?? {
                            label: title,
                            href: '#',
                            isInternal: true,
                        ...
                        tonePreset: event.target.value as SiteBannerCta['tonePreset'],
                                ...
                                {bannerTonePresetOptions.map((option) => (<option key={option.value} value={option.value}>{option.label}</option>))}
                              </select>
                            </label>

                            <label className="grid gap-2">
                              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Tipo de link</span>
                              <select value={cta?.isInternal === false ? 'external' : 'internal'} onChange={(event) => setCta(key, (current) => ({
                        ...(current ?? {
                            label: title,
                            href: '#',
                            openInNewTab: false,
                        ...
                        openInNewTab: event.target.value === 'external',
                            ...
                            <ColorField label="Cor do botao" value={draft.elementStyles[key].backgroundColor} fallback={toneDefaults.backgroundColor} onChange={(value) => setElementStyle(key, (current) => ({ ...current, backgroundColor: value }))}/>

                            <ColorField label="Cor do texto" value={draft.elementStyles[key].textColor} fallback={toneDefaults.textColor} onChange={(value) => setElementStyle(key, (current) => ({ ...current, textColor: value }))}/>

                            <button type="button" onClick={() => setElementStyle(key, () => ({}))} className="inline-flex h-9 items-center justify-center rounded-xl border border-[#D8E6EB] bg-white px-3 text-xs font-black uppercase tracking-[0.12em] text-[#5F7077] hover:bg-[#F2F7F9]">
