...
import { fetchAdminPaymentInvoiceUrl, fetchCommerceDashboardSummaries, fetchPaymentGatewayDiagnostics, fetchPaymentGatewaySettings, updatePaymentGatewaySettings, type CommerceCheckoutSessionSummary, type CommerceDashboardMetrics, type CommerceEventSummary, type PaymentDiagnosticCheck, type PaymentGatewayDiagnostics, type PaymentGatewayEnvironment, } from '@/features/admin/commerce/api';
...
export function AdminPaymentSettingsPage() {
    ...
    const runDiagnostics = useCallback(async () => {
        ...
        try {
            ...
            setDiagnosticError(diagnosticsError instanceof Error
                ? diagnosticsError.message
                : "N?o foi possvel diagnosticar as configura??es de pagamento.");
        }
        finally {
    ...
    const loadSettings = useCallback(async () => {
        ...
        try {
            ...
            setError(loadError instanceof Error ? loadError.message : "N?o foi possvel carregar as configura??es.");
        }
        finally {
            setIsLoading(false);
        }
    ...
    async function handleSave(nextEnvironment: PaymentGatewayEnvironment) {
        ...
        try {
            ...
            setError(saveError instanceof Error ? saveError.message : "N?o foi possvel salvar as configura??es.");
        }
        finally {
            setIsSaving(false);
        }
    ...
    async function openInvoice(session: CommerceCheckoutSessionSummary) {
        ...
        try {
            ...
            setError(openError instanceof Error ? openError.message : "N?o foi possvel abrir a fatura do pedido.");
        }
        finally {
    ...
    return (<div className="animate-in space-y-6 fade-in duration-500">
      ...
      {error ? (<div className="rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-semibold text-rose-700">
          {error}
        </div>) : null}
      ...
      <section className="space-y-6 rounded-[28px] border border-[#C6D9E6] bg-[#F8FCFF] p-6 md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <Button type="button" onClick={() => {
            void loadSettings();
            void runDiagnostics();
        }} disabled={isLoading || isDiagnosing} variant="outline" className="h-12 rounded-full border-[#B7CFDF] bg-[#E9F4FB] px-6 text-base font-semibold text-[#0A2A49]">
        ...
        <div className="flex flex-wrap gap-3 border-b border-[#C6D9E6] pb-5">
          {tabs.map((tab) => {
            ...
            return (<button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)} className={`rounded-full border px-6 py-2.5 text-lg font-semibold transition-all ${isActive
                    ? 'border-[#031437] bg-[#031437] text-white'
                    : 'border-[#B7CFDF] bg-white text-[#1A3354] hover:border-[#7EA8C6]'}`} title={tab.description}>
                {tab.label}
              </button>);
        })}
        </div>
        ...
        {activeTab === 'history' ? (<>
            <section className="grid gap-4 md:grid-cols-3">
              {metricCards.map((card) => (<article key={card.label} className="rounded-3xl border border-[#B7CFDF] bg-white p-6">
            ...
            <section className="space-y-4 rounded-3xl border border-[#B7CFDF] bg-[#F4FAFF] p-5">
              ...
              <div className="flex flex-wrap gap-3">
                <input type="text" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Buscar por cliente, item, referencia ou status..." className="h-12 flex-1 rounded-2xl border border-[#C6D9E6] bg-white px-4 text-base text-[#1A3354] placeholder:text-[#70879D] focus:border-[#7EA8C6] focus:outline-none"/>
            ...
            ].map((filter) => (<button key={filter.key} type="button" onClick={() => setStatusFilter(filter.key)} className={`h-12 rounded-full border px-5 text-lg font-semibold transition-colors ${statusFilter === filter.key
                    ? 'border-[#031437] bg-[#031437] text-white'
              ...
              <table className="min-w-full divide-y divide-[#D7E5EF]">
                ...
                <tbody className="divide-y divide-[#E1ECF3]">
                  {filteredSessions.length === 0 ? (<tr>
                      <td colSpan={6} className="px-5 py-8 text-center text-sm font-semibold text-[#5F7077]">
                    ...
                    </tr>) : (filteredSessions.map((session) => (<tr key={session.id} className="align-top">
                        ...
                        <td className="px-5 py-4">
                          <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-black uppercase tracking-widest ${paymentStatusClassName(normalizePaymentStatus(session.status).tone)}`}>
                        ...
                        <td className="px-5 py-4">
                          <Button type="button" variant="outline" onClick={() => void openInvoice(session)} disabled={openingInvoiceId === session.id} className="h-10 rounded-full border-[#B7CFDF] px-4 font-semibold text-[#0A2A49] disabled:opacity-60">
        ...
        {activeTab === 'events' ? (<section className="space-y-4">
            <article className="rounded-3xl border border-[#B7CFDF] bg-white p-6">
              ...
              <div className="mt-4 space-y-3">
                {recentEvents.length === 0 ? (<p className="text-sm font-medium text-[#5F7077]">Nenhum evento recebido ainda.</p>) : (recentEvents.map((event) => (<div key={event.id} className="rounded-2xl border border-[#D8E6EB] bg-[#F4FAFF] px-4 py-3">
                      <p className="text-base font-bold text-[#021B41]">{event.event_type}</p>
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-widest ${paymentStatusClassName(normalizePaymentStatus(event.status).tone)}`}>
        ...
        {activeTab === 'settings' ? (<section className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
            <div className="space-y-5">
              <article className="rounded-3xl border border-[#D8E6EB] bg-white p-6 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 font-readex text-xl font-semibold tracking-tight text-[#15323b]">Configurao operacional
                    </h3>
                  ...
                  <Button type="button" onClick={() => void runDiagnostics()} disabled={isDiagnosing} variant="outline" className="rounded-full border-[#D8E6EB] font-black text-[#15323b]">
                    <RefreshCw className={`mr-2 h-4 w-4 ${isDiagnosing ? 'animate-spin' : ''}`}/>
                    Verificar
                  </Button>
                </div>

                {diagnosticError ? (<div className="mt-5 border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">
                    {diagnosticError}
                  </div>) : null}
                ...
                return (<div key={check.key} className={`border px-4 py-4 ${tone.className}`}>
              ...
              <article className="rounded-3xl border border-[#D8E6EB] bg-white p-6 shadow-sm">
                ...
                <h3 className="mt-2 font-readex text-xl font-semibold tracking-tight text-[#15323b]">O que precisa est?r configurado
                </h3>
                <div className="mt-5 space-y-2 text-sm leading-7 text-[#5F7077]">
                  <p>1. Configure a chave de API do Asaas para produo e sandbox nas variaveis de ambiente do deploy.</p>
                  <p>2. Aponte o webhook do Asaas para a URL exibida nest? tela.</p>
                  <p>3. Se usar segredo no webhook, cadastre o mesmo valor no Asaas e em `ASAAS_WEBHOOK_SECRET`.</p>
                  <p>4. Escolha o ambiente ativo abaixo e salve a configura??o.</p>
                  <p>5. Quando o pagamento for confirmado, o acesso ao curso sera liberado automticamente.</p>
                </div>
              </article>

              <article className="rounded-3xl border border-[#D8E6EB] bg-white p-6 shadow-sm">
                <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#5F7077]">Ambiente</p>
                <h3 className="mt-2 font-readex text-xl font-semibold tracking-tight text-[#15323b]">Trocar entre sandbox e produo
                </h3>
                <div className="mt-5 flex flex-wrap gap-3">
                  {(['sandbox', 'production'] as const).map((option) => {
                const isActive = environment === option;
                return (<button key={option} type="button" onClick={() => void handleSave(option)} disabled={isSaving || isLoading} className={`rounded-full border px-5 py-3 text-sm font-black transition-all ${isActive
            ...
            <div className="space-y-5">
              <article className="rounded-3xl border border-[#D8E6EB] bg-white p-6 shadow-sm">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <Button type="button" onClick={() => void copyWebhook()} className="rounded-full bg-gradient-to-b from-[#1398B7] to-[#0A3640] font-black text-white hover:opacity-95">
              ...
              <article className="rounded-3xl border border-[#D8E6EB] bg-white p-6 shadow-sm">
                ...
                <ul className="mt-4 space-y-2 text-sm leading-7 text-[#5F7077]">
                  ...
                  <li><span className="font-black text-[#15323b]">ASAAS_ACCESS_TOKEN_PRODUCTION</span>para produo.</li>
