...
import { COURSE_QUIZ_TYPE_OPTIONS, DEFAULT_COURSE_QUIZ_TYPE_SETTINGS, normalizeCourseQuizTypeSettings, } from '@/features/assessments/course-quiz-type-settings';
import { fetchGlobalQuizTypeSettings, updateGlobalQuizTypeSettings, } from '@/features/admin/quiz-types/api';
...
export function AdminQuizTypesPage() {
    ...
    async function handleSave() {
        ...
        setSuccess(false);
        if (!user) {
            setError("Sesso expirada. Fa\u00E7a login novamente.");
            return;
        }
        setIsSaving(true);
        try {
    ...
    return (<div className="space-y-8 animate-in fade-in duration-500">
      ...
      {error ? (<div className="rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm font-semibold text-rose-700">
          {error}
        </div>) : null}

      {success ? (<div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm font-semibold text-emerald-700">
      ...
      <section className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-5">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-slate-500">Use est? tela para definir a disponibilidade base da plataforma. As configurações do curso continuam existindo,
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {COURSE_QUIZ_TYPE_OPTIONS.map((option) => {
            ...
            return (<label key={option.key} className={`block rounded-[24px] border p-5 shadow-sm transition-all ${isEnabled ? option.accentClassName : 'border-slate-200 bg-slate-50/80 text-slate-500'}`}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <input type="checkbox" checked={isEnabled} onChange={(event) => setSettings((current) => ({
                    ...current,
        ...
        <div className="mt-8 flex justify-end border-t border-slate-100 pt-6">
          <Button type="button" onClick={() => void handleSave()} disabled={isSaving || isLoading} className="h-14 rounded-2xl bg-slate-900 px-10 font-black shadow-xl shadow-slate-100 hover:bg-slate-800">
