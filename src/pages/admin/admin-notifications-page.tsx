...
import { fetchNotificationAdminSettings, fetchNotificationStats, fetchRecentQueueItems, fetchPlatformNotifications, processNotificationQueue, saveNotificationAdminSettings, sendBroadcastNotification, type BroadcastNotificationResult, type NotificationAdminSettings, type NotificationChannel, type NotificationPriority, type NotificationQueueItem, type PlatformNotification, } from '@/features/notifications/api';
...
function getTabButtonClassName(isActive: boolean) {
    return cn('rounded-full border px-4 py-2 text-xs font-black uppercase tracking-[0.2em] transition', isActive
...
export function AdminNotificationsPage() {
    ...
    const activeTab: AdminNotificationsTab = requestedTab === 'plataforma' || requestedTab === 'operacoes' ? requestedTab : 'manuais';
    ...
    function renderTabButton(tab: AdminNotificationsTab, label: string) {
        ...
        return (<button key={tab} type="button" onClick={() => setSearchParams((current) => {
                ...
                return next;
            }, { replace: true })} className={getTabButtonClassName(isActive)}>
        {label}
      </button>);
    }
    ...
    const selectedChannelLabels = useMemo(() => channelOptions
    ...
    async function handleSaveAdminSettings(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            setSuccessMessage(savedSettings.admin_notification_email
    ...
    async function handleSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            setSuccessMessage(`N?otifica\u00E7\u00E3o enviada para ${result.recipient_count} usuário(s), com ${result.queued_count} entrega(s) registradas.`);
            setTitle('');
            setBody('');
            setActionUrl('');
            await loadDashboard();
    ...
    async function handleProcessQueue() {
        ...
        try {
            ...
            setSuccessMessage(`Fila processada: ${result.processed} item(ns), ${result.sent} enviado(s), ${result.retrying} em nova tentativa, ${result.failed} falha(s), ${result.ignored} ignorado(s).`);
            await loadDashboard();
        }
        catch (error) {
    ...
    return (<div className="space-y-7">
      <header className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 font-readex text-3xl font-semibold tracking-tight text-[#15323b]">N?otificações</h1>
          <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-[#6d7f84]">
            ...
            : activeTab === 'operacoes'
                ...
                : "Envie avisos para usu\u00E1rios e acompanhe apenas os envios manuais registrados nest? \u00E1rea."}
          </p>
        ...
        <div className="flex flex-wrap gap-3">
          {activeTab === 'manuais' ? (<Button type="button" variant="outline" onClick={() => void handleProcessQueue()} disabled={isProcessingQueue} className="h-11 rounded-none border-[#1398B7] bg-white font-black text-[#0A3640] hover:bg-[#E8F6FA]">
              <Send className="mr-2 h-4 w-4"/>
              {isProcessingQueue ? 'Processando...' : 'Processar fila'}
            </Button>) : null}
          {activeTab === 'operacoes' ? (<Link to="/admin/pendencias" className="inline-flex h-11 items-center justify-center rounded-none border border-[#D8E6EB] bg-white px-4 font-black text-[#0A3640] transition hover:border-[#1398B7] hover:bg-[#F2F7F9]">
      ...
      {errorMessage ? (<div className="border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700">
          {errorMessage}
        </div>) : null}

      {successMessage ? (<div className="border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-700">
          {successMessage}
        </div>) : null}
      ...
      {activeTab === 'manuais' ? (<>
          ...
          <section className="border border-[#D8E6EB] bg-white p-6 shadow-[0_18px_42px_rgba(10,54,64,0.05)]">
            ...
            <form onSubmit={(event) => void handleSaveAdminSettings(event)} className="mt-5 flex flex-col gap-3 lg:flex-row lg:items-end">
              <label className="flex-1">
                ...
                <input type="email" value={adminNotificationEmail} onChange={(event) => setAdminNotificationEmail(event.target.value)} className="mt-2 h-12 w-full border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="ex.: operacao@genflix.com.br"/>
              </label>

              <Button type="submit" disabled={isLoading || isSavingSettings} className="h-12 rounded-none bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-6 font-black text-white hover:opacity-95">
          ...
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {[
                { label: 'Total manual', value: totalNotifications, helper: "N?otifica\u00E7\u00F5es enviadas manualmente" },
          ...
          <section className="grid gap-6 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
            <form onSubmit={(event) => void handleSubmit(event)} className="border border-[#D8E6EB] bg-white p-6 shadow-[0_18px_42px_rgba(10,54,64,0.05)]">
              ...
              <div className="mt-5 grid gap-4">
                ...
                <label className="grid gap-2">
                  ...
                  <textarea value={body} onChange={(event) => setBody(event.target.value)} required rows={5} className="resize-none border border-[#D8E6EB] bg-white px-4 py-3 text-sm font-semibold leading-7 text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Escreva uma mensagem curta e clara..."/>
                </label>

                <div className="grid gap-4 md:grid-cols-2">
                  <label className="grid gap-2">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#5F7077]">Categoria</span>
                    <input value={category} onChange={(event) => setCategory(event.target.value)} className="h-12 border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="course, payment, system..."/>
                  </label>

                  <label className="grid gap-2">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#5F7077]">Prioridade</span>
                    <select value={priority} onChange={(event) => setPriority(event.target.value as NotificationPriority)} className="h-12 border border-[#D8E6EB] bg-white px-4 text-sm font-black text-[#15323b] outline-none focus:border-[#1398B7]">
                ...
                <label className="grid gap-2">
                  ...
                  <input value={actionUrl} onChange={(event) => setActionUrl(event.target.value)} className="h-12 border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="/aluno/cursos ou https://..."/>
                </label>

                <div className="grid gap-3">
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#5F7077]">Público</span>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {roleOptions.map((role) => (<label key={role.value} className="flex cursor-pointer items-center gap-3 border border-[#D8E6EB] bg-[#F2F7F9] px-4 py-3">
                        <input type="checkbox" checked={selectedRoles.includes(role.value)} onChange={() => toggleRole(role.value)} className="h-4 w-4 accent-[#1398B7]"/>
                ...
                return (<label key={channel.value} className={cn('flex cursor-pointer items-center gap-3 border px-4 py-3 transition', isSelected
                        ? 'border-[#1398B7] bg-[#E8F6FA] text-[#0A3640]'
                        : 'border-[#D8E6EB] bg-white text-[#5F7077]')}>
                          <input type="checkbox" checked={isSelected} onChange={() => toggleChannel(channel.value)} className="h-4 w-4 accent-[#1398B7]"/>
              ...
              <div className="mt-6 border-t border-[#D8E6EB] pt-5">
                <Button type="submit" disabled={isSending} className="h-12 w-full rounded-none bg-[linear-gradient(180deg,#1398B7_0%,#0A3640_100%)] font-readex text-sm font-semibold text-white hover:opacity-95">
                  <Send className="mr-2 h-4 w-4"/>
            ...
            <section className="border border-[#D8E6EB] bg-white p-6 shadow-[0_18px_42px_rgba(10,54,64,0.05)]">
              ...
              <div className="mt-5 overflow-hidden border border-[#D8E6EB]">
                {isLoading ? (<p className="p-5 text-sm font-semibold text-[#6d7f84]">Carregando fila...</p>) : queueItems.length === 0 ? (<p className="p-5 text-sm font-semibold text-[#6d7f84]">Nenhuma entrega registrada.</p>) : (<div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-[#D8E6EB] text-left text-sm">
                      ...
                      <tbody className="divide-y divide-[#D8E6EB]">
                        {queueItems.map((item) => (<tr key={item.id} className="align-top">
          ...
          <div className="border border-[#D8E6EB] bg-[#F2F7F9] p-4 text-sm font-semibold leading-6 text-[#15323b]">Esta aba consolida o pulso operacional da plataforma: fila de e-mails, reprocessamentos, pontos de falha
            e atalhos para os fluxos críticos que precisam est?r saudáveis no dia a dia.
          </div>
          ...
          <section className="space-y-6">
            <article className="w-full border border-[#D8E6EB] bg-white p-6 shadow-[0_18px_42px_rgba(10,54,64,0.05)]">
              <div className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-5 md:flex-row md:items-end md:justify-between">
                ...
                <span className={cn('inline-flex items-center gap-2 border px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em]', operationsSummary.health.className)}>
              ...
              <div className="mt-5 overflow-hidden border border-[#D8E6EB]">
                {isLoading ? (<p className="p-5 text-sm font-semibold text-[#6d7f84]">Carregando operação...</p>) : operationsQueueItems.length === 0 ? (<p className="p-5 text-sm font-semibold text-[#6d7f84]">Nenhuma atividade operacional recente encontrada.</p>) : (<div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-[#D8E6EB] text-left text-sm">
                      <thead className="bg-[#F2F7F9] text-[10px] font-black uppercase tracking-[0.2em] text-[#5F7077]">
                        <tr>
                          <th className="px-4 py-3">Canal</th>
                      ...
                      <tbody className="divide-y divide-[#D8E6EB]">
                        {operationsQueueItems.map((item) => (<tr key={item.id} className="align-top">
                            <td className="px-4 py-3 text-xs font-black uppercase text-[#0A3640]">{item.channel}</td>
                            <td className="px-4 py-3">
                              <span className={cn('inline-flex border px-2 py-1 text-[10px] font-black uppercase tracking-[0.16em]', getStatusClassName(item.status))}>
                                {item.status}
              ...
              {[
                {
                    ...
                    description: "Use est? \u00E1rea para acompanhar os processos que alimentam a opera\u00E7\u00E3o, como envio autom\u00E1tico, rotinas agendadas e reprocessamentos.",
                    status: 'Vinculado',
                ...
                },
                {
                    ...
                    description: 'Quando um processo crítico não for da fila de notificação, verifique o ciclo financeiro e os eventos recebidos do gateway.',
                    status: 'Monitorar',
                ...
                },
                {
                    ...
                    description: 'Repasses e sincronizações em segundo plano continuam como ponto de atenção operacional até a rotina ficar totalmente estável.',
                    status: 'Acompanhar',
                    tone: 'border-[#BEE3EA] bg-[#E8F6FA] text-[#0A3640]',
                    actionLabel: 'Abrir repasses',
                    actionTo: '/admin/repasses',
                },
            ].map((item) => (<article key={item.title} className="border border-[#D8E6EB] bg-white p-5 shadow-[0_14px_34px_rgba(10,54,64,0.05)]">
                  ...
                  <Link to={item.actionTo} className="mt-4 inline-flex items-center gap-2 text-sm font-black text-[#1398B7] underline decoration-[#1398B7]/40 underline-offset-4">
                    {item.actionLabel}
          ...
          <section className="border border-[#D8E6EB] bg-white p-6 shadow-[0_18px_42px_rgba(10,54,64,0.05)]">
            <div className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-5 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="mt-2 font-readex text-2xl font-semibold text-[#15323b]">N?otificações da plataforma</h2>
                <p className="mt-2 max-w-3xl text-sm font-medium leading-6 text-[#6d7f84]">
            ...
            <div className="mt-5 overflow-hidden border border-[#D8E6EB]">
              {isLoading ? (<p className="p-5 text-sm font-semibold text-[#6d7f84]">Carregando histórico...</p>) : platformItems.length === 0 ? (<p className="p-5 text-sm font-semibold text-[#6d7f84]">Nenhuma notificação registrada.</p>) : (<div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-[#D8E6EB] text-left text-sm">
                    <thead className="bg-[#F2F7F9] text-[10px] font-black uppercase tracking-[0.2em] text-[#5F7077]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-[#D8E6EB]">
                      {platformItems.map((item) => (<tr key={item.id} className="align-top">
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex flex-wrap gap-2">
                              {item.channels.length > 0 ? (item.channels.map((channel) => (<span key={channel} className={cn('inline-flex border px-2 py-1 text-[10px] font-black uppercase tracking-[0.16em]', getChannelClassName(channel))}>
                                    {channel}
                                  </span>))) : (<span className="text-xs font-semibold text-[#6d7f84]">Sem canal</span>)}
                            </div>
                          </td>
                          <td className="px-4 py-3 text-xs font-black uppercase text-[#0A3640]">{item.category}</td>
                          <td className="px-4 py-3">
                            <span className={cn('inline-flex border px-2 py-1 text-[10px] font-black uppercase tracking-[0.16em]', item.read_at ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-amber-200 bg-amber-50 text-amber-700')}>
                              {item.read_at ? 'lida' : "no lida"}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-xs font-semibold text-[#6d7f84]">{formatDateTime(item.created_at)}</td>
                        </tr>))}
