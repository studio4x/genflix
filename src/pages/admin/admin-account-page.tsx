          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block">
                ...
                <input value={timezone} onChange={(event) => setTimezone(event.target.value)} className="mt-2 h-12 w-full rounded-2xl border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7] focus:ring-4 focus:ring-[#E8F6FA]"/>
              </label>

              <label className="block">
                <span className="text-xs font-black uppercase tracking-[0.2em] text-[#5F7077]">Idioma</span>
                <input value={locale} onChange={(event) => setLocale(event.target.value)} className="mt-2 h-12 w-full rounded-2xl border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7] focus:ring-4 focus:ring-[#E8F6FA]"/>
              </label>
            </div>
          </div>
        ...
        <form onSubmit={(event) => void handlePasswordSubmit(event)} className="rounded-[28px] border border-[#D8E6EB] bg-[#F2F7F9] p-5">
          ...
          <div className="mt-5 space-y-4">
            ...
            <label className="block">
              ...
              <PasswordField value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} minLength={8} className="mt-2 h-12 w-full rounded-2xl border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7] focus:ring-4 focus:ring-[#E8F6FA]" placeholder="Repita a nova senha" required/>
            </label>
          </div>
