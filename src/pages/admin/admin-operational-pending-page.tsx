...
const pendingItems: PendingItem[] = [
    {
        ...
        area: 'SMTP e domínio',
        status: 'ready',
        description: 'A fila de notificações e o processador de e-mails já estão operando com SMTP ativo no deploy principal. Enquanto a plataforma estiver em construção, o remetente configurado é genflix@e-clix.com.br.',
        missing: [
    ...
    },
    {
        ...
        area: 'Pagamentos',
        status: 'ready',
        description: 'O checkout e os repasses PIX já estão operando com o ambiente de produção do Asaas configurado no deploy principal da plataforma.',
        missing: [
    ...
    },
    {
        ...
        area: 'Pagamentos',
        status: 'ready',
        description: 'A plataforma já possui credenciais e segredo dedicados para sandbox do Asaas, permitindo validar compras e repasses em ambiente de testes sem afetar a produção.',
        missing: [
    ...
    },
    {
        ...
        area: 'Infraestrutura',
        status: 'partial',
        description: 'O projeto segue publicado no domínio temporário da Vercel. Algumas URLs e o remetente final devem ser revisados quando genflix.com.br estiver apontado.',
...
function areaIcon(area: string) {
    if (area.includes('SMTP'))
        return Mail;
    if (area.includes('Pagamento'))
        return WalletCards;
    return KeyRound;
}
    ...
    return (<div className="animate-in space-y-7 fade-in duration-500">
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        {pendingItems.map((item) => {
            ...
            return (<article key={item.title} className="border border-[#D8E6EB] bg-white p-6 shadow-sm">
              ...
              {item.actionTo && item.actionLabel ? (<div className="mt-5">
                  <Link to={item.actionTo} className="inline-flex items-center gap-2 bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-5 py-3 text-sm font-black text-white transition hover:opacity-95">
                    {item.actionLabel}
