...
export function AdminCreatorPayoutsPage() {
  ...
  function toggleCommission(commission: AdminCreatorCommission) {
    ...
    setSelectedCommissionIds((current) => (
      current.includes(commission.id)
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(340px,0.8fr)]">
        <div className="space-y-5">
          ...
          <article className="overflow-hidden border border-[#D8E6EB] bg-white shadow-sm">
            ...
            {isLoading ? (
              <p className="p-5 text-sm font-medium text-[#5F7077]">Carregando comissões...</p>
                      ...
                      return (
                        <tr key={commission.id} className="align-top">
                          ...
                          <td className="px-5 py-4">
                            ...
                            {payoutItemCommissionIds.has(commission.id) ? (
                              <p className="mt-2 text-xs font-semibold text-[#5F7077]">Já incluída em repasse.</p>
                            ) : null}
                          </td>
                        </tr>
                      )
          ...
          <article className="border border-[#D8E6EB] bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 border border-[#D8E6EB] bg-[#F2F7F9] p-4">
              ...
              {selectedCreatorProfile ? (
                <div className="mt-3 space-y-2 text-sm font-semibold text-[#15323b]">
                ...
                <p className="mt-3 text-sm font-semibold text-[#5F7077]">
          ...
          <article className="border border-[#D8E6EB] bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-3">
              {(dashboard?.payouts ?? []).length === 0 ? (
                <p className="text-sm font-medium text-[#5F7077]">Nenhum repasse registrado ainda.</p>
              ) : (
                (dashboard?.payouts ?? []).slice(0, 8).map((payout) => (
                  <div key={payout.id} className="border border-[#D8E6EB] bg-[#F2F7F9] p-4">
