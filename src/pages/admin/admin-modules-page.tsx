...
import { createModule, deleteModule, exportModuleContent, fetchCourse, fetchModules, moveModule, toErrorMessage, updateModule, } from '@/features/admin/content/api';
...
export function AdminModulesPage() {
    ...
    const draftStorageKey = useMemo(() => `admin:modules:${courseId ?? 'unknown'}:editor-draft`, [courseId]);
    const { state: draft, setState: setDraft, clear: clearDraft } = useLocalStorageState<ModuleEditorDraft>(draftStorageKey, initialDraft);
    ...
    async function handleDelete(module: CourseModule) {
        const confirmed = window.confirm(`ATENÇÃO: Excluir o módulo "${module.title}"
...
Todas as aulas vinculadas ser\u00E3o removidas permanentemente.`);
    ...
    async function handleExportModule(module: CourseModule) {
        ...
        try {
            ...
            downloadJsonFile(`m?dulo_${module.title}`, exportData);
        }
        catch (err) {
    ...
    return (<div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="grid gap-8 lg:grid-cols-[380px_1fr] xl:grid-cols-[400px_1fr] items-start">
          ...
          <div className={`p-5 min-h-[64px] flex items-center border-b ${isEditing ? 'bg-amber-50/50 border-amber-100' : 'bg-slate-50 border-slate-100'}`}>
            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              {isEditing ? (<>
                  ...
                  <svg className="h-5 w-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>N?ovo Módulo
                </>)}
            </h3>
          </div>
          ...
          <form className="p-6 grid gap-5" onSubmit={handleSubmit}>
            <div className="space-y-4">
              <label className="block space-y-1.5">
                ...
                <input className="w-full rounded-lg border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm transition-all focus:border-blue-500 focus:ring-1 focus:ring-blue-500 placeholder:text-slate-400" placeholder="Ex: Introdução aos Cuidados" value={form.title} onChange={(event) => setDraft((prev) => ({
            ...prev,
            form: { ...prev.form, title: event.target.value },
              ...
              <label className="block space-y-1.5">
                ...
                <textarea className="min-h-[100px] w-full rounded-lg border-slate-200 bg-white px-4 py-3 text-sm shadow-sm transition-all focus:border-blue-500 focus:ring-1 focus:ring-blue-500 resize-y placeholder:text-slate-400" placeholder="Visão geral sobre o módulo (opcional)." value={form.description} onChange={(event) => setDraft((prev) => ({
            ...prev,
        ...
        <section className="space-y-4">
          ...
          {!isLoading && modules.length === 0 ? (<div className="rounded-2xl border border-dashed border-slate-300 p-12 text-center bg-slate-50/50">
          ...
          <div className="space-y-3">
            {modules.map((module, index) => (<article key={module.id} className={`flex flex-col overflow-hidden rounded-2xl border bg-white shadow-sm transition-all ${editingModuleId === module.id ? 'border-amber-300 ring-1 ring-amber-300' : 'border-slate-200 hover:border-slate-300'}`}>
                <div className="p-4 sm:p-5 flex gap-4">
                  <div className="flex flex-col items-center gap-1 shrink-0">
                    <Button type="button" variant="ghost" size="sm" className="h-7 w-7 p-0 text-slate-400 hover:text-blue-600 hover:bg-blue-50" onClick={() => void handleMove(module, 'up')} disabled={index === 0} title="Mover para cima">
                    ...
                    <Button type="button" variant="ghost" size="sm" className="h-7 w-7 p-0 text-slate-400 hover:text-blue-600 hover:bg-blue-50" onClick={() => void handleMove(module, 'down')} disabled={index === modules.length - 1} title="Mover para baixo">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {module.is_required ? (<span className="inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-200">Obrigatório</span>) : (<span className="inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-500 border border-slate-200">Opcional</span>)}
                        </div>
                      </div>
                      
                      <div className="flex items-center gap-1">
                        <Button type="button" variant="ghost" size="sm" className="h-8 px-3 text-xs font-semibold text-amber-700 hover:bg-amber-50 hover:text-amber-800" onClick={() => handleEdit(module)}>
                          Editar
                        </Button>
                        <Button type="button" variant="ghost" size="sm" className="h-8 w-8 p-0 text-slate-400 hover:text-rose-600 hover:bg-rose-50" onClick={() => void handleDelete(module)} title="Excluir Módulo">
                          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        </Button>
                      </div>
                    </div>
                    
                    {module.description && (<p className="text-sm text-slate-500 line-clamp-2 leading-relaxed">{module.description}</p>)}
                  </div>
                </div>

                <div className="bg-slate-50 border-t border-slate-100 px-4 py-3 flex flex-wrap items-center justify-end gap-2">
                  <Button type="button" variant="outline" size="sm" className="h-8 px-4 text-xs font-semibold bg-white border-slate-200 hover:bg-slate-100 hover:text-slate-900 shadow-sm" onClick={() => void handleExportModule(module)} disabled={exportingModuleId === module.id}>
