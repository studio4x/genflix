...
import { fetchAssessmentAttemptRequests, fetchCompletionReport, fetchLessonAudioModerationRequestsReport, resolveLessonAudioModerationRequestReport, reviewAssessmentAttemptRequest, type AssessmentAttemptRequestReportItem, type CompletionReport, type LessonAudioModerationRequestAdminItem, } from '@/features/admin/reports/api';
...
export function AdminReportsPage() {
    ...
    const pendingRequestsCount = useMemo(() => attemptRequests.filter((item) => item.status === 'pending').length, [attemptRequests]);
    ...
    const pendingLessonAudioRequestsCount = useMemo(() => lessonAudioRequests.filter((item) => item.status === 'pending').length, [lessonAudioRequests]);
    ...
    async function handleReviewRequest(request: AssessmentAttemptRequestReportItem, decision: ReviewDecision) {
        ...
        try {
            ...
            setError("Falha ao analisar solicita??o de nova tentativa.");
        }
        finally {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-4 rounded-xl border bg-white p-4 shadow-sm md:grid-cols-2 lg:grid-cols-4">
        <label className="space-y-1">
          ...
          <select className="w-full rounded-md border border-slate-200 p-2 text-sm" value={selectedCourseId} onChange={(event) => setSelectedCourseId(event.target.value)}>
        ...
        <label className="space-y-1">
          ...
          <input type="text" placeholder="Digite o email do aluno..." className="w-full rounded-md border border-slate-200 p-2 text-sm" value={searchEmail} onChange={(event) => setSearchEmail(event.target.value)}/>
        </label>

        <label className="space-y-1">
          <span className="text-xs font-bold uppercase text-slate-500">Solicitacoes</span>
          <select className="w-full rounded-md border border-slate-200 p-2 text-sm" value={requestFilter} onChange={(event) => setRequestFilter(event.target.value as typeof requestFilter)}>
        ...
        <div className="rounded-lg border border-blue-100 bg-blue-50 p-3">
          ...
          <p className="text-xs text-slate-500">Solicitacoes aguardando an?lise</p>
        </div>
      </div>
      ...
      <section className="space-y-3">
        ...
        <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="border-b bg-slate-50 text-xs font-bold uppercase text-slate-500">
                <tr>
                  ...
                  <th className="px-6 py-4">Avalia??o</th>
                  <th className="px-6 py-4">Tentativas</th>
                  <th className="px-6 py-4">Status</th>
                  <th className="px-6 py-4">An?lise</th>
                </tr>
              </thead>
            ...
            return (<tr key={request.request_id} className="align-top hover:bg-slate-50 transition-colors">
                    ...
                    : "Avalia??o final"}
                        ...
                        {request.requested_message ? (<p className="mt-2 rounded-md bg-slate-50 p-2 text-xs text-slate-600">{request.requested_message}</p>) : null}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${request.status === 'approved'
                    ...
                    : request.status === 'rejected'
                        ...
                        {request.admin_response ? (<p className="mt-2 rounded-md bg-slate-50 p-2 text-xs text-slate-600">{request.admin_response}</p>) : null}
                      </td>
                      <td className="px-6 py-4">
                        {isPending ? (<div className="space-y-3">
                            <label className="block space-y-1">
                              <span className="text-xs font-bold uppercase text-slate-500">Tentativas extras</span>
                              <input type="number" min={1} className="w-28 rounded-md border border-slate-200 px-3 py-2 text-sm" value={extraAttemptsByRequest[request.request_id] ?? '1'} onChange={(event) => setExtraAttemptsByRequest((prev) => ({
                        ...prev,
                        [request.request_id]: event.target.value,
                            ...
                            <label className="block space-y-1">
                              ...
                              <textarea className="min-h-24 w-full min-w-64 rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="Mensagem opcional para registrar a decisao." value={adminResponseByRequest[request.request_id] ?? ''} onChange={(event) => setAdminResponseByRequest((prev) => ({
                        ...prev,
                        [request.request_id]: event.target.value,
                    }))}/>
                            </label>
                            ...
                            <div className="flex flex-wrap gap-2">
                              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" disabled={isReviewing} onClick={() => void handleReviewRequest(request, 'approved')}>
                                Liberar Tentativas
                              </Button>
                              <Button size="sm" variant="outline" className="border-rose-200 text-rose-700 hover:bg-rose-50" disabled={isReviewing} onClick={() => void handleReviewRequest(request, 'rejected')}>
                                Recusar
                              </Button>
                            </div>
                ...
                {filteredRequests.length === 0 && (<tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">Nenhuma solicita??o encontrada para os filtros selecionados.
                    </td>
                  </tr>)}
              </tbody>
            </table>
        ...
        <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
          <div className="border-b bg-slate-50 px-6 py-3">
            <label className="space-y-1">
              ...
              <select className="w-48 rounded-md border border-slate-200 p-2 text-sm" value={lessonAudioRequestFilter} onChange={(event) => setLessonAudioRequestFilter(event.target.value as typeof lessonAudioRequestFilter)}>
                <option value="pending">Pendentes</option>
                <option value="all">Todas</option>
          ...
          <div className="overflow-x-auto">
            ...
            return (<tr key={request.id} className="align-top hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${isPending ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
                      ...
                      <td className="px-6 py-4">
                        {isPending ? (<div className="space-y-2">
                            <textarea className="min-h-20 w-full min-w-64 rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="Resposta opcional para registrar a resolução." value={lessonAudioAdminResponseByRequest[request.id] ?? ''} onChange={(event) => setLessonAudioAdminResponseByRequest((prev) => ({
                        ...prev,
                        [request.id]: event.target.value,
                    }))}/>
                            <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" disabled={isResolving} onClick={() => void handleResolveLessonAudioRequest(request.id)}>
                ...
                {filteredLessonAudioRequests.length === 0 && (<tr>
      ...
      <section className="space-y-3">
        <div>
          <h3 className="text-lg font-bold text-slate-900">Progresso de conclus?o</h3>
        ...
        <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.map((item, idx) => (<tr key={`${item.student_email}-${item.course_title}-${idx}`} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      {item.is_completed ? (<span className="rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-bold text-green-700">Concluido</span>) : (<span className="rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-bold text-amber-700">Em curso</span>)}
                    </td>
