...
import { createResourcesItemsFallback, normalizeResourcesItems, resolveResourceVideoUrl, } from '@/features/public/genflix-resource-items-editor';
import { fetchSiteContent, fetchSiteContentVersions, restoreSiteContentVersion, saveSiteContentEntry, } from '@/features/site-editor/api';
...
export function AdminResourceVideosPage() {
    ...
    const configuredCount = useMemo(() => items.filter((item) => resolveResourceVideoUrl(item) !== '').length, [items]);
    ...
    async function saveResources(targetIndex: number | null = null) {
        ...
        try {
            ...
            setError(saveError instanceof Error ? saveError.message : "N?o foi possvel salvar os dados dos recursos.");
        }
        finally {
            setIsSaving(false);
    ...
    async function handleRestoreCardStyleVersion(version: SiteContentVersion) {
        const confirmed = window.confirm("Deseja reverter o padr\u00E3o dos cards para est? revis\u00E3o Esta a\u00E7\u00E3o cria uma nova revis\u00E3o.");
        if (!confirmed)
            return;
        setMessage(null);
        setError(null);
    ...
    return (<div className="space-y-6">
      ...
      <section className="rounded-[20px] border border-[#D8E6EB] bg-white p-3">
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => setActiveTab('items')} className={`rounded-xl px-4 py-2 text-xs font-black uppercase tracking-[0.14em] ${activeTab === 'items' ? 'bg-[#1398B7] text-white' : 'bg-[#F2F7F9] text-[#5F7077]'}`}>
            Itens de recursos
          </button>
      ...
      {error ? (<div className="rounded-[16px] border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-semibold text-rose-700">
          {error}
        </div>) : null}

      {message ? (<div className="rounded-[16px] border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm font-semibold text-emerald-700">
          {message}
        </div>) : null}
      ...
      {activeTab === 'items' ? (<>
          ...
          <form onSubmit={(event) => void handleItemsSubmit(event)} className="space-y-4">
            ...
            <div className="grid gap-4">
              {items.map((item, index) => {
                ...
                return (<article key={item.id || `${item.label}-${index}`} className="rounded-[20px] border border-[#D8E6EB] bg-white px-5 py-5">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${isConfigured
                    ...
                    <div className="mt-4 grid gap-3">
                      <label className="grid gap-2">
                        ...
                        <input value={currentVideoUrl} onChange={(event) => updateVideoUrl(index, event.target.value)} placeholder="https://..." className="h-11 rounded-[14px] border border-[#D8E6EB] bg-white px-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"/>
                      </label>
                    </div>
                    ...
                    <label className="mt-4 flex items-center justify-between rounded-[14px] border border-[#D8E6EB] bg-[#F8FCFD] px-4 py-3 text-sm font-semibold text-[#15323b]">
                      ...
                      <input type="checkbox" checked={readMoreEnabled} onChange={(event) => updateReadMoreEnabled(index, event.target.checked)}/>
                    </label>
                    ...
                    <div className="mt-4 flex justify-end">
                      <Button type="button" onClick={() => void saveResources(index)} disabled={isSaving || isLoading} className="rounded-none bg-[#1398B7] font-black text-white hover:bg-[#0F7E99]">
          ...
          <section className="grid gap-4 rounded-[20px] border border-[#D8E6EB] bg-white p-5 md:grid-cols-2">
            <label className="grid gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-[#5F7077]">Cor do t?tulo</span>
            ...
            <label className="grid gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-[#5F7077]">Tamanho do t?tulo (px)</span>
            ...
            <label className="grid gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-[#5F7077]">Peso do t?tulo</span>
            ...
            <label className="grid gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-[#5F7077]">Cor da descri??o</span>
            ...
            <label className="grid gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-[#5F7077]">Tamanho da descri??o (px)</span>
            ...
            <label className="grid gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-[#5F7077]">Line-height da descri??o (px)</span>
          ...
          <section className="rounded-[20px] border border-[#D8E6EB] bg-white p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#1398B7]">Hist?rico de revis?es</p>
                <h2 className="mt-1 text-lg font-bold text-[#15323b]">Revisões do padrão dos cards</h2>
                <p className="mt-2 max-w-3xl text-sm leading-6 text-[#5F7077]">Cada salvamento dest? aba gera uma nova revisão. Ao reverter uma revisão, o padrão global dos cards volta para aquele est?do e uma nova revisão é criada automticamente.
                </p>
              </div>
            </div>

            <div className="mt-4 grid gap-3">
              {isLoadingCardStyleVersions ? (<div className="rounded-[16px] border border-dashed border-[#D8E6EB] bg-[#F8FCFD] px-4 py-4 text-sm font-semibold text-[#5F7077]">
                  Carregando histórico...
                </div>) : cardStyleVersions.length === 0 ? (<div className="rounded-[16px] border border-dashed border-[#D8E6EB] bg-[#F8FCFD] px-4 py-4 text-sm font-semibold text-[#5F7077]">
                  Nenhuma revisão registrada para o padrão dos cards ainda.
                </div>) : cardStyleVersions.slice(0, 12).map((version, index) => (<article key={version.id} className="rounded-[16px] border border-[#D8E6EB] bg-[#F8FCFD] px-4 py-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                    ...
                    <Button type="button" variant="outline" disabled={isSaving || isLoading || restoringCardStyleVersionId === version.id} onClick={() => void handleRestoreCardStyleVersion(version)} className="rounded-none border-[#D8E6EB] bg-white font-black text-[#15323b] hover:bg-[#F2F7F9]">
