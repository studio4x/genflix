...
export function AdminSiteEditorPage() {
    ...
    const selectedPage = useMemo(() => pages.find((page) => page.page_key === selectedPageKey) ?? null, [pages, selectedPageKey]);
    ...
    const availableEntryTypes = useMemo(() => Array.from(new Set(entries.map((entry) => entry.entry_type))), [entries]);
    const pageStats = useMemo(() => pages.map((page) => ({
    ...
    const filteredUploadedIconAssets = useMemo(() => {
        ...
        return iconLibraryAssets.filter((asset) => {
            ...
            return (assetName.toLowerCase().includes(normalizedQuery)
    ...
    return (<div className="space-y-7">
      <header className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <Link to="/" className="inline-flex h-11 items-center justify-center bg-gradient-to-b from-[#1398B7] to-[#0A3640] px-5 text-sm font-black text-white">
          Abrir site
        </Link>
      </header>
      <article className="w-full border border-[#D8E6EB] bg-white p-4 shadow-sm">
        <div className="flex h-11 w-full overflow-hidden rounded-[14px] border border-[#D8E6EB] bg-[#F8FBFC]">
          <button type="button" onClick={() => setActiveTab('overrides')} className={'flex-1 px-4 text-xs font-black uppercase tracking-[0.14em] ' + (activeTab === 'overrides' ? 'bg-[#0A3640] text-white' : 'text-[#5F7077]')}>
            Overrides
          </button>
          <button type="button" onClick={() => setActiveTab('icon-library')} className={'inline-flex flex-1 items-center justify-center gap-2 px-4 text-xs font-black uppercase tracking-[0.14em] ' + (activeTab === 'icon-library' ? 'bg-[#1398B7] text-white' : 'text-[#5F7077]')}>
            <Upload className="h-3.5 w-3.5"/>
            Biblioteca de ícones
          </button>
          <button type="button" onClick={() => setActiveTab('typography')} className={'inline-flex flex-1 items-center justify-center gap-2 px-4 text-xs font-black uppercase tracking-[0.14em] ' + (activeTab === 'typography' ? 'bg-[#1398B7] text-white' : 'text-[#5F7077]')}>
      ...
      {message ? (<div className="border border-[#D8E6EB] bg-[#F2F7F9] p-4 text-sm font-bold text-[#15323b]">
          {message}
        </div>) : null}

      {activeTab === 'overrides' ? (<section className="grid gap-4 xl:grid-cols-4">
      ...
      <section className={`grid gap-6 ${activeTab === 'icon-library' || activeTab === 'typography' ? '' : 'xl:grid-cols-[0.85fr_minmax(0,1.15fr)]'}`}>
        <div className="min-w-0 space-y-6">
          ...
          <article className="border border-[#D8E6EB] bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              {(['draft', 'review', 'approved', 'published'] as const).map((status) => (<div key={status} className="flex items-center justify-between rounded-[16px] border border-[#D8E6EB] px-4 py-3 text-sm font-semibold text-[#15323b]">
          ...
          <article className="border border-rose-200 bg-rose-50 p-5">
            ...
            <Button type="button" onClick={() => void handleDisableFallback()} disabled={isSaving} className="mt-5 h-12 w-full rounded-none bg-rose-700 font-black text-white hover:bg-rose-800">
          ...
          <article className="border border-[#D8E6EB] bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid gap-3">
              <Button type="button" variant="outline" disabled={isSaving} onClick={() => void handleSettingsUpdate({ is_enabled: !settings.is_enabled })} className="h-11 rounded-none justify-start border-[#D8E6EB]">
              ...
              <Button type="button" variant="outline" disabled={isSaving} onClick={() => void handleSettingsUpdate({ read_overrides_enabled: !settings.read_overrides_enabled, fallback_mode: settings.read_overrides_enabled })} className="h-11 rounded-none justify-start border-[#D8E6EB]">
                <RotateCcw className="mr-2 h-4 w-4"/>
                {settings.read_overrides_enabled ? 'Usar fallback original' : 'Ler overrides salvos'}
              </Button>
              <Button type="button" variant="outline" disabled={isSaving} onClick={() => void handleSettingsUpdate({ editing_enabled: !settings.editing_enabled })} className="h-11 rounded-none justify-start border-[#D8E6EB]">
          ...
          {activeTab === 'icon-library' ? (<article className="border border-[#D8E6EB] bg-white p-5 shadow-sm">
            ...
            <label className="mt-4 grid gap-2">
              ...
              <div className="flex h-11 items-center gap-2 rounded-[14px] border border-[#D8E6EB] bg-[#F8FBFC] px-3">
                ...
                <input value={iconSearchQuery} onChange={(event) => setIconSearchQuery(event.target.value)} placeholder="Ex.: saúde, vídeo, link, arquivo..." className="w-full border-0 bg-transparent text-sm font-semibold text-[#15323b] outline-none"/>
              </div>
            </label>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="grid gap-2 rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] p-3">
                ...
                <input type="file" accept=".svg,image/svg+xml" onChange={handleSingleIconUpload} disabled={isUploadingIcons || isDeletingAllIcons} className="text-xs font-semibold text-[#15323b] file:mr-3 file:rounded-full file:border file:border-[#D8E6EB] file:bg-white file:px-3 file:py-2 file:text-[10px] file:font-black file:uppercase file:tracking-[0.12em] file:text-[#0A3640]"/>
              </label>
              <label className="grid gap-2 rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] p-3">
                ...
                <input type="file" accept=".svg,image/svg+xml,.zip,application/zip" multiple onChange={handleBulkIconUpload} disabled={isUploadingIcons || isDeletingAllIcons} className="text-xs font-semibold text-[#15323b] file:mr-3 file:rounded-full file:border file:border-[#D8E6EB] file:bg-white file:px-3 file:py-2 file:text-[10px] file:font-black file:uppercase file:tracking-[0.12em] file:text-[#0A3640]"/>
              </label>
            </div>
            ...
            <div className="mt-4 rounded-[18px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={() => void handleDeleteAllUploadedIcons()} disabled={iconLibraryAssets.length === 0 || isDeletingAllIcons || isUploadingIcons} className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-rose-700 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60">
                    {isDeletingAllIcons ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <Trash2 className="h-3.5 w-3.5"/>}
                    Excluir todos
                  </button>
                </div>
              </div>
              {isUploadingIcons || isLoadingIconLibrary ? (<div className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-[#5F7077]">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                ...
                const originalName = typeof asset.metadata?.original_name === 'string' ? asset.metadata.original_name : '';
                const iconName = (asset.alt ?? '').trim() || resolveIconNameFromFileName(originalName);
                return (<article key={asset.id} className="rounded-[14px] border border-[#D8E6EB] bg-white p-3">
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <button type="button" onClick={() => void handleCopyIconKey(iconName)} className="inline-flex items-center gap-2 rounded-full border border-[#D8E6EB] bg-[#F8FBFC] px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-[#0A3640] hover:bg-white">
                          <Copy className="h-3.5 w-3.5"/>
                          Copiar nome
                        </button>
                        <button type="button" onClick={() => void handleDeleteUploadedIcon(asset)} disabled={isDeletingIconId === asset.id || isDeletingAllIcons || isUploadingIcons} className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50 px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-rose-700 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60">
          ...
          {activeTab === 'typography' ? (<article className="border border-[#D8E6EB] bg-white p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Button type="button" variant="outline" onClick={handleTypographyReset} disabled={isSaving} className="rounded-none border-[#D8E6EB]">
                  <RotateCcw className="mr-2 h-4 w-4"/>
                  Limpar
                </Button>
                <Button type="button" onClick={() => void handleTypographySave()} disabled={isSaving} className="rounded-none bg-[#0A3640] text-white hover:bg-[#0A3640]/90">
                  <Save className="mr-2 h-4 w-4"/>
                  Salvar tipografia
                </Button>
              </div>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              {(['h1', 'h2', 'h3', 'body', 'list', 'link'] as SiteTypographyGroupKey[]).map((groupKey) => (<section key={groupKey} className="rounded-[16px] border border-[#D8E6EB] bg-[#F8FBFC] p-4">
                  ...
                  <div className="mt-3 grid gap-3">
                    <label className="grid gap-1">
                      ...
                      <select value={typographyDraft[groupKey].fontFamily} onChange={(event) => handleTypographyGroupChange(groupKey, 'fontFamily', event.target.value)} className="h-11 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]">
                        {SITE_TEXT_FONT_PRESETS.map((fontPreset) => (<option key={fontPreset.family} value={fontPreset.family} style={{ fontFamily: fontPreset.family }}>
                            {fontPreset.label}
                          </option>))}
                      </select>
                    </label>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <label className="grid gap-1">
                        ...
                        <input value={typographyDraft[groupKey].fontSize} onChange={(event) => handleTypographyGroupChange(groupKey, 'fontSize', event.target.value)} placeholder="Ex.: 16px" className="h-11 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"/>
                      </label>
                      <label className="grid gap-1">
                        <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Peso (100-900)</span>
                        <input value={typographyDraft[groupKey].fontWeight} onChange={(event) => handleTypographyGroupChange(groupKey, 'fontWeight', event.target.value)} placeholder="Ex.: 700" className="h-11 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"/>
                      </label>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <label className="grid gap-1">
                        <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Entrelinha (em)</span>
                        <input value={typographyDraft[groupKey].lineHeight} onChange={(event) => handleTypographyGroupChange(groupKey, 'lineHeight', event.target.value)} placeholder="Ex.: 1.5" className="h-11 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"/>
                      </label>
                      <label className="grid gap-1">
                        <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">Letter spacing (em)</span>
                        <input value={typographyDraft[groupKey].letterSpacing} onChange={(event) => handleTypographyGroupChange(groupKey, 'letterSpacing', event.target.value)} placeholder="Ex.: -0.02em" className="h-11 rounded-[12px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"/>
                      </label>
                    </div>
                    <label className="grid gap-1">
        ...
        {activeTab === 'overrides' ? (<div className="min-w-0 space-y-6">
          <article className="border border-[#D8E6EB] bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              ...
              <label className="grid min-w-[280px] gap-2">
                ...
                <select value={selectedPageKey} onChange={(event) => setSelectedPageKey(event.target.value as SitePageKey)} className="h-11 rounded-[14px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none">
                  {pages.map((page) => (<option key={page.page_key} value={page.page_key}>
          ...
          <div className="mt-5 grid gap-4 rounded-[24px] border border-[#D8E6EB] bg-[#F8FBFC] p-4 xl:grid-cols-[1.3fr_0.9fr_0.9fr_auto]">
            ...
            <label className="grid gap-2">
              ...
              <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as 'all' | SiteContentEntry['entry_type'])} className="h-11 rounded-[14px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none">
                <option value="all">Todos</option>
                {availableEntryTypes.map((entryType) => (<option key={entryType} value={entryType}>{formatEntryTypeLabel(entryType)}</option>))}
              </select>
            </label>

            <label className="grid gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.18em] text-[#5F7077]">Status</span>
              <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as 'all' | 'enabled' | 'disabled')} className="h-11 rounded-[14px] border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none">
                <option value="all">Todos</option>
              ...
              <table className="min-w-full divide-y divide-[#D8E6EB] text-left text-sm">
                ...
                <tbody className="divide-y divide-[#D8E6EB]">
                  {pageEntries.map((entry) => {
                    const workspace = workspaceState[createSiteEditorWorkspaceKey(entry.page_key, entry.entry_key)];
                    return (<tr key={entry.id} className="align-top">
                      ...
                      <td className="px-4 py-3 text-xs font-semibold leading-5 text-[#5F7077]">
                        ...
                        {workspace?.comments.length ? (<div className="mt-2 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.14em] text-[#1398B7]">
                      ...
                      {editorMode === 'advanced' ? (<td className="px-4 py-3 text-xs font-semibold text-[#5F7077]">{entry.page_key}</td>) : null}
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
          ...
          {selectedEntryId ? (<div className="mt-6 border border-[#D8E6EB] bg-[#F2F7F9] p-4">
              <h3 className="font-readex text-lg font-semibold text-[#15323b]">Histórico do campo</h3>
              <div className="mt-3 grid gap-2">
                {versions.length === 0 ? (<p className="text-sm font-semibold text-[#5F7077]">Nenhuma versão registrada.</p>) : versions.map((version) => (<div key={version.id} className="border border-[#D8E6EB] bg-white p-3">
