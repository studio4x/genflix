...
function RatingStars({ rating }: {
    ...
    return (<span className="inline-flex items-center gap-0.5 text-[#1398B7]" aria-label={`${rating} de 5 estrelas`}>
      {[1, 2, 3, 4, 5].map((star) => (<Star key={star} className={cn('h-4 w-4', star <= rating ? 'fill-current' : 'text-[#D8E6EB]')}/>))}
    </span>);
}
export function AdminReviewsPage() {
    ...
    return (<div className="space-y-6">
      <header className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <Button type="button" variant="outline" onClick={() => void loadReviews()} disabled={isLoading} className="h-11 rounded-none border-[#D8E6EB] bg-white font-black text-[#0A3640] hover:border-[#1398B7]">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        ...
        ].map(([label, value]) => (<div key={label} className="border border-[#D8E6EB] bg-white p-4 shadow-[0_12px_28px_rgba(10,54,64,0.04)]">
      ...
      <section className="border border-[#D8E6EB] bg-[#F8FBFC] p-4">
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_180px]">
          <label className="relative block">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} className="h-12 w-full border border-[#D8E6EB] bg-white pl-10 pr-4 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Buscar por curso, autor, título ou conteúdo"/>
          </label>

          <select value={status} onChange={(event) => setStatus(event.target.value as ReviewModerationStatus | 'all')} className="h-12 border border-[#D8E6EB] bg-white px-3 text-sm font-bold text-[#15323b] outline-none focus:border-[#1398B7]">
            {Object.entries(statusLabels).map(([value, label]) => (<option key={value} value={value}>{label}</option>))}
          </select>

          <select value={rating ?? ''} onChange={(event) => setRating(event.target.value ? Number(event.target.value) : null)} className="h-12 border border-[#D8E6EB] bg-white px-3 text-sm font-bold text-[#15323b] outline-none focus:border-[#1398B7]">
      ...
      <section className="space-y-4">
        {isLoading && reviews.length === 0 ? (<div className="border border-[#D8E6EB] bg-white p-6 text-sm font-bold text-[#6d7f84]">
            Carregando avaliações...
          </div>) : reviews.length === 0 ? (<div className="border border-dashed border-[#D8E6EB] bg-white p-8 text-center">
          ...
          </div>) : (reviews.map((review) => (<article key={review.id} className="border border-[#D8E6EB] bg-white p-5 shadow-[0_14px_32px_rgba(21,50,59,0.04)]">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={cn('px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.18em]', review.moderation_status === 'approved'
                ...
                : review.moderation_status === 'rejected'
                    ...
                    {review.is_verified_purchase ? (<span className="bg-[#E8F6FA] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-[#0A3640]">
                        Compra verificada
                      </span>) : null}
                  ...
                  {review.moderation_reason ? (<p className="mt-3 border border-red-100 bg-red-50 p-3 text-xs font-bold text-red-700">
                      Motivo da rejeição: {review.moderation_reason}
                    </p>) : null}
                </div>

                <div className="w-full shrink-0 space-y-3 lg:w-72">
                  <textarea value={moderationReason} onChange={(event) => setModerationReason(event.target.value)} rows={3} maxLength={300} className="w-full resize-none border border-[#D8E6EB] bg-white px-3 py-2 text-sm font-semibold leading-6 text-[#15323b] outline-none focus:border-[#1398B7]" placeholder="Motivo opcional para rejeição"/>
                  <div className="grid grid-cols-2 gap-2">
                    <Button type="button" onClick={() => void handleModerate(review.id, 'approve')} disabled={activeReviewId === review.id} className="rounded-none bg-emerald-600 font-black text-white hover:bg-emerald-700">
                      <CheckCircle2 className="mr-2 h-4 w-4"/>
                      Aprovar
                    </Button>
                    <Button type="button" onClick={() => void handleModerate(review.id, 'reject')} disabled={activeReviewId === review.id} variant="destructive" className="rounded-none font-black">
