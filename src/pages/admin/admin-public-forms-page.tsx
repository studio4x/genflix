...
const formDefinitions: SiteFormDefinition[] = [
  {
    id: 'contact',
    label: 'Contato',
    description: 'Formulario publico de contato.',
    route: '/contato',
    pageKey: 'contact',
    editableFields: [
      { key: 'title', label: 'Titulo', entryKey: 'contact.title', pageKey: 'contact', fallback: 'Fale com a gente.' },
      { key: 'description', label: 'Descricao', entryKey: 'contact.description', pageKey: 'contact', fallback: 'Duvidas, sugestoes ou suporte - estamos aqui.' },
      { key: 'submit', label: 'Texto do botao', entryKey: 'contact.form.submit', pageKey: 'contact', fallback: 'Enviar' },
      { key: 'nameLabel', label: 'Rotulo nome', entryKey: 'contact.form.name.label', pageKey: 'contact', fallback: 'Nome completo:' },
  ...
  },
  {
    ...
    pageKey: 'global',
    editableFields: [
      { key: 'title', label: 'Titulo', entryKey: 'global.teach.title', pageKey: 'global', fallback: 'Ensine na GenFlix' },
      { key: 'description', label: 'Descricao', entryKey: 'global.teach.description', pageKey: 'global', fallback: 'Duvidas, sugestoes ou suporte - estamos aqui.' },
      { key: 'submit', label: 'Texto do botao', entryKey: 'global.teach.form.submit', pageKey: 'global', fallback: 'Enviar' },
      { key: 'nameLabel', label: 'Rotulo nome', entryKey: 'global.teach.form.name.label', pageKey: 'global', fallback: 'Nome completo:' },
  ...
  },
  {
    id: 'auth-login',
    label: 'Login',
    description: 'Formulario de acesso do usuario.',
    route: '/login',
    pageKey: 'global',
    editableFields: [
...
export function AdminPublicFormsPage() {
  ...
  const selectedForm = useMemo(
    () => formDefinitions.find((form) => form.id === selectedFormId) ?? formDefinitions[0],
    [selectedFormId],
  )
  ...
  async function saveSelectedFormConfig() {
    ...
    try {
      await Promise.all(
        selectedForm.editableFields.map((field) => saveSiteContentEntry({
        ...
        <div className="flex flex-wrap gap-3">
          <div className="inline-flex rounded-2xl border border-[#D8E6EB] bg-white p-1">
            <button
              ...
              className={`rounded-xl px-3 py-2 text-xs font-black uppercase tracking-[0.16em] ${activeTab === 'submissions' ? 'bg-[#0A3640] text-white' : 'text-[#5F7077]'}`}
            >
              Recebimentos
            </button>
            <button
              ...
              className={`rounded-xl px-3 py-2 text-xs font-black uppercase tracking-[0.16em] ${activeTab === 'forms' ? 'bg-[#0A3640] text-white' : 'text-[#5F7077]'}`}
            >
            ...
            <div className="mt-3 space-y-2">
              {formDefinitions.map((form) => (
                <button
                  ...
                  className={`w-full rounded-2xl border px-4 py-3 text-left transition ${selectedForm?.id === form.id ? 'border-[#1398B7] bg-[#E8F6FA]' : 'border-[#D8E6EB] bg-white hover:border-[#BEE3EA]'}`}
                >
            ...
            {!selectedForm ? null : (
              <>
                <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[#D8E6EB] pb-4">
                  ...
                  <Link
                    to={selectedForm.route}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center rounded-xl border border-[#D8E6EB] px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-[#15323b] hover:bg-[#F2F7F9]"
                  >
                    Editar no site
                  </Link>
                </div>
                ...
                <div className="mt-5 space-y-4">
                  ...
                  {isLoadingConfig ? (
                    <p className="text-sm font-medium text-[#6d7f84]">Carregando configuracoes...</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {selectedForm.editableFields.map((field) => (
                        <label key={field.key} className="space-y-1">
                          ...
                          <input
                            ...
                            className="h-11 w-full rounded-xl border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"
                          />
                        </label>
                      ))}
                    </div>
                ...
                <div className="mt-6 border-t border-[#D8E6EB] pt-5">
                  ...
                  <div className="mt-3 space-y-3">
                    {selectedCustomFields.length === 0 ? (
                      <p className="text-sm font-medium text-[#6d7f84]">Nenhum campo personalizado cadastrado.</p>
                    ) : selectedCustomFields.map((field) => (
                      <div key={field.id} className="rounded-2xl border border-[#D8E6EB] bg-[#F8FBFC] p-3">
                        <div className="grid gap-3 md:grid-cols-2">
                          <input
                            type="text"
                            value={field.label}
                            onChange={(event) => updateCustomField(field.id, (current) => ({ ...current, label: event.target.value }))}
                          ...
                          <select
                            value={field.type}
                            onChange={(event) => updateCustomField(field.id, (current) => ({ ...current, type: event.target.value as CustomFormField['type'] }))}
                            className="h-10 rounded-xl border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7]"
                          >
                          ...
                          <input
                            ...
                            onChange={(event) => updateCustomField(field.id, (current) => ({ ...current, placeholder: event.target.value }))}
                            placeholder="Placeholder (opcional)"
                            className="h-10 rounded-xl border border-[#D8E6EB] bg-white px-3 text-sm font-semibold text-[#15323b] outline-none focus:border-[#1398B7] md:col-span-2"
                          />
                        </div>
                        <div className="mt-3 flex items-center justify-between">
                          <label className="inline-flex items-center gap-2 text-xs font-semibold text-[#5F7077]">
                            <input
                              ...
                              onChange={(event) => updateCustomField(field.id, (current) => ({ ...current, required: event.target.checked }))}
                            />
                            Campo obrigatorio
                          </label>
                          <button
                            ...
                            className="text-xs font-black uppercase tracking-[0.14em] text-[#A53A3A]"
                          >
                            Remover
                          </button>
                        </div>
                ...
                {configFeedback ? (
                  <p className="mt-4 text-sm font-semibold text-[#5F7077]">{configFeedback}</p>
                ) : null}

                <div className="mt-5 flex justify-end">
                  <Button
                    type="button"
                    onClick={() => void saveSelectedFormConfig()}
