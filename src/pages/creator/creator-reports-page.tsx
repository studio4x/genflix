...
import { fetchCreatorCommissions, fetchCreatorCourses, fetchCreatorPayoutSettings, fetchCreatorPayouts, fetchCreatorSalesReport, formatMoneyFromCents, type CreatorCommissionRow, type CreatorCourseSummary, type CreatorPayoutRow, type CreatorPayoutSettingsRow, type CreatorSalesReportRow, } from '@/features/creator/reports/api';
...
export function CreatorReportsPage() {
    ...
    const commissionTotals = useMemo(() => {
        return filteredCommissions.reduce((acc, commission) => {
    ...
    return (<div className="space-y-6">
      ...
      <section className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <label className="block border border-[#D8E6EB] bg-[#F2F7F9] p-4">
          ...
          <select value={selectedCourseId} onChange={(event) => handleCourseChange(event.target.value)} className="mt-2 h-12 w-full border border-[#D8E6EB] bg-white px-4 text-sm font-bold text-[#15323b] outline-none focus:border-[#1398B7]">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {metricCards.map((card) => (<div key={card.label} className="border border-[#D8E6EB] bg-white p-5 shadow-sm">
