    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-4 border-b border-[#D8E6EB] pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm font-medium leading-6 text-[#6d7f84]">
            Acompanhe seus tickets, veja o SLA da primeira resposta e continue a conversa com a equipe quando necessario.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <Button
            ...
            className="h-11 rounded-2xl bg-gradient-to-b from-[#1398B7] to-[#0A3640] font-black text-white hover:opacity-95"
          >
            Novo chamado
          </Button>
        </div>
      ...
      <article className="rounded-[28px] border border-[#BEE3EA] bg-[#F2F8FA] p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#1398B7]">SLA publico de primeira resposta</p>
          ...
          <div className="inline-flex items-center rounded-full border border-[#D8E6EB] bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-[#5F7077]">
            Nao e prazo de resolucao final
          </div>
        </div>
      </article>
      ...
      <section className="overflow-hidden rounded-[30px] border border-[#D8E6EB] bg-white shadow-sm">
        {isLoading ? (
          <p className="px-6 py-8 text-sm font-semibold text-[#5F7077]">Carregando chamados...</p>
        ) : tickets.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <MessageSquare className="mx-auto h-10 w-10 text-[#1398B7]/42" />
            <p className="mt-4 font-readex text-xl font-semibold text-[#15323b]">Voce ainda nao abriu nenhum chamado.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-[900px] w-full border-collapse text-left text-sm">
              ...
              <tbody>
                {tickets.map((ticket) => (
                  <tr key={ticket.id} className="border-t border-[#D8E6EB] align-top transition-colors hover:bg-slate-50/80">
