    ...
    return (<tr className="border-b border-slate-100 last:border-0">
      ...
      <td className="px-4 py-3 text-right">
        <div className="flex flex-wrap justify-end gap-2">
          {invoiceUrl ? (<a href={invoiceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#BFD8E2] bg-[#F0F8FB] px-4 py-2 text-xs font-black tracking-[0.02em] text-[#113845] transition-colors hover:bg-[#E4F2F7]">
              {actionLabel}
              <ExternalLink className="h-3.5 w-3.5"/>
            </a>) : (<span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-100 px-4 py-2 text-xs font-semibold text-slate-500">
              Sem fatura externa
            </span>)}

          <button type="button" onClick={() => onOpenRefundModal(payment)} disabled={!refundAllowed} className="inline-flex items-center gap-2 rounded-full border border-[#E6C87B] bg-[#FFF8E8] px-4 py-2 text-xs font-black tracking-[0.02em] text-[#8A5B00] transition-colors hover:bg-[#FDEEC6] disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400">
            Solicitar reembolso
          </button>
        </div>
      </td>
...
function RefundRequestModal({ payment, isSubmitting, onClose, onSubmit, }: {
    payment: StudentPaymentRecord;
    ...
    return (<div className="fixed inset-0 z-[150] flex items-center justify-center bg-[#09131fb0] p-4 backdrop-blur-sm">
      <div className="w-full max-w-[640px] rounded-[36px] border border-[#D7DEE6] bg-white p-6 shadow-[0_30px_90px_rgba(9,19,31,0.3)] sm:p-8">
        <div className="flex justify-end">
          <button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-full border border-[#D7DEE6] text-[#546273] transition-colors hover:bg-[#F3F6F9]" aria-label="Fechar">
            <X className="h-4 w-4"/>
          </button>
        </div>

        {step === 'options' ? (<div className="space-y-6">
            <div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <Button type="button" asChild className="h-12 flex-1 rounded-full border border-[#BFD8E2] bg-[#EAF5F9] text-base font-black text-[#173847] hover:bg-[#DCEFF6]">
                <Link to="/aluno/suporteopenTicketModal=1&ticketStep=form">Abrir ticket de suporte</Link>
              </Button>

              <Button type="button" onClick={() => setStep('confirm')} className="h-12 flex-1 rounded-full bg-[#282B57] text-base font-black text-white hover:bg-[#202347]">
                Solicitar reembolso
              </Button>
            </div>
            ...
            <label className="grid gap-3">
              ...
              <textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Conta rapidamente o motivo do pedido..." rows={4} className="resize-none rounded-3xl border border-[#D7DEE6] bg-[#F7F9FC] px-4 py-3 text-base text-[#1F2A3D] outline-none"/>
            </label>

            {errorMessage ? (<p className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{errorMessage}</p>) : null}
...
export function StudentPaymentsPage() {
    ...
    return (<div className="space-y-6">
      <header className="flex flex-col gap-5 border-b border-slate-100 pb-7 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Button type="button" variant="outline" onClick={() => void loadHistory(true)} disabled={isLoading || isRefreshing} className="h-12 rounded-2xl border-slate-200 px-5 font-bold text-slate-700">
          <RefreshCw className={`mr-2 h-4 w-4 ${isRefreshing ? 'animate-spin' : ''}`}/>
          Atualizar
        </Button>
      </header>

      {isLoading ? (<section className="flex min-h-[240px] items-center justify-center rounded-[28px] border border-slate-200 bg-slate-50">
      ...
      {!isLoading && error ? (<section className="rounded-[28px] border border-red-200 bg-red-50 p-5 text-red-700">
      ...
      {!isLoading && !error ? (<section className="rounded-[28px] border border-slate-200 bg-white shadow-sm">
          {hasPayments ? (<div className="overflow-x-auto">
              <table className="min-w-full">
                ...
                <tbody>
                  {payments.map((payment) => (<PaymentRow key={payment.id} payment={payment} onOpenRefundModal={setSelectedRefundPayment}/>))}
                </tbody>
              </table>
      ...
      {selectedRefundPayment ? (<RefundRequestModal payment={selectedRefundPayment} isSubmitting={isSubmittingRefund} onClose={() => setSelectedRefundPayment(null)} onSubmit={handleSubmitRefund}/>) : null}
    </div>);
}
