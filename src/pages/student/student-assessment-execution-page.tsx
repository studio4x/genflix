...
import { assessmentInteractionContentSchema, createDefaultImageHotspotResponsePayload, getColoringSlotIds as getColoringInteractionSlotIds, isImageHotspotResponsePayload, } from '@/features/assessments/gamified';
import { buildInteractionAnswerPayload, createStudentImageHotspotAnswerPayload, fetchAssessmentForExecution, fetchOwnAssessmentAttemptRequest, fetchOwnAssessmentReview, fetchStudentCourseAssessments, requestAssessmentAttemptRetry, submitAssessmentAttempt, toErrorMessage, type AssessmentAttemptRequest, type StudentAssessmentCaseStudyWithQuestions, type StudentAssessmentOption, type StudentAssessmentQuestionWithOptions, type StudentAssessmentReview, type StudentCourseAssessmentSummary, type SubmitAssessmentAttemptResult, } from '@/features/student/assessments/api';
...
function shuffleQuestionOptionsForAttempt(questions: StudentAssessmentQuestionWithOptions[], attemptNumber: number) {
...
function buildExecutionItems(questions: StudentAssessmentQuestionWithOptions[], caseStudies: StudentAssessmentCaseStudyWithQuestions[]) {
    return [
...
function getInteractionSlotIds(question: StudentAssessmentQuestionWithOptions) {
    ...
    return parsed.data.segments
        .filter((segment): segment is Extract<typeof parsed.data.segments[number], {
        type: 'blank';
    }> => segment.type === 'blank')
        .map((segment) => segment.id);
}
function isInteractionAnswered(question: StudentAssessmentQuestionWithOptions, interactionAnswers: Record<string, Record<string, string | null>>) {
...
function isImageHotspotAnswered(question: StudentAssessmentQuestionWithOptions, hotspotAnswers: Record<string, ImageHotspotResponsePayload>) {
    const parsed = assessmentInteractionContentSchema.safeParse(question.interaction?.content ?? null);
...
function buildInteractionAnswerState(review: StudentAssessmentReview | null) {
    return Object.fromEntries((review?.answers ?? [])
        ...
        .map((answer) => [
        answer.question_id,
        Object.fromEntries((answer.response_payload && 'entries' in answer.response_payload ? answer.response_payload.entries : [])
...
function buildInteractionFeedbackLookup(question: StudentAssessmentQuestionWithOptions) {
    ...
    const tokenLabels = new Map(('tokens' in parsed.data ? parsed.data.tokens : []).map((token) => [token.id, token.label]));
    if (parsed.data.kind === 'drag_drop_labeling') {
        return {
            slotLabels: new Map(parsed.data.targets.map((target, index) => [
                target.id,
    ...
    if (parsed.data.kind === 'coloring' && parsed.data.render_mode === 'svg_regions') {
        return {
            slotLabels: new Map(parsed.data.regions.map((region, index) => [
                region.region_id,
    ...
    const blanks = parsed.data.segments.filter((segment): segment is Extract<typeof parsed.data.segments[number], {
        type: 'blank';
    }> => segment.type === 'blank');
    return {
        slotLabels: new Map(blanks.map((blank, index) => [
            blank.id,
...
export function StudentAssessmentExecutionPage() {
    ...
    const executionItems = useMemo(() => buildExecutionItems(questions, caseStudies), [caseStudies, questions]);
    const moduleForAssessment = useMemo(() => {
        if (!studentAssessment?.module_id)
            return null;
    ...
    useEffect(() => {
        async function loadAssessment() {
            ...
            try {
                ...
                setSelectedOptions(Object.fromEntries(reviewData.answers
                    .filter((answer) => Boolean(answer.selected_option_id))
                    .map((answer) => [answer.question_id, answer.selected_option_id ?? ''])));
                setTextAnswers(Object.fromEntries(reviewData.answers
    ...
    const progressPercent = executionItems.length > 0
        ...
        : 0;
    const scoredQuestions = useMemo(() => [...questions, ...caseStudies.flatMap((caseStudy) => caseStudy.questions)].filter(isScoredQuestion), [caseStudies, questions]);
    ...
    const possiblePoints = useMemo(() => scoredQuestions.reduce((total, question) => total + getQuestionPossiblePoints(question), 0), [scoredQuestions]);
    ...
    const finalAssessmentBlockReason = useMemo(() => {
        ...
        const incompleteModule = modules.find((module) => {
            if (!module.is_required)
                return false;
            if (!module.lessons.every((lesson) => lesson.is_completed))
                return true;
            if (module.has_required_assessment && !module.required_assessment_approved)
                return true;
            return false;
        });
        if (!incompleteModule) {
            return {
                ...
                description: "A prova final so e liberada depois que todos os requisitos obrigatrios do curso forem concluidos.",
            };
        }
        const blockingQuiz = assessments.find((item) => item.assessment_type === 'module'
        ...
        if (blockingQuiz && !blockingQuiz.last_is_approved) {
            return {
                title: 'Prova Final Bloqueada por Quiz Pendente',
                description: `A prova final so sera liberada depois da aprova??o no quiz "${blockingQuiz.title}" do m?dulo "${incompleteModule.title}".`,
            };
        }
        return {
            title: "Prova Final Bloqueada por Mdulo Incompleto",
    ...
    async function handleSubmit() {
        if (!assessmentId || !courseId || isPerfectApprovedReview)
            return;
                ...
                return {
                    ...
                    response_payload: isImageHotspotQuestion(question)
                        ? (hotspotAnswers[question.id]
                            ?? createStudentImageHotspotAnswerPayload(getImageHotspotContent(question)?.mode ?? 'single_attempt'))
    ...
    function renderChoiceOptions(question: StudentAssessmentQuestionWithOptions) {
        ...
        {question.options.map((option: StudentAssessmentOption) => {
                ...
                return (<button key={option.id} onClick={() => handleOptionSelect(question.id, option.id)} disabled={isPerfectApprovedReview} className={`group relative flex items-center gap-5 rounded-[32px] border-2 p-6 text-left transition-all duration-300 ${isCorrectInReview
                        ? 'border-emerald-500 bg-emerald-50 shadow-lg'
                        : isSelected
                            ? 'border-blue-600 bg-blue-50/50 shadow-xl ring-8 ring-blue-600/5'
                            : 'border-slate-100 bg-white hover:border-blue-200 hover:bg-slate-50'} ${isPerfectApprovedReview ? 'cursor-default' : ''}`}>
              <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-300 ${isCorrectInReview
                        ? 'border-emerald-600 bg-emerald-600 text-white'
                        : isSelected
                            ? 'border-blue-600 bg-blue-600 text-white shadow-lg'
                            : 'border-slate-200 bg-white group-hover:border-blue-300'}`}>
                {(isCorrectInReview || isSelected) ? (<div className="h-2.5 w-2.5 rounded-full bg-white"/>) : null}
              </div>
              <span className={`text-lg font-bold leading-relaxed transition-colors sm:text-xl ${isCorrectInReview
                        ? 'text-emerald-900'
                        : isSelected
                            ? 'text-blue-900'
    ...
    function renderQuestionBody(question: StudentAssessmentQuestionWithOptions) {
        if (isEssayQuestion(question)) {
          ...
          <textarea value={textAnswers[question.id] ?? ''} onChange={(event) => handleEssayAnswerChange(question.id, event.target.value)} readOnly={isPerfectApprovedReview} className={`min-h-[180px] w-full rounded-[32px] border-2 px-6 py-6 text-lg leading-relaxed shadow-sm transition-all focus:ring-8 ${isPerfectApprovedReview
                    ? 'border-emerald-200 bg-emerald-50 text-emerald-900 focus:ring-emerald-100'
                    : 'border-slate-200 bg-white text-slate-700 focus:border-blue-300 focus:ring-blue-100'}`} placeholder="Escreva sua resposta com suas proprias palavras..."/>
          {isPerfectApprovedReview && reviewAnswersByQuestion.get(question.id)?.ai_feedback ? (<div className="rounded-2xl border border-amber-100 bg-amber-50 px-5 py-4 text-sm text-amber-900">
        ...
        if (isImageHotspotQuestion(question)) {
            ...
            return (<ImageHotspotInteraction content={hotspotContent} value={hotspotAnswers[question.id] ?? createDefaultImageHotspotResponsePayload(hotspotContent.mode)} onChange={(payload) => handleHotspotChange(question.id, payload)} readOnly={isPerfectApprovedReview}/>);
        }
        return (<GamifiedInteraction question={question} value={interactionAnswers[question.id] ?? {}} onChange={(slotId, tokenId) => handleInteractionChange(question.id, slotId, tokenId)} readOnly={isPerfectApprovedReview}/>);
    }
    if (isModuleQuizLockedByLessons && !result) {
        return (<div className="mx-auto max-w-2xl animate-in fade-in zoom-in-95 p-4 py-12 duration-500 md:py-24">
        <div className="overflow-hidden rounded-[40px] border border-amber-100 bg-white shadow-2xl shadow-amber-200/20">
          <div className="bg-gradient-to-b from-amber-50/60 to-white p-12 text-center">
            ...
            <p className="mt-4 font-medium leading-relaxed text-slate-500">Para realizar o quiz deste m?dulo, conclua todas as aulas do m?dulo primeiro.
            </p>
            <Button size="lg" className="mt-10 h-14 w-full rounded-2xl bg-slate-900 font-bold shadow-xl shadow-slate-200 hover:bg-slate-800" onClick={() => navigate(firstPendingLessonHref)}>
              Voltar para o Curso
            </Button>
          </div>
        </div>
    ...
    if (studentAssessment?.state === 'failed_limit' && !result) {
        ...
        <div className="overflow-hidden rounded-[40px] border border-rose-100 bg-white shadow-2xl shadow-rose-200/20">
          <div className="bg-gradient-to-b from-rose-50/50 to-white p-12 text-center">
            ...
            {approvalRequirementText ? (<div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-left text-sm font-semibold text-amber-900">
            ...
            <div className="mt-8 rounded-3xl border border-slate-100 bg-slate-50 p-6 text-left">
              ...
              <p className="text-xs leading-relaxed text-slate-500">
                ...
                : isRejectedRetryRequest
                    ? "Sua \u00FAltima solicita\u00E7\u00E3o foi recusada. Se necess\u00E1rio, envie um novo pedido para que a equipe an?lise outra libera\u00E7\u00E3o." : "Voc\u00EA pode enviar uma solicita\u00E7\u00E3o de nova tentativa para an\u00E1lise do administrador diretamente por est? tela."}
              </p>
              {attemptRequest?.admin_response ? (<div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 text-xs text-slate-600">
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <Button size="lg" className="h-14 flex-1 rounded-2xl bg-slate-900 font-bold shadow-xl shadow-slate-200 hover:bg-slate-800" onClick={() => navigate(`/aluno/cursos/${courseId}`)}>
                Voltar para o Curso
              </Button>
    ...
    if (finalAssessmentBlockReason && !result) {
        ...
        <div className="overflow-hidden rounded-[40px] border border-amber-100 bg-white shadow-2xl shadow-amber-200/20">
          <div className="bg-gradient-to-b from-amber-50/60 to-white p-12 text-center">
            ...
            <Button size="lg" className="mt-10 h-14 w-full rounded-2xl bg-slate-900 font-bold shadow-xl shadow-slate-200 hover:bg-slate-800" onClick={() => navigate(`/aluno/cursos/${courseId}`)}>
              Voltar para o Curso
            </Button>
          </div>
        </div>
    ...
    if (error && !result) {
        return (<div className="mx-auto mt-10 max-w-2xl rounded-2xl border border-red-200 bg-red-50 p-8 text-center shadow-sm">
    ...
    if (result) {
        return (<div className="mx-auto max-w-3xl animate-in fade-in zoom-in-95 p-4 py-8 duration-500 md:py-16">
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-200/50">
          <div className={`p-10 text-center ${result.is_approved ? 'bg-gradient-to-b from-emerald-50 to-white' : 'bg-gradient-to-b from-rose-50 to-white'}`}>
            <div className={`mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full border shadow-sm ${result.is_approved ? 'border-emerald-200 bg-emerald-100 text-emerald-600' : 'border-rose-200 bg-rose-100 text-rose-600'}`}>
              {result.is_approved ? (<svg className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/>
                </svg>) : (<svg className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {approvalRequirementText ? (<p className="mx-auto mt-5 max-w-xl text-sm font-semibold leading-relaxed text-slate-600">
                {approvalRequirementText}
              </p>) : null}
          </div>
          ...
          <div className="space-y-8 bg-white p-8 sm:p-10">
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-5 text-center shadow-sm">
                <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Saldo Rest?nte</p>
            ...
            {result.interaction_feedbacks.length > 0 && (<div className="space-y-4 rounded-3xl border border-cyan-100 bg-cyan-50/50 p-6">
                <div>
                ...
                <div className="space-y-3">
                  {result.interaction_feedbacks.map((feedback, index) => (<div key={feedback.question_id} className="rounded-2xl border border-white/70 bg-white p-5 shadow-sm">
                      <div className="flex items-center justify-between gap-4">
                        <p className="text-sm font-black uppercase tracking-widest text-slate-400">Questão {index + 1}</p>
                        <span className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest ${feedback.is_correct ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                      ...
                      <div className="mt-4 grid gap-3">
                        {feedback.kind === 'image_hotspot' ? (<>
                            ...
                            {(feedback.incorrect_target_ids?.length ?? 0) > 0 ? (<div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                            ...
                            {(feedback.outside_click_count ?? 0) > 0 ? (<div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                          ...
                          </>) : feedback.entries?.map((entry, entryIndex) => (<div key={`${feedback.question_id}-${entry.slot_id}`} className={`rounded-2xl border px-4 py-3 text-sm ${entry.is_correct
                            ...
                            {!entry.is_correct ? (<p className="mt-1 font-medium">
            ...
            {result.essay_feedbacks.length > 0 && (<div className="space-y-4 rounded-3xl border border-amber-100 bg-amber-50/50 p-6">
                <div>
                ...
                <div className="space-y-3">
                  {result.essay_feedbacks.map((feedback, index) => (<div key={feedback.question_id} className="rounded-2xl border border-white/70 bg-white p-5 shadow-sm">
                      <div className="flex items-center justify-between gap-4">
                        <p className="text-sm font-black uppercase tracking-widest text-slate-400">Questão {index + 1}</p>
                        <span className={`rounded-full px-3 py-1 text-[11px] font-black uppercase tracking-widest ${feedback.is_correct ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
            ...
            {!result.is_approved && result.remaining_attempts === 0 && (<div className="space-y-4 rounded-3xl border border-rose-100 bg-rose-50/50 p-6">
                <div>
                ...
                <div className="rounded-2xl border border-white/70 bg-white p-5 text-sm text-slate-600 shadow-sm">
                  <p className="font-semibold leading-relaxed">
                    ...
                    : isRejectedRetryRequest
                        ? "Sua \u00FAltima solicita\u00E7\u00E3o foi recusada. Se precisar, voc\u00EA pode enviar um novo pedido por est? tela." : 'Se desejar, envie agora uma solicitação de nova tentativa.'}
                  </p>
                  {attemptRequest?.admin_response ? (<div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
    ...
    return (<div className="mx-auto max-w-[1440px] animate-in fade-in space-y-8 p-4 pb-32 duration-500 sm:p-8">
      <header className="space-y-6">
        ...
        {isPerfectApprovedReview && (<div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm font-bold text-emerald-700">
            Você já concluiu este quiz com 100%. Suas respostas ficaram registradas e uma nova tentativa não é permitida.
          </div>)}

        {approvalRequirementText ? (<div className="rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm font-semibold text-blue-900">
            {approvalRequirementText}
          </div>) : null}

        <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200/60 p-0.5 shadow-inner">
          <div className="h-full rounded-full bg-blue-600 shadow-lg shadow-blue-200 transition-all duration-500 ease-out" style={{ width: `${progressPercent}%` }}/>
        </div>
      </header>

      <main className="space-y-8">
        {currentItem?.type === 'case_study' ? (<article className="overflow-hidden rounded-[48px] border border-slate-200 bg-white shadow-2xl shadow-slate-200/50">
            <div className="p-8 sm:p-14 xl:p-16">
              <div className="space-y-10">
                ...
                <div className="space-y-8">
                  {currentItem.caseStudy.questions.map((question, index) => (<div key={question.id} className="space-y-5 rounded-[32px] border border-slate-200 bg-slate-50/40 p-6">
                      <div className="space-y-3">
                        ...
                        {question.question_type === 'case_study_ai' ? (<div className="rounded-2xl border border-amber-100 bg-amber-50 px-5 py-4 text-sm font-medium text-amber-900">
          ...
          </article>) : currentQuestion ? (<article className="overflow-hidden rounded-[48px] border border-slate-200 bg-white shadow-2xl shadow-slate-200/50">
            <div className="p-8 sm:p-14 xl:p-16">
              <div className="space-y-10">
                <div className="space-y-6">
                  ...
                  {currentQuestion.question_type === 'essay_ai' && (<div className="rounded-2xl border border-amber-100 bg-amber-50 px-5 py-4 text-sm font-medium text-amber-900">
        ...
        <footer className="sticky bottom-4 z-20 flex flex-col-reverse items-center justify-between gap-4 rounded-[32px] border border-white bg-white/50 p-6 shadow-xl shadow-slate-100 backdrop-blur-sm sm:flex-row">
          <Button variant="ghost" size="lg" className="h-14 w-full rounded-2xl px-8 text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-slate-100 hover:text-slate-900 sm:w-auto" disabled={currentQuestionIndex === 0} onClick={() => setCurrentQuestionIndex((index) => index - 1)}>
            QUESTÃO ANTERIOR
          </Button>

          {isPerfectApprovedReview ? (<Button size="lg" className="h-14 w-full rounded-[24px] bg-slate-900 px-12 text-sm font-black uppercase tracking-widest text-white transition-all hover:bg-slate-800 sm:w-auto" onClick={() => navigate(approvedAction.href)}>
              {approvedAction.label}
            </Button>) : isLastQuestion ? (<div className="flex w-full flex-col items-center sm:w-auto">
              <Button size="lg" className="h-14 w-full rounded-[24px] bg-emerald-600 px-12 text-sm font-black uppercase tracking-widest text-white shadow-2xl shadow-emerald-200 transition-all hover:bg-emerald-700 sm:w-auto" disabled={!allQuestionsAnswered || isSubmitting} onClick={handleSubmit}>
                {isSubmitting ? 'ENVIANDO RESPOSTAS...' : 'FINALIZAR AVALIAÇÃO'}
              </Button>
            </div>) : (<Button size="lg" className="h-14 w-full rounded-[24px] bg-blue-600 px-12 text-sm font-black uppercase tracking-widest text-white shadow-2xl shadow-blue-200 transition-all hover:bg-blue-700 sm:w-auto" disabled={!currentQuestionAnswered} onClick={() => setCurrentQuestionIndex((index) => index + 1)}>
              PROXIMA QUESTAO
            </Button>)}
        </footer>
      </main>
