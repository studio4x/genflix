...
export function StudentCoursePlayerLayout() {
    ...
    const filteredSidebarModules = useMemo(() => {
        return modules
            ...
            if (normalizedSidebarQuery.length > 0
                && !matchesModule
            ...
            module: StudentCourseModuleProgress;
            lessons: StudentCourseModuleProgress['lessons'];
    ...
    if (error || !course) {
        return (<div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 p-6">
    ...
    return (<div className="flex h-screen w-full overflow-hidden bg-slate-50 text-slate-900">
      <aside className={`shrink-0 flex flex-col border-r border-slate-200 bg-white transition-all duration-300 ${isSidebarOpen ? 'w-80' : 'w-0 overflow-hidden'}`}>
        ...
        <div className="no-scrollbar flex-1 overflow-y-auto py-2">
          {filteredSidebarModules.map(({ module: m, lessons, moduleQuizzes }, mIdx) => (<div key={m.id} className="mb-4">
              ...
              <div className="mt-1 space-y-0.5 px-2">
                ...
                return (<Link key={l.id} to={isBlocked ? '#' : `/aluno/cursos/${courseId}/player/aulas/${l.id}`} className={`group flex items-start gap-3 rounded-lg p-2.5 text-sm transition-all ${isBlocked ? 'cursor-not-allowed grayscale opacity-50' : 'cursor-pointer hover:bg-slate-100/80'} ${isActive ? 'bg-blue-50 hover:bg-blue-50' : ''}`} onClick={(e) => {
                      ...
                      <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border transition-colors ${l.is_completed
                        ? 'border-emerald-500 bg-emerald-500 text-white'
                        : isActive ? 'border-2 border-blue-500' : 'border-slate-300 bg-white'}`}>
                        {l.is_completed ? (<svg className="h-3 w-3" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>) : isActive ? (<div className="h-1.5 w-1.5 rounded-full bg-blue-500"/>) : null}
                      </div>
                      <div className="flex-1 overflow-hidden">
                        <p className={`truncate font-medium ${isActive ? 'font-bold text-blue-700' : 'text-slate-700'}`}>
                          {l.title}
                        </p>
                        <p className="mt-0.5 flex items-center gap-1 text-[10px] font-semibold text-slate-400">
                          {l.lesson_type === 'video' ? (<svg className="h-3 w-3" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>) : l.lesson_type === 'text' ? (<svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>) : (<svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>)}
                ...
                return moduleQuizzes.map((moduleQuiz) => {
                    ...
                    return (<Link key={moduleQuiz.assessment_id} to={isLocked ? '#' : `/aluno/cursos/${courseId}/player/avaliacoes/${moduleQuiz.assessment_id}`} onClick={(e) => isLocked && e.preventDefault()} className={`relative mt-2 flex items-start gap-3 rounded-2xl border-2 p-3 transition-all group ${isLocked ? 'cursor-not-allowed border-slate-100 bg-slate-50 opacity-50 grayscale' :
                      ...
                      <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-lg border transition-colors ${isApproved ? 'border-emerald-500 bg-emerald-500 text-white' :
                            isFailedLimit ? 'border-rose-200 bg-rose-100 text-rose-500' :
                                isLockedByLessons ? 'border-amber-200 bg-amber-100 text-amber-600' :
                                    'border-blue-400 bg-white text-blue-500'}`}>
                        {isApproved ? (<svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg>) : (<svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>)}
                      </div>
                      <div className="flex-1 overflow-hidden">
                        <p className={`truncate text-[10px] font-black uppercase tracking-tighter ${isFailedLimit ? 'text-rose-600' : isLockedByLessons ? 'text-amber-600' : 'text-blue-600'}`}>
          ...
          {finalAssessment && (<div className="mb-4">
              ...
              <div className="mt-1 space-y-0.5 px-2">
                <Link to={finalAssessment.state === 'blocked' ? '#' : `/aluno/cursos/${courseId}/player/avaliacoes/${finalAssessment.assessment_id}`} onClick={(e) => finalAssessment.state === 'blocked' && e.preventDefault()} className={`relative mt-2 flex items-start gap-3 rounded-2xl border-2 p-3 transition-all group ${finalAssessment.state === 'blocked'
                ...
                : finalAssessment.state === 'failed_limit'
                    ...
                    : finalAssessment.assessment_id === assessmentId
                        ...
                        : finalAssessment.state === 'approved'
                            ...
                            : 'border-slate-100 bg-white hover:border-blue-200'}`}>
                  <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-lg border transition-colors ${finalAssessment.state === 'approved'
                ? 'border-emerald-500 bg-emerald-500 text-white'
                : finalAssessment.state === 'failed_limit'
                    ...
                    : finalAssessment.state === 'blocked'
                        ...
                        : 'border-blue-400 bg-white text-blue-500'}`}>
                    {finalAssessment.state === 'approved' ? (<svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <p className={`truncate text-[10px] font-black uppercase tracking-tighter ${finalAssessment.state === 'failed_limit'
                ? 'text-rose-600'
                : finalAssessment.state === 'blocked'
                    ? 'text-amber-600'
                    : 'text-blue-600'}`}>
                ...
                : finalAssessment.state === 'blocked'
                    ...
                    <p className={`truncate font-bold ${finalAssessment.state === 'approved'
                ? 'text-emerald-700'
                : finalAssessment.state === 'failed_limit'
                    ? 'text-rose-900'
                    : 'text-slate-700'}`}>
      ...
      <main className="flex h-full flex-1 flex-col overflow-hidden bg-slate-50">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="rounded-lg p-2 -ml-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600">
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16"/>
              </svg>
            </button>
            <h2 className="line-clamp-1 border-l border-slate-200 pl-4 text-[10px] font-bold uppercase tracking-wider text-slate-800">{course.title}</h2>
          </div>

          <div className="flex items-center gap-4">
            <div className="hidden items-center gap-3 sm:flex">
              <div className="text-right">
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Seu Progresso</p>
                <p className="text-xs font-bold text-slate-900">{courseProgressPercent}% Concluído</p>
              </div>
