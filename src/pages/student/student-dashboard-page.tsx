...
export function StudentDashboardPage() {
    ...
    function renderTabButton(tab: 'resumo' | 'notificacoes' | 'preferencias', label: string) {
        ...
        return (<button key={tab} type="button" onClick={() => setSearchParams((current) => {
                ...
                return next;
            }, { replace: true })} className={`rounded-full border px-4 py-2 text-xs font-black uppercase tracking-[0.2em] transition ${isActive
    ...
    return (<div className="space-y-8">
      <header className="flex flex-col gap-5 border-b border-slate-100 pb-8 xl:flex-row xl:items-start xl:justify-between">
        ...
        <Button type="button" variant="outline" onClick={() => void loadDashboard()} disabled={isLoading} className="h-12 rounded-2xl border-slate-200 px-5 font-bold text-slate-600">
      ...
      {error ? (<div className="rounded-[28px] border border-red-200 bg-red-50 p-5 text-sm font-medium text-red-600">
          {error}
        </div>) : null}
        ...
        <div className="space-y-6">
          <article className="hidden rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-3">
              {attentionItems.map((item) => (<Link key={item.id} to={item.to} className={`block rounded-[24px] border px-4 py-4 transition-all ${item.tone === 'amber'
          ...
          <article className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-3">
              <Link to="/aluno/cursos" className="flex items-center justify-between rounded-[24px] border border-blue-100 bg-blue-50 px-5 py-4 transition-all hover:border-blue-200 hover:bg-blue-100/60">
              ...
              {featuredCourse ? (<Link to={`/aluno/cursos/${featuredCourse.id}`} className="flex items-center justify-between rounded-[24px] border border-slate-200 bg-slate-50 px-5 py-4 transition-all hover:border-slate-300 hover:bg-slate-100/70">
                  <div className="flex items-center gap-4">
          ...
          <article className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5">
              {recommendedCourses.length > 0 ? (<div className="grid gap-5 xl:grid-cols-2">
                  {recommendedCourses.map((course) => {
                const journeyStatus = getStudentCourseJourneyStatus(courseStatuses.get(course.id) ?? null);
                const learningActionLabel = getLearningActionLabel(journeyStatus, startedCourseIds.has(course.id));
                return (<article key={course.id} className="overflow-hidden rounded-[28px] border border-slate-200 bg-slate-50">
                        <div className="relative overflow-hidden bg-slate-900" style={{ aspectRatio: '4 / 3' }}>
                          {course.thumbnail_url ? (<img src={course.thumbnail_url} alt={course.title} className="h-full w-full object-cover object-center"/>) : (<div className="flex h-full items-center justify-center bg-gradient-to-br from-slate-900 to-slate-700 text-white/20">
                        ...
                        : journeyStatus === 'final_pending'
                          ...
                          <div className="flex flex-wrap gap-3">
                            <Link to={`/aluno/cursos/${course.id}`} className="inline-flex h-11 items-center justify-center rounded-2xl bg-blue-600 px-4 text-sm font-black text-white transition-colors hover:bg-blue-700">
                              {learningActionLabel}
                            </Link>
                            <Link to="/aluno/cursos" className="inline-flex h-11 items-center justify-center rounded-2xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-600 transition-colors hover:text-slate-900">
                              Ver catálogo
                            </Link>
                          </div>
                        </div>
