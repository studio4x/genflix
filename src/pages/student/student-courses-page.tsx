...
import { fetchStartedCourseIds, fetchReleasedCourses, fetchStudentCoursesStatusMap, getStudentCourseJourneyStatus, toErrorMessage, type StudentCourseStatus, } from '@/features/student/courses/api';
...
export function StudentCoursesPage() {
    ...
    return (<div className="space-y-12 animate-in fade-in duration-700 pb-20">
      ...
      {isLoading ? (<div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3].map((item) => (<div key={item} className="h-[400px] animate-pulse rounded-[32px] border border-slate-50 bg-slate-100/50 shadow-sm"/>))}
        </div>) : null}

      {error ? (<div className="flex items-center gap-4 rounded-[24px] border border-rose-200 bg-rose-50 p-6 text-sm font-bold text-rose-600 shadow-sm animate-in shake duration-500">
      ...
      {!isLoading && courses.length === 0 && !error ? (<div className="rounded-[40px] border-4 border-dashed border-slate-100 bg-slate-50/30 p-20 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm font-medium text-slate-500">Assim que novos treinamentos forem designados para voc?, eles aparecerao aqui para serem explorados.
          </p>
        </div>) : null}
        ...
        {courses.map((course) => {
            ...
            return (<article key={course.id} className="group relative flex flex-col overflow-hidden rounded-[32px] border border-slate-100 bg-white shadow-sm transition-all duration-500 hover:border-blue-200 hover:shadow-2xl">
              <Link to={`/aluno/cursos/${course.id}`} className="relative block w-full overflow-hidden bg-slate-200" style={{ aspectRatio: '4/3' }}>
                {course.thumbnail_url ? (<img src={course.thumbnail_url} alt={course.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110"/>) : (<div className="flex h-full w-full items-center justify-center bg-slate-900">
                ...
                <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center bg-blue-600/90 opacity-0 transition-opacity group-hover:opacity-100">
                  <div className="flex h-12 items-center justify-center rounded-2xl bg-white px-6 font-black text-[#15323B] shadow-xl">Acessar Conteúdo
                  </div>
                </div>
              </Link>
              ...
              <div className="flex flex-1 flex-col space-y-4 p-8">
                ...
                <div className="mt-auto flex flex-col gap-5 border-t border-slate-50 pt-6">
                  ...
                  <Link to={`/aluno/cursos/${course.id}`} className="group/btn flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-blue-600 transition-colors hover:text-blue-700">
                    {footerStatus.cta}
