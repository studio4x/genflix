...
function matchesPostCategory(postCategory: string | null, filter: BlogCategoryFilter) {
    ...
    return (normalizedPostCategory === normalizedFilterId ||
...
export function PublicBlogPage() {
    ...
    useEffect(() => {
        ...
        async function loadBlogCategories() {
            ...
            if (isMounted) {
                setBlogCategories(genflixBlogFilters
    ...
    useEffect(() => {
        ...
        const matchedCategory = blogCategories.find((category) => {
            return (normalizeFilterValue(category.id) === normalizedSelectedFilter ||
    ...
    return (<main className="min-h-screen bg-white font-manrope text-[#1f2e39]">
          ...
          <div className="space-y-10">
            <div>
              ...
              <p className="mt-3 text-base leading-7 text-[#20364f] sm:text-lg">
                <EditableText entryKey="blog.hero.subtitle" fallback="Conteúdos para aprofundar seu aprendizado e aplicar o conhecimento com mais confiança." label="Subtítulo da página Blog" pageKey="blog"/>
              </p>
            </div>
            ...
            : 'border-[#ffb57a] bg-[#fff3e8] text-[#ff7a00] hover:border-[#ff7a00] hover:bg-[#ffe6d0]')}>
                ...
                cardImage?: string;
            }).cardImage ?? post.image)} alt={post.title} className="blog-grid-card-image h-full w-full object-cover object-center bg-[#e9ecef]" loading="lazy" decoding="async" fetchPriority="low"/>
                      </div>
                      ...
                      <div className="blog-grid-card-content flex min-h-[360px] flex-1 flex-col p-7">
                        ...
                        <Link to={`/blog/${post.slug}`} className="blog-grid-card-link mt-auto inline-flex items-center gap-1 pt-6 text-sm font-semibold uppercase tracking-[0.08em] text-[#ff7a00] hover:text-[#e86f00]">
                          Leia mais
                ...
                <div className="mt-10 flex items-center justify-center gap-3">
                  <button type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={visibleCurrentPage === 1} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#d8d8d8] bg-white text-[#20364f] transition-colors hover:bg-[#f4f4f4] disabled:cursor-not-allowed disabled:opacity-40">
                    <ChevronLeft className="h-4 w-4"/>
                  </button>

                  {Array.from({ length: pageCount }, (_, index) => index + 1).map((page) => (<button key={page} type="button" onClick={() => setCurrentPage(page)} className={cn('inline-flex h-10 w-10 items-center justify-center rounded-full border text-sm font-semibold transition-colors', visibleCurrentPage === page
                ? 'border-[#243a64] bg-[#243a64] text-white'
                : 'border-[#d8d8d8] bg-white text-[#20364f] hover:bg-[#f4f4f4]')}>
                      {page}
                    </button>))}

                  <button type="button" onClick={() => setCurrentPage((page) => Math.min(pageCount, page + 1))} disabled={visibleCurrentPage === pageCount} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#d8d8d8] bg-white text-[#20364f] transition-colors hover:bg-[#f4f4f4] disabled:cursor-not-allowed disabled:opacity-40">
            ...
            if (!hasCarousel) {
                return fallbackSlide.linkUrl.trim() ? (<a key={`sidebar-block-${blockIndex}`} href={fallbackSlide.linkUrl} className="block cursor-pointer">
                        <img src={fallbackSlide.url} alt={fallbackSlide.alt || 'Imagem lateral do blog'} className="aspect-[7/10] w-full object-cover" loading="lazy"/>
                      </a>) : (<img key={`sidebar-block-${blockIndex}`} src={fallbackSlide.url} alt={fallbackSlide.alt || 'Imagem lateral do blog'} className="aspect-[7/10] w-full object-cover" loading="lazy"/>);
            }
            return (<div key={`sidebar-block-${blockIndex}`} className="aspect-[7/10] w-full overflow-hidden">
                      <div className="flex h-full w-full transition-transform duration-700 ease-in-out" style={{ transform: `translateX(-${activeSlideIndex * 100}%)` }}>
                        {block.slides.map((slide, slideIndex) => (<div key={`sidebar-block-${blockIndex}-slide-${slideIndex}`} className="h-full w-full shrink-0">
                            {slide.linkUrl.trim() ? (<a href={slide.linkUrl} className="block h-full w-full cursor-pointer">
                                <img src={slide.url} alt={slide.alt || 'Imagem lateral do blog'} className="h-full w-full object-cover" loading="lazy"/>
                              </a>) : (<img src={slide.url} alt={slide.alt || 'Imagem lateral do blog'} className="h-full w-full object-cover" loading="lazy"/>)}
                          </div>))}
                      </div>
                    </div>);
