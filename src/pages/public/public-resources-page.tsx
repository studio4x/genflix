...
import { createResourcesItemsFallback, findResourceFallbackByLabel, resolveResourceVideoUrl, } from '@/features/public/genflix-resource-items-editor';
...
function resolveUploadedIconForResource(item: EditableListItem, uploadedIcons: SiteAsset[]) {
    ...
    for (const iconAsset of uploadedIcons) {
        ...
        if (!iconName || !iconAsset.public_url)
            continue;
...
function renderResourceTextContent(value: string, className: string) {
    ...
    return (<div className={`rich-text-content ${className}`} dangerouslySetInnerHTML={{ __html: sanitizeResourceRichText(trimmedValue) }}/>);
}
...
function resolveReadMoreContent(item: EditableListItem | null | undefined) {
    if (!item)
        return '';
    const metadata = getItemMetadata(item);
    if (metadata.readMoreEnabled !== true)
        return '';
...
function ResourceVideoModal({ state, onClose, }: {
    ...
    return (<div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#061b21]/78 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="resource-video-modal-title" onMouseDown={(event) => {
      ...
      <div className="relative h-[80vh] w-[70vw] max-w-[1280px] overflow-hidden rounded-[18px] border border-[#0f5b64]/40 bg-[#04171c] shadow-[0_30px_80px_rgba(6,27,33,0.38)]">
        ...
        <button type="button" onClick={onClose} className="absolute right-3 top-3 z-20 inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-black/45 text-white shadow-sm transition hover:bg-black/62" aria-label="Fechar video">
          <X className="h-5 w-5"/>
        </button>

        <div className="h-full w-full">
          {directFile ? (<video controls autoPlay className="h-full w-full bg-black object-contain" src={state.url}>Seu navegador não suporta reprodução de vídeo.
            </video>) : (<iframe title={state.title} src={embedUrl} allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen className="h-full w-full border-0 bg-black"/>)}
        </div>
      </div>
    </div>);
}
function ResourceReadMoreModal({ state, onClose, }: {
    state: ResourceReadMoreModalState;
    onClose: () => void;
}) {
    return (<div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#061b21]/78 px-4 py-6 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="resource-read-more-modal-title" onMouseDown={(event) => {
            if (event.target === event.currentTarget) {
                onClose();
            }
        }}>
      <div className="relative max-h-[80vh] w-full max-w-[760px] overflow-y-auto rounded-[18px] border border-[#D8E6EB] bg-white p-6 shadow-[0_30px_80px_rgba(6,27,33,0.38)]">
        ...
        <button type="button" onClick={onClose} className="absolute right-3 top-3 z-20 inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#D8E6EB] bg-white text-[#15323b] shadow-sm transition hover:bg-[#F2F7F9]" aria-label="Fechar leia mais">
          <X className="h-5 w-5"/>
        </button>
...
function ResourcesCatalogSection({ cardStyle, uploadedIcons, onOpenVideo, onOpenReadMore, }: {
    ...
    return (<section className="bg-white pb-16 pt-4">
      <div className="public-site-container">
        <div className="mx-auto max-w-[680px] text-center">
          <h1 className="text-[2.35rem] font-extrabold leading-[0.96] tracking-[-0.05em] text-[#183139] sm:text-[2.8rem]">
            <EditableText entryKey="resources.title" fallback="Muito além do vídeo" label="Título da seção de recursos"/>
          </h1>
          <p className="mx-auto mt-4 max-w-[560px] text-base leading-7 text-[#61737a]">
            <EditableText entryKey="resources.description" fallback="Ferramentas pensadas para você aprender, fixar e revisar do seu jeito." label="Descrição da seção de recursos"/>
          </p>
        </div>

        <EditableList entryKey="resources.items" fallback={createResourcesItemsFallback()} label="Cards de recursos">
          {(resourceItems) => (<div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {resourceItems.filter(isEditableItemVisible).map((item) => {
                ...
                return (<article key={item.id} className="rounded-[18px] border px-5 py-5 text-left transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_20px_42px_rgba(19,152,183,0.12)]" style={{
                    ...
                    <h2 className="mt-5 leading-6" style={{
                    ...
                    {videoUrl || readMoreContent ? (<div className="mt-4 flex flex-wrap gap-2">
                        {videoUrl ? (<button type="button" onClick={(event) => {
                            ...
                            }} className="inline-flex items-center rounded-full border border-[#1398B7]/30 bg-white px-3 py-1.5 text-xs font-black uppercase tracking-[0.12em] text-[#0F7E99] hover:bg-[#E8F6FA]" style={{
                        ...
                        {readMoreContent ? (<button type="button" onClick={(event) => {
                                event.stopPropagation();
                                onOpenReadMore({
                                    title: popupItem.label || 'Leia mais',
                                    content: readMoreContent,
                                });
                            }} className="inline-flex items-center rounded-full border border-[#15323B]/20 bg-white px-3 py-1.5 text-xs font-black uppercase tracking-[0.12em] text-[#15323B] hover:bg-[#EAF2F5]" style={{
                                backgroundColor: cardStyle.buttonBackgroundColor,
                                color: cardStyle.buttonTextColor,
                                borderColor: cardStyle.buttonBorderColor,
                                borderRadius: `${cardStyle.buttonRadius}px`,
        ...
        <div className="mt-12 flex justify-center">
          <EditableButton entryKey="resources.cta" fallback={{
            ...
            {(buttonValue) => buttonValue.isHidden === true ? null : (<GenflixCtaButton asChild className="px-5 py-3">
                {buttonValue.isInternal === true ? (<Link to={typeof buttonValue.href === 'string' ? buttonValue.href : '/login'}>
                  ...
                  </Link>) : (<a href={typeof buttonValue.href === 'string' ? buttonValue.href : '/login'} target={buttonValue.openInNewTab === true ? '_blank' : undefined} rel={buttonValue.openInNewTab === true ? 'noreferrer' : undefined}>
                    {typeof buttonValue.label === 'string' ? buttonValue.label : 'Entrar para explorar tudo'}
                  </a>)}
              </GenflixCtaButton>)}
          </EditableButton>
...
export function PublicResourcesPage() {
    ...
    return (<main className="min-h-screen bg-[#F2F7F9] font-manrope text-[#163138]">
      ...
      {activeReadMore ? (<ResourceReadMoreModal state={activeReadMore} onClose={() => setActiveReadMore(null)}/>) : null}
    </main>);
}
