...
import { createSectionRegistryFallback, createSectionRegistrySchema, renderVisibleSectionList, resolveSectionRegistryEntryPrefix, resolveSectionRegistryPageKey, resolveSectionRegistryTemplateKey, SectionStructureControl, } from '@/features/site-editor/section-registry';
...
const contactSectionTemplates = [
    {
        ...
        description: "Bloco principal com imagem lateral, textos e formulário público.",
    },
];
...
function ContactFormSection({ name, email, emailConfirmation, message, feedback, isSubmitting, namePlaceholder, emailPlaceholder, emailConfirmationPlaceholder, messagePlaceholder, onNameChange, onEmailChange, onEmailConfirmationChange, onMessageChange, onSubmit, }: {
    name: string;
    email: string;
    ...
    return (<section className="bg-white pb-0 pt-6">
      <div className="public-site-container">
        <EditableContainer entryKey="contact.form.card" label="Container interno do formulário de contato" pageKey="contact">
          ...
          <EditableImage entryKey="contact.form.image" fallback={{ src: genflixHeroImage, alt: 'Contato GenFlix' }} label="Imagem lateral da página Contato" pageKey="contact">
            {(imageValue) => (<div className="relative min-h-[360px]" style={{
          ...
          <div className="flex items-center px-6 py-12 sm:px-10 lg:px-12">
            <EditableContainer entryKey="contact.form.content.card" label="Container interno do conteúdo de contato" pageKey="contact">
              ...
              <p className="mt-4 text-base leading-7 text-[#5f7178]">
                <EditableText entryKey="contact.description" fallback="Dúvidas, sugestões ou suporte - estamos aqui." label="Descrição de contato"/>
              </p>
              ...
              <form onSubmit={onSubmit} className="mt-8 space-y-5">
                <div>
                  ...
                  <input id="contact-name" type="text" value={name} onChange={(event) => onNameChange(event.target.value)} required placeholder={namePlaceholder} className="h-12 w-full rounded-[12px] border border-[#D8E6EB] bg-[#EDF4F6] px-4 text-sm text-[#183139] outline-none transition-colors placeholder:text-[#9babb0] focus:border-[#1398B7] focus:bg-white"/>
                </div>
                ...
                <div>
                  ...
                  <input id="contact-email" type="email" value={email} onChange={(event) => onEmailChange(event.target.value)} required placeholder={emailPlaceholder} className="h-12 w-full rounded-[12px] border border-[#D8E6EB] bg-[#EDF4F6] px-4 text-sm text-[#183139] outline-none transition-colors placeholder:text-[#9babb0] focus:border-[#1398B7] focus:bg-white"/>
                </div>

                <div>
                  <label htmlFor="contact-email-confirmation" className="mb-2 block text-sm font-medium text-[#4f656c]">
                    <EditableText entryKey="contact.form.email_confirmation.label" fallback="Reinsira seu e-mail:" label="Rótulo de reinsira e-mail"/>
                  </label>
                  <input id="contact-email-confirmation" type="email" value={emailConfirmation} onChange={(event) => onEmailConfirmationChange(event.target.value)} required placeholder={emailConfirmationPlaceholder} className="h-12 w-full rounded-[12px] border border-[#D8E6EB] bg-[#EDF4F6] px-4 text-sm text-[#183139] outline-none transition-colors placeholder:text-[#9babb0] focus:border-[#1398B7] focus:bg-white"/>
                </div>
                ...
                <div>
                  ...
                  <textarea id="contact-message" value={message} onChange={(event) => onMessageChange(event.target.value)} required rows={5} placeholder={messagePlaceholder} className="w-full rounded-[16px] border border-[#D8E6EB] bg-[#EDF4F6] px-4 py-3 text-sm text-[#183139] outline-none transition-colors placeholder:text-[#9babb0] focus:border-[#1398B7] focus:bg-white"/>
                </div>
...
export function PublicContactPage() {
    ...
    return (<main className="min-h-screen bg-[#F2F7F9] font-manrope text-[#163138]">
      ...
      {renderVisibleSectionList(contactSections, (item) => {
            ...
            if (templateKey === 'form') {
                return (<EditableContainer entryKey={`${sectionEntryPrefix}.layout`} label="Bloco Formulário da página Contato" pageKey={sectionPageKey}>
              <ContactFormSection name={name} email={email} emailConfirmation={emailConfirmation} message={message} feedback={feedback} isSubmitting={isSubmitting} namePlaceholder={namePlaceholder} emailPlaceholder={emailPlaceholder} emailConfirmationPlaceholder={emailConfirmationPlaceholder} messagePlaceholder={messagePlaceholder} onNameChange={setName} onEmailChange={setEmail} onEmailConfirmationChange={setEmailConfirmation} onMessageChange={setMessage} onSubmit={handleSubmit}/>
            </EditableContainer>);
            }
            return null;
        })}
