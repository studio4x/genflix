...
import { genflixCommunityItems, genflixNavLinks, } from '@/features/public/genflix-site-content';
...
const communitySectionTemplates = [
    {
        ...
        description: "Abertura com título, descrição e grade de comunidades.",
    },
    {
        id: 'support',
    ...
    },
    {
        id: 'newsletter',
        label: 'Newsletter',
        description: "Bloco global de captação compartilhado com outras páginas públicas.",
...
function CommunityHeroSection({ communityItems }: {
    communityItems: EditableListItem[];
}) {
    return (<section className="bg-white pb-14 pt-4">
      <div className="public-site-container">
        <EditableContainer entryKey="community.hero.header.card" label="Container interno do cabeçalho da comunidade" pageKey="community">
          <div className="mx-auto max-w-[760px] text-center">
            <h1 className="text-[2.35rem] font-extrabold leading-[0.96] tracking-[-0.05em] text-[#183139] sm:text-[2.8rem]">
              <EditableText entryKey="community.hero.title" fallback="Tire dúvidas, troque experiências e aprofunde o que aprendeu." label="Título da comunidade"/>
            </h1>
            <p className="mx-auto mt-4 max-w-[720px] text-base leading-7 text-[#61737a]">
              <EditableText entryKey="community.hero.description" fallback="Escolha sua comunidade, entre na conversa e leve o aprendizado além da tela. Aqui as dúvidas viram debate e o debate vira conhecimento." label="Descrição da comunidade"/>
            </p>
          </div>
        </EditableContainer>
        ...
        <EditableContainer entryKey="community.hero.grid.wrap" label="Container interno da grade de comunidades" pageKey="community">
          ...
          <EditableList entryKey="community.items" fallback={communityItems} label="Comunidades">
            ...
            return (<article key={item.id} className="group rounded-[14px] bg-[linear-gradient(180deg,#1398B7_0%,#0A3640_100%)] px-4 py-4 text-white shadow-[0_10px_20px_rgba(10,54,64,0.18)] transition-transform duration-300 hover:-translate-y-1">
...
function CommunitySupportSection() {
    return (<section className="bg-white pb-16">
      <div className="public-site-container">
        <EditableContainer entryKey="community.support.card" label="Container interno do bloco de apoio" pageKey="community">
          ...
          <div className="relative overflow-hidden rounded-[22px] bg-[#173039]">
            <img src="/images/genflix/home/featured-6.jpg" alt="Comunidade GenFlix" className="h-full w-full object-cover grayscale"/>
            <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(11,31,36,0.18)_0%,rgba(11,31,36,0.5)_100%)]"/>
            <button type="button" aria-label="Assistir video institucional" className="absolute left-1/2 top-1/2 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/36 bg-white/12 text-white backdrop-blur-sm transition-transform hover:scale-105">
          ...
          <div className="flex items-center">
            <div className="max-w-[460px]">
              ...
              <p className="mt-4 text-[15px] leading-8 text-[#5f7178]">
                <EditableText entryKey="community.support.paragraph1" fallback="Oferecer a profissionais e estudantes universitários conteúdos modernos, objetivos e práticos, a preços justos, para que a compreensão aconteça de verdade. Nas comunidades da GenFlix, esse conhecimento ganha continuidade em conversas, trocas e repertório compartilhado." label="Texto de apoio 1"/>
              </p>
              <p className="mt-4 text-[15px] leading-8 text-[#5f7178]">
                <EditableText entryKey="community.support.paragraph2" fallback="O espaço foi pensado para aproximar quem aprende, estimular o debate e transformar dúvidas em avanço concreto. Cada área reúne temas, experiências e perspectivas que ajudam a estender o estudo para além da aula." label="Texto de apoio 2"/>
              </p>
...
export function PublicCommunityPage() {
    ...
    const communityItems = useEditableValue('community.items', genflixCommunityItems.map((item) => ({
        id: item.label,
        label: item.label,
    ...
    return (<main className="min-h-screen bg-[#F2F7F9] font-manrope text-[#163138]">
      ...
      {renderVisibleSectionList(communitySections.filter(isEditableItemVisible), (item) => {
            ...
            if (templateKey === 'hero') {
                return (<EditableContainer entryKey={`${sectionEntryPrefix}.layout`} label="Bloco Hero da página Comunidade" pageKey={sectionPageKey}>
              <CommunityHeroSection communityItems={communityItems}/>
            </EditableContainer>);
            }
            if (templateKey === 'support') {
                return (<EditableContainer entryKey={`${sectionEntryPrefix}.layout`} label="Bloco Apoio da página Comunidade" pageKey={sectionPageKey}>
