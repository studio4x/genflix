...
const aboutParagraphs = [
    "Ha quem prefira abrir o livro, sublinhar, fazer anotacoes e avancar pgina por pgina. Ha tambm quem se envolva mais com videos, aulas curtas e recursos interativos.",
...
const aboutSectionTemplates = [
    {
        ...
        description: "Bloco de abertura com título e subtítulo da página Sobre.",
    },
    {
        id: 'story',
...
function AboutHeroSection() {
    return (<section className="bg-white pb-10 pt-8">
        ...
        <EditableContainer entryKey="about.hero.card" label="Container interno do Hero" pageKey="about">
          <div className="rounded-[30px] bg-[#F2F7F9] px-8 py-14 text-center shadow-[0_20px_44px_rgba(21,50,59,0.04)] sm:px-12">
            ...
            <p className="mt-4 text-base font-medium text-[#1398B7]">
              <EditableText entryKey="about.hero.subtitle" fallback="Porque aprender e ensinar não precisa ser complicado." label="Subtítulo da página Sobre"/>
            </p>
          </div>
        </EditableContainer>
      </div>
...
function AboutStorySection({ richTextFallback }: {
    richTextFallback: string;
}) {
    return (<section className="bg-white pb-16 pt-4">
      <div className="public-site-container">
        <EditableContainer entryKey="about.story.card" label="Container interno da História" pageKey="about">
          <div className="rounded-[28px] border border-[#D8E6EB] bg-white px-8 py-10 shadow-[0_18px_42px_rgba(21,50,59,0.03)] sm:px-10 lg:px-12">
            <EditableContainer entryKey="about.story.contentWrap" label="Container de conteúdo da História" pageKey="about">
              <div className="mx-auto max-w-[960px] border-t border-[#BEE3EA] pt-10">
                <EditableRichText entryKey="about.storyContent" fallback={richTextFallback} label="Conteúdo da página Sobre" pageKey="about" className="rich-text-content space-y-5 text-[15px] leading-8 text-[#4f666d]"/>
              </div>
            </EditableContainer>
          </div>
        </EditableContainer>
...
function AboutMissionSection() {
    return (<section className="bg-white pb-20">
      <div className="public-site-container">
        <EditableContainer entryKey="about.mission.card" label="Container interno da Missão" pageKey="about">
          ...
          <div className="max-w-[520px]">
            ...
            <p className="mt-4 text-[15px] leading-8 text-[#5f7178]">
              <EditableText entryKey="about.mission.description" fallback="Oferecer a estudantes universitários, concurseiros e profissionais conteúdos modernos, objetivos, práticos, de qualidade e a preços adequados, com materiais que proporcionem compreensão rápida e efetiva daquilo que os usuários desejam aprender." label="Descrição da missão"/>
            </p>
            <p className="mt-5 text-sm font-semibold text-[#1398B7]">
              <EditableText entryKey="about.mission.tagline" fallback="Porque aprender e ensinar não precisa ser complicado." label="Frase final da missão"/>
            </p>
...
export function PublicAboutPage() {
    ...
    const legacyParagraphs = useEditableValue('about.paragraphs', aboutParagraphs.map((paragraph, index) => ({
    ...
    return (<main className="min-h-screen bg-[#F2F7F9] font-manrope text-[#163138]">
      ...
      <SectionStructureControl buttonLabel="Gerenciar blocos da página" pageKey="about" entryKey="about.layout.sections" label="Estrutura da página Sobre" sections={aboutSections} schema={aboutLayoutSchema}/>
      {renderVisibleSectionList(aboutSections.filter(isEditableItemVisible), (item) => {
            ...
            if (templateKey === 'hero') {
                return (<EditableContainer entryKey={`${sectionEntryPrefix}.layout`} label="Bloco Hero da página Sobre" pageKey={sectionPageKey}>
              <AboutHeroSection />
            </EditableContainer>);
            }
            if (templateKey === 'story') {
                return (<EditableContainer entryKey={`${sectionEntryPrefix}.layout`} label="Bloco História da página Sobre" pageKey={sectionPageKey}>
              <AboutStorySection richTextFallback={storyRichTextFallback}/>
            </EditableContainer>);
            }
            if (templateKey === 'mission') {
                return (<EditableContainer entryKey={`${sectionEntryPrefix}.layout`} label="Bloco Missão da página Sobre" pageKey={sectionPageKey}>
              <AboutMissionSection />
            </EditableContainer>);
            }
