...
export function PublicLegalPageShell({ documentKey, eyebrow, title, summary, updatedAt, children, currentPage, }: PublicLegalPageShellProps) {
          ...
          <div className="border-b border-[#D8E6EB] bg-[#F2F8FA] px-6 py-10 sm:px-10 lg:px-12">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="rounded-[22px] border border-[#D8E6EB] bg-white px-5 py-4 shadow-sm">
                ...
                <EditableButton entryKey={`legal.${title}.backLoginButton`} fallback={{ label: 'Voltar ao login', href: '/login', isInternal: true, tone: 'surface', isHidden: false }} label="Botão voltar ao login">
                  {(buttonValue) => buttonValue.isHidden === true ? null : (<GenflixCtaButton asChild tone={normalizeGenflixCtaTone(buttonValue.tone)} className="mt-4 px-4 py-2 text-xs font-black uppercase tracking-[0.2em]">
            ...
            <article className="rounded-[28px] border border-[#D8E6EB] bg-white p-6 shadow-sm sm:p-8 lg:p-10">
              <div className="space-y-8">
                ...
                <div className="border-t border-[#D8E6EB] pt-8">
                  <PublicTextBlocksSection entryKey={`legal.${documentKey}.textBlocks`} label="Blocos de texto" title="Adicionar novos textos" description="Use este bloco para incluir trechos adicionais, observacoes, avisos ou complementos ao documento." fallback={[]}/>
                </div>
              </div>
            </article>
          </section>
