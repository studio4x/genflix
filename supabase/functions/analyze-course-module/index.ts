...
const MODEL_PRICING_USD_PER_1M: Record<AiProvider, Record<string, {
    input: number;
    output: number;
}>> = {
    openai: {
...
function resolveConfiguredApiKey(envKey: string, dbKey: string, provider: AiProvider) {
...
Deno.serve(async (request) => {
    ...
    try {
        ...
        if (roleError || !isAdmin) {
            return jsonResponse({ error: "Apenas administradores podem usar est? an?lise." }, 403);
        }
        const { courseId, moduleId } = requestBody;
        if (!courseId || !moduleId) {
            return jsonResponse({ error: "courseId e moduleId sao obrigatrios." }, 400);
        }
        ...
        if (courseResult.error)
            return jsonResponse({ error: courseResult.error.message }, 400);
        if (moduleResult.error)
            return jsonResponse({ error: moduleResult.error.message }, 400);
        if (lessonsResult.error)
            return jsonResponse({ error: lessonsResult.error.message }, 400);
        if (assessmentsResult.error)
            return jsonResponse({ error: assessmentsResult.error.message }, 400);
        if (standardsResult.error)
            return jsonResponse({ error: standardsResult.error.message }, 400);
...
An?lise um modulo individual de curso e retorne um unico objeto JSON valido.
...
function extractJsonObject(value: string) {
    ...
    throw new Error("Gemini retornou texto sem um objeto JSON v?lido.");
}
async function generateAnalysisJson(prompt: string, providers: {
    openAiApiKey: string;
    geminiApiKey: string;
    ...
    throw new Error("Nenhum provedor de IA disponvel para a an?lise.");
}
async function generateWithOpenAi(prompt: string, apiKey: string) {
    ...
    if (!text || typeof text !== 'string') {
        return {
            ok: false as const,
            errorText: "OpenAI no retornou contedo textual v?lido.",
        };
    }
    return {
        ok: true as const,
...
async function generateWithGemini(prompt: string, apiKey: string) {
    ...
    if (!rawText || typeof rawText !== 'string') {
        throw new Error("Gemini no retornou um payload v?lido.");
    }
    return {
        text: rawText,
        usage: buildUsageMetrics({
...
function calculateEstimatedCostUsd(input: {
    ...
    const inputCost = (input.inputTokens / 1000000) * pricing.input;
    const outputCost = (input.outputTokens / 1000000) * pricing.output;
...
async function buildAssessmentsWithQuestions(supabase: ReturnType<typeof createClient>, assessmentIds: string[], assessments: Record<string, unknown>[]) {
    ...
    for (const question of questions) {
        ...
        if (typeof assessmentId !== 'string' || typeof questionId !== 'string')
            continue;
