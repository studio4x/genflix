...
type QuestionType = 'single_choice' | 'essay_ai' | 'case_study_ai' | 'case_study_single_choice' | 'drag_drop_labeling' | 'fill_in_the_blanks' | 'image_hotspot' | 'coloring';
...
type InteractionFeedback = TokenMappingInteractionFeedback | ImageHotspotInteractionFeedback;
...
Deno.serve(async (request) => {
    ...
    try {
        ...
        if (!assessmentId || !rawAnswers) {
            return jsonResponse({ error: "assessment_id e answers sao obrigatrios." }, 400);
        }
        const answers = normalizeAnswers(rawAnswers);
        const answerMap = new Map(answers.map((answer) => [answer.question_id, answer]));
        if (answerMap.size !== answers.length) {
            return jsonResponse({ error: "Existem respostas duplicadas para a mesma quest?o." }, 400);
        }
        ...
        if (!assessment) {
            return jsonResponse({ error: "Avalia??o no encontrada ou inativa." }, 404);
        }
        ...
        if (releaseError || !isReleased) {
            return jsonResponse({ error: "Avalia??o no liberada para o usurio." }, 403);
        }
        if (assessment.assessment_type === 'module') {
            ...
            if (unlockError || !isUnlocked) {
                return jsonResponse({ error: "Mdulo bloqueado para avalia??o." }, 403);
            }
        }
        else {
            ...
            if (completedError || !allRequiredCompleted) {
                return jsonResponse({ error: "Avalia??o final bloqueada ate concluir os m?dulos obrigatrios." }, 403);
            }
        }
        ...
        if (attemptsUsed >= effectiveMaxAttempts) {
            return jsonResponse({ error: "Limite de tentativas atingido para est? avalia??o." }, 400);
        }
        ...
        const essayQuestions = questions.filter((question) => question.question_type === 'essay_ai' || question.question_type === 'case_study_ai');
        ...
        const essayInputs = essayQuestions
            ...
            if (!answerText && question.is_required) {
                throw new Error("Todas as questoes obrigatrias devem ser respondidas.");
            }
            return answerText
                ? { question, answerText }
                : null;
        ...
        const essayEvaluations = await Promise.all(essayInputs.map(({ question, answerText }) => evaluateEssayAnswer({
        ...
        for (const question of questions) {
            ...
            if (question.question_type === 'essay_ai' || question.question_type === 'case_study_ai') {
                ...
                if (!answerText && question.is_required) {
                    return jsonResponse({ error: "Todas as questoes obrigatrias devem ser respondidas." }, 400);
                }
...
function normalizeAnswers(rawAnswers: unknown[]): SubmissionAnswerInput[] {
    return rawAnswers.map((item) => {
        ...
        const responsePayload = answer.response_payload && typeof answer.response_payload === 'object'
            ...
            : null;
        if (!questionId) {
            throw new Error("Quest?o invalida.");
        }
        return {
...
function normalizeImageHotspotAnswerKey(content: Record<string, unknown> | null, answerKey: AnswerKeyRow['answer_key']) {
...
function scoreInteractionAnswer(input: {
    ...
    if (normalizedPayload.kind === 'image_hotspot') {
        ...
        const correctTargetIds = normalizeImageHotspotAnswerKey(input.interaction?.content ?? null, input.answerKey?.answer_key ?? null);
        if (correctTargetIds.length === 0) {
            throw new Error("Quest?o de hotspot sem gabarito configurado.");
        }
        const foundTargetIds = Array.from(new Set(normalizedPayload.found_target_ids.filter((targetId) => targetIds.has(targetId) && correctTargetIds.includes(targetId))));
        const incorrectTargetIds = Array.from(new Set(normalizedPayload.incorrect_target_ids.filter((targetId) => targetIds.has(targetId) && !correctTargetIds.includes(targetId))));
    ...
    const answerEntries = normalizeInteractionAnswerKeyEntries(input.interaction?.content ?? null, Array.isArray(input.answerKey?.answer_key?.entries) ? input.answerKey?.answer_key?.entries : []);
    if (answerEntries.length === 0) {
        throw new Error("Quest?o gamificada sem gabarito configurado.");
    }
...
function normalizeInteractionAnswerKeyEntries(content: Record<string, unknown> | null, rawEntries: Array<{
    slot_id?: string;
    token_id?: string;
}>) {
...
function getInteractionSlotIds(content: Record<string, unknown> | null) {
    ...
    if (content.kind === 'drag_drop_labeling') {
        ...
        return targets
            .map((target) => (target && typeof target === 'object' && typeof target.id === 'string'
            ? target.id.trim()
            : ''))
            .filter(Boolean);
    }
    ...
    if (content.kind === 'coloring') {
        if (content.render_mode === 'svg_regions') {
            ...
            return regions
                .map((region) => (region && typeof region === 'object' && typeof region.region_id === 'string'
...
function getInteractionTokenIds(content: Record<string, unknown> | null) {
    ...
    return content.tokens
        .map((token) => (token && typeof token === 'object' && typeof token.id === 'string'
        ? token.id.trim()
        : ''))
        .filter(Boolean);
}
async function insertAttemptWithRetry(input: {
    ...
    throw new Error("N?o foi possvel registrar a tentativa da avalia??o.");
}
async function evaluateEssayAnswer(input: {
    ...
    throw new Error("Nenhum provedor de IA disponvel para corrigir questoes discursivas.");
}
async function evaluateWithOpenAi(input: {
    ...
    if (typeof rawContent !== 'string') {
        return {
            ok: false as const,
            error: "OpenAI no retornou um contedo v?lido.",
        };
    }
    return {
        ok: true as const,
...
async function evaluateWithGemini(input: {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContentkey=${apiKey}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    if (typeof rawText !== 'string') {
        throw new Error("Gemini no retornou um payload v?lido para a correcao.");
    }
...
function buildEssayPrompt(input: {
    ...
    return `
Voc? e um corretor pedagogico rigoroso de quiz.
Compare a resposta do aluno com o gabarito esperado e retorne apenas um objeto JSON v?lido.
...
function extractJsonObject(value: string) {
    ...
    throw new Error("A IA no retornou um JSON v?lido para a correcao.");
}
function normalizeEssayEvaluation(rawJson: string) {
    ...
    return {
        ...
        feedback: feedback || "Sua resposta foi analisada, mas no foi possvel gerar um comentario detalhado.",
    };
}
