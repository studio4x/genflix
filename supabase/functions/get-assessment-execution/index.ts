...
Deno.serve(async (request) => {
    ...
    try {
        ...
        if (!assessmentId) {
            return jsonResponse({ error: "assessment_id obrigatrio." }, 400);
        }
        ...
        if (adminRoleError || studentRoleError) {
            return jsonResponse({ error: "Falha ao validar permiss?o do usurio." }, 500);
        }
        if (!isAdmin && !isStudent) {
            return jsonResponse({ error: "Usurio sem permiss?o para acessar avalia??es." }, 403);
        }
        ...
        const [questionsResult, optionsResult, caseStudiesResult, interactionsResult] = await Promise.all([
            ...
            supabaseAdmin
                ...
                .select('*')
                .in('question_id', (((await supabaseAdmin
                .from('assessment_questions')
                .select('id')
                .eq('assessment_id', assessmentId)).data ?? []) as Array<{
                id: string;
...
async function withSignedAssetUrl(supabaseAdmin: ReturnType<typeof createClient>, interaction: InteractionRow) {
