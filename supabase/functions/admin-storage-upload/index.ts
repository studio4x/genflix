...
import { buildObjectPath, createSignedPutUrl, deleteObject, getMaxUploadSizeBytes, resolveStorageProvider, type StorageProvider, } from '../_shared/storage-provider.ts';
...
Deno.serve(async (request) => {
    ...
    try {
        ...
        if (hasAdminRoleResult.error || hasCreatorRoleResult.error) {
            return jsonResponse(request, { error: "Falha ao validar perfil do usurio." }, 500);
        }
...
async function handlePrepareUpload(request: Request, requestBody: Record<string, unknown>, supabaseAdmin: ReturnType<typeof createClient>) {
    ...
    if (!entityId || !fileName) {
        return jsonResponse(request, { error: "entity_id e file_name sao obrigatrios." }, 400);
    }
    if (!Number.isFinite(fileSizeBytes) || fileSizeBytes <= 0) {
        return jsonResponse(request, { error: "file_size_bytes inv?lido." }, 400);
    }
...
async function handleDeleteObject(request: Request, requestBody: Record<string, unknown>, supabaseAdmin: ReturnType<typeof createClient>) {
    ...
    if (!storagePath) {
        return jsonResponse(request, { error: "storage_path obrigatrio para deletar objeto." }, 400);
    }
    await deleteObject({
        provider,
...
function resolveR2Bucket() {
    const bucket = Deno.env.get('R2_PRIVATE_BUCKET')?.trim() ?? '';
    if (!bucket) {
        throw new Error("R2_PRIVATE_BUCKET no configurado.");
    }
    return bucket;
}
